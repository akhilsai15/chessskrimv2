...
export const MOCK_USERS = [
  {
    user: "Bappu Bhai Sharma",
    handle: "@bappu_bhai",
    avatar: "https://i.pravatar.cc/150?u=bappu",
  },
  {
    user: "Pappu Pandey",
    handle: "@pappu_pass_hogaya",
    avatar: "https://i.pravatar.cc/150?u=pappu",
  },
  {
  ...
  {
    user: "Munni Lal",
    handle: "@munni_badnaam_nahi",
    avatar: "https://i.pravatar.cc/150?u=munni",
  },
  {
    user: "Bablu Mechanic",
    handle: "@bablu_ka_garage",
    avatar: "https://i.pravatar.cc/150?u=bablu",
  },
  {
    user: "Pinky Patel",
    handle: "@pinky_se_pink",
    avatar: "https://i.pravatar.cc/150?u=pinky",
  },
  {
    user: "Golu Mishra",
    handle: "@golu_fitness_goals",
    avatar: "https://i.pravatar.cc/150?u=golu",
  },
];
...
export function calculateSkrimScore(
  post: any,
  userMood: string,
  userHistory: any = {},
) {
  ...
  const moodScore =
    post.mood === userMood
      ? 100
      : RELATED_MOODS[userMood]?.includes(post.mood)
        ? 60
        : 10;

  // Mock friend activity
  const friendPulses = Math.floor(Math.random() * 5);
  const friendScore = friendPulses >= 3 ? 80 : friendPulses >= 1 ? 50 : 10;
...
export function generateSinglePost(
  mood: string,
  forceNewTime = false,
  idxOverride?: number,
) {
...
export function assembleFeed(
  ...
  const normalPosts = posts.filter(
    (p) => p.type === "post" || p.type === "collab_post",
  );
  const specialCards = posts.filter(
    (p) => p.type === "suggested_user" || p.type === "pulse_battle",
  );
  ...
  for (let i = 0; i < posts.length; i++) {
    if (
      posts[i].type === "suggested_user" ||
      posts[i].type === "pulse_battle"
    ) {
