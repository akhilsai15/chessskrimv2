...
export function hasSentRequest(
  fromUsername: string,
  ...
  return requests.some(
    (r: any) =>
      r.fromUsername === fromUsername && r.targetUsername === targetUsername,
  );
}
...
export function sendRequest(
  fromUsername: string,
  targetUsername: string,
  message: string = "Hey! Let's connect.",
  fromAvatar?: string,
) {
  const requests = getMessageRequests();
...
export function getUserCounts(): Record<
  string,
...
export function saveUserCounts(
...
export function unfollowUser(
  ...
  if (arr.includes(targetUsername)) {
    ...
    counts[targetUsername].followers = Math.max(
      0,
      counts[targetUsername].followers - 1,
    );
    saveUserCounts(counts);
