...
export function updateSparkFlow(chatId: string): {
...
export function getTierInfo(count: number) {
  ...
  if (count >= 30)
    return {
      ...
      shadow: "drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]",
    };
  if (count >= 14)
    return {
      emojis: "🔥🔥🔥",
      ...
      shadow: "drop-shadow-[0_0_6px_rgba(249,115,22,0.8)]",
    };
  if (count >= 7)
    return {
      emojis: "🔥🔥",
      ...
      shadow: "drop-shadow-[0_0_4px_rgba(239,68,68,0.6)]",
    };
  if (count >= 4)
    return {
      emojis: "🔥",
      name: "Heating Up",
      iconSize: 18,
      cssClass: "text-[#FF4500]",
      shadow: "drop-shadow-[0_0_2px_rgba(255,69,0,0.4)]",
    };
  if (count >= 1)
    return {
      emojis: "🔥",
