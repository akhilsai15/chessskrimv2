  ...
  return (
    <div className="flex flex-col p-2 min-w-[220px]">
      <div className="flex items-center gap-3">
        ...
        >
          {playing ? (
            <Pause size={20} className={isMe ? "fill-current" : ""} />
          ) : (
            <Play
              size={20}
              className={isMe ? "fill-current ml-0.5" : "ml-0.5"}
            />
          )}
    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <mark
            key={i}
            className="bg-[rgba(255,200,0,0.3)] text-inherit rounded-[2px] px-[1px]"
          >
            {part}
          </mark>
        ) : (
...
export function MessageBubble({
  ...
  // Text size logic
  const textSizeClass =
    customization?.textSize === "small"
      ? "text-xs"
      : customization?.textSize === "large"
        ? "text-lg"
        : "text-[15px]";
  ...
  let borderRadius = "";
  if (isMe) {
    const tr = isConsecutiveTop
      ? "4px"
      : customization?.font === "rounded"
        ? "24px"
        : "18px";
    const br = isConsecutiveBottom
      ? "4px"
      : customization?.font === "rounded"
        ? "24px"
        : "18px";
    ...
    const tl = isConsecutiveTop
      ? "4px"
      : customization?.font === "rounded"
        ? "24px"
        : "18px";
    const bl = isConsecutiveBottom
      ? "4px"
      : customization?.font === "rounded"
        ? "24px"
        : "18px";
  ...
  if (
    ...
    if (isMe) {
      ...
      if (currentMoodObj) {
        ...
        if (
          customization.bubbleColor.startsWith("#") ||
          customization.bubbleColor.startsWith("hsl")
        ) {
          bubbleStyle.background = customization.bubbleColor;
        } else {
          const bc = BUBBLE_COLORS.find(
            (c) => c.id === customization.bubbleColor,
          );
          bubbleStyle.background = bc
            ? `linear-gradient(135deg, ${bc.gradient[0]}, ${bc.gradient[1]})`
            : defaultSentGradient;
        }
      ...
      if (customization?.theirBubbleStyle === "dark") {
        ...
        const tint = customization?.bubbleColor?.startsWith("#")
          ? `${customization.bubbleColor}30`
          : customization?.bubbleColor?.startsWith("hsl")
    ...
    if (isMe && isPulsed) {
      ...
      bubbleStyle.boxShadow =
        "0 0 10px rgba(255,215,0,0.5), 0 2px 12px rgba(176,38,255,0.3)";
    }

    if (isSearchFocused) {
      bubbleStyle.background = isMe
        ? "rgba(255,200,0,0.6)"
        : "rgba(255,200,0,0.5)";
      bubbleStyle.border = "1px solid #FFD700";
      ...
      if (
        ...
        (message.translation &&
          message.translation.toLowerCase().includes(searchQuery.toLowerCase()))
      ) {
    ...
    switch (message.status) {
      ...
      case "read":
        return (
          <CheckCheck
            size={14}
            className="text-[#00B4FF] ml-1 inline drop-shadow-[0_0_2px_rgba(0,180,255,0.8)]"
          />
        );
    }
  };
  ...
  const handleEncryptTap = (e: React.MouseEvent) => {
    e.stopPropagation();
    setShowEncryptTooltip(true);
    setTimeout(() => setShowEncryptTooltip(false), 3000);
  };

  if (message.type === "call") {
    return (
      <div className="mx-auto my-3 text-center">
        <div className="bg-[#1A1A24]/90 border border-white/10 rounded-full px-5 py-2.5 inline-block cursor-pointer hover:bg-white/10 transition-colors shadow-lg">
          <div className="text-white/90 text-[13px] font-medium flex items-center gap-2">
            {message.text}
          </div>
          <div className="text-white/40 text-[10px] flex items-center justify-center gap-1.5 mt-1">
            {message.missed ? "Tap to call back" : "Encrypted call"}{" "}
  ...
  const renderTimeAndStatus = () => (
    <div
      className={`text-[10px] text-white/50 flex items-center mt-1 select-none ${isMe ? "justify-end" : "justify-start"}`}
    >
      {message.time}
      {message.encrypted && (
        <div
          className="relative inline-flex items-center ml-1 cursor-pointer"
          onClick={handleEncryptTap}
        >
          <svg
          ...
          <AnimatePresence>
            {showEncryptTooltip && (
              <motion.div
                ...
                className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-[#1A1A24]/95 backdrop-blur-md border border-[#B026FF]/50 shadow-[0_0_15px_rgba(176,38,255,0.3)] rounded-xl py-2 px-3 text-xs text-left z-50 pointer-events-none"
              >
                <div className="font-bold text-white mb-1">
                  🔐 AES-256-GCM Encrypted
                </div>
                <div className="text-white/80 leading-tight">
                  This message is secured with military-grade crypto. Only you
                  can read this.
                </div>
              </motion.div>
            )}
  ...
  const renderContent = () => {
    switch (message.type) {
      case "text":
        return (
          <div
            className={`px-4 py-2.5 leading-relaxed text-white break-words ${textSizeClass}`}
          >
            {isPulsed && (
              <div className="text-[#FFD700] text-sm font-bold flex items-center gap-1 mb-1">
              ...
              >
                <div className="flex items-start gap-2 text-white font-bold">
                  ...
                  <div className="break-words leading-relaxed">
                    {highlightMatch(message.translation, searchQuery)}
                  </div>
                </div>
                <div className="flex justify-between items-center mt-1">
                  <span className="text-[10px] text-white/50 font-medium">
                    Translated from {message.language || "language"}
                  </span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      ...
                      window.dispatchEvent(
                        new CustomEvent("hide-translation", {
                          detail: message.id,
                        }),
                      );
                    }}
            ...
            {isPulsed && (
              <div className="text-[#FFD700]/80 text-[10px] mt-1">
                [Pulsed with energy! 🔥]
              </div>
            )}
          </div>
      ...
      case "gif":
        return (
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${message.gif.color}40, ${message.gif.color})`,
            }}
          >
            <div className="absolute inset-0 bg-black/10" />
            <div className="flex flex-col items-center z-10">
              <span
                className="text-6xl filter drop-shadow-md mb-2"
                style={{
                  animation: `gif-${message.gif.animation} 2s infinite`,
                }}
              >
                {message.gif.emoji}
              </span>
      ...
      case "challenge":
        ...
        return (
          <div
            className={`p-4 min-w-[240px] ${isMe ? "opacity-90" : ""}`}
            style={{
              border: isMe
          ...
          >
            <div
              className={`text-[10px] font-bold uppercase tracking-wider mb-2 ${isMe ? "text-white/70" : "text-[#10b981]"}`}
            >
              🎮 Game Challenge!
            </div>
            ...
            <div className="flex justify-between items-center text-[11px] text-white/50 mb-4 font-mono bg-black/30 p-2 rounded">
              ...
              <span>
                {Math.max(
                  1,
                  Math.floor((message.expiresAt - Date.now()) / 3600000),
                )}
                h
              </span>
            </div>
              ...
              isMe ? (
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-[#10b981] hover:bg-[#059669] text-white text-xs font-bold py-2.5 rounded-lg transition-colors"
                  >
                    🎮 Accept!
                  </button>
                  <button
                    onClick={onDeclineChallenge}
                    className="bg-white/10 hover:bg-white/20 text-white/70 text-xs font-medium px-4 py-2.5 rounded-lg transition-colors"
                  >
                    Decline
                  </button>
                </div>
      ...
      case "challenge_result":
        const iWon =
          message.winnerId ===
          (isMe ? message.challengerId : message.opponentId);
        ...
        return (
          <div
            className={`p-4 min-w-[240px] border ${color}`}
            style={{ borderRadius }}
          >
            ...
            <div className="space-y-1 mb-3 text-sm">
              <div className="flex justify-between items-center text-white">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-bold">
                    {message.challengerScore} ⚡
                  </span>
                  {message.winnerId === message.challengerId && (
              ...
              <div className="flex justify-between items-center text-white">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {message.winnerId === message.opponentId && (
                    <span className="text-xs">👑 WINNER!</span>
                  )}
                </div>
              </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-white/10 hover:bg-white/20 text-white text-xs font-bold py-2 rounded transition-colors flex items-center justify-center gap-1"
              >
                🔄 Rematch!
              </button>
              {!iWon && !isTie && (
                <button
                  onClick={onAcceptChallenge}
                  className="bg-white/10 hover:bg-white/20 text-white/70 text-xs font-medium px-3 py-2 rounded transition-colors"
                >
                  😤 Accept
                </button>
              )}
          ...
          <div className="flex flex-col">
            <div
              onClick={
                !isPhotoUploading
                  ? () =>
                      window.dispatchEvent(
                        new CustomEvent("open-media-viewer", {
                          detail: message,
                        }),
                      )
                  : undefined
            ...
            >
              ...
              <div
                ...
                style={{
                  filter:
                    message.photo.filter === "Vivid"
                      ? "saturate(200%)"
                      : message.photo.filter === "Cool"
                        ? "hue-rotate(90deg)"
                        : message.photo.filter === "Warm"
                          ? "sepia(50%)"
                          : "none",
                }}
              >
      ...
      case "file":
        ...
        return (
          <div className="flex flex-col">
            <div
              className={`p-4 rounded-xl flex items-center gap-3 w-[260px] ${isMe ? "bg-black/20 border border-white/10" : "bg-black/40 border border-white/10"}`}
            >
              <div
                className={`w-10 h-10 rounded-lg flex items-center justify-center ${message.file.fileType === "pdf" ? "bg-red-500/20 text-red-400" : message.file.fileType === "excel" ? "bg-green-500/20 text-green-400" : "bg-blue-500/20 text-blue-400"}`}
              >
                <File size={24} />
              </div>
              <div className="flex-1 overflow-hidden flex flex-col justify-center">
                <div className="text-white text-sm font-bold truncate mb-1">
                  {highlightMatch(message.file.name, searchQuery)}
                </div>
                <div className="text-white/50 text-[11px] font-medium whitespace-nowrap">
                  {message.file.size} · {message.file.fileType.toUpperCase()}
                </div>
              </div>
      ...
      case "song":
        return (
          <div
            className={`p-3 rounded-2xl flex items-center gap-3 w-[260px] relative overflow-hidden backdrop-blur-md border border-white/10 shadow-lg`}
            style={{
              background: `linear-gradient(135deg, ${message.song.color}40, rgba(0,0,0,0.6))`,
            }}
          >
            <div
              className="absolute top-0 right-0 w-24 h-24 rounded-full mix-blend-screen opacity-20 filter blur-xl"
              style={{ backgroundColor: message.song.color }}
            ></div>
            <div
              className="w-14 h-14 rounded-xl flex items-center justify-center shadow-md relative z-10"
              style={{ backgroundColor: message.song.color }}
            >
            ...
            <div className="flex-1 z-10">
              <div className="text-white text-sm font-bold truncate">
                {highlightMatch(message.song.title, searchQuery)}
              </div>
              <div className="text-white/70 text-[10px] uppercase tracking-wider mb-2">
                {message.song.movie} ·{" "}
                {highlightMatch(message.song.artist, searchQuery)}
              </div>

              {/* Mini player mock */}
              <div className="flex items-center gap-2">
                <button className="text-white hover:text-neon-purple">
        ...
        return (
          <div className="flex flex-col bg-black/40 rounded-2xl border border-white/10 overflow-hidden w-[240px]">
            <div className="w-full h-24 bg-gradient-to-br from-green-500/20 to-teal-600/40 relative flex items-center justify-center overflow-hidden">
              ...
              <MapPin
                size={28}
                className="text-red-500 drop-shadow-md z-10"
                fill="currentColor"
              />
            </div>
            <div className="p-3">
      ...
      case "announcement":
        return (
          <div className="w-full flex justify-center py-2 relative">
            {message.pinned && (
              <div className="absolute top-0 right-4 p-1 bg-white/10 rounded-full text-[10px]">
                📌
              </div>
            )}
                ...
                <div className="flex flex-col gap-1 text-[11px] text-white/50">
                  ...
                  <div>
                    ...
                    <span
                      ...
                      onClick={() => onSeenByClick && onSeenByClick()}
                    >
                      {(message as any).seenBy || 4} members
                    </span>
                  </div>
                </div>
              </div>
      ...
      case "poll":
        return (
          <div
            className={`p-4 min-w-[260px] ${isMe ? "opacity-95" : ""}`}
            style={{
              border: isMe
                ? "1px solid rgba(255,255,255,0.2)"
                : "1px solid #B026FF",
          ...
          >
            <div
              className={`text-[10px] font-bold uppercase tracking-wider mb-2 flex items-center gap-1.5 ${isMe ? "text-white/70" : "text-[#B026FF]"}`}
            >
              <span>📊</span> POLL
            </div>
              ...
              {message.options.map((opt) => {
                const percent =
                  message.totalVotes > 0
                    ? Math.round((opt.votes / message.totalVotes) * 100)
                    : 0;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <span
                        className={`text-sm font-medium ${isSelected ? "text-white font-bold" : "text-white/80"}`}
                      >
                        {opt.text}{" "}
                        {isSelected && (
                          <Check
                            size={14}
                            className="inline text-green-400 ml-1"
                          />
                        )}
                      </span>
                    </div>
    ...
    <div {...wrapperProps}>
      ...
      >
        ...
        >
          {message.replyTo && (
            <div
              className={`mx-3 mt-3 mb-1 p-2 rounded-lg text-[13px] border-l-2 ${isMe ? "bg-black/20 border-white/50 text-white/90" : "bg-white/10 border-neon-purple text-white/80"}`}
            >
              <div
                className="font-bold text-xs mb-0.5"
                style={{ color: isMe ? "rgba(255,255,255,0.7)" : "#B026FF" }}
              >
