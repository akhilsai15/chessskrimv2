...
export function PulseCommentsSheet({
  ...
  const [comments, setComments] = useState([
    ...
    {
      id: "4",
      handle: "dolly_ka_dhaba",
      text: "Ekdum mast hai bhai 💜",
      pulses: 15,
      time: "4h ago",
      avatar: "https://i.pravatar.cc/150?u=dolly",
    },
    {
      id: "5",
      handle: "chikoo_official",
      text: "Gym ke baad ye dekhna 😂",
      pulses: 31,
      time: "5h ago",
      avatar: "https://i.pravatar.cc/150?u=chikoo",
    },
  ]);
  ...
  const handlePulseComment = (id: string) => {
    setComments(
      comments.map((c) => (c.id === id ? { ...c, pulses: c.pulses + 1 } : c)),
    );
  };
          ...
          >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-4 mt-2">
                      <button
                        ...
                        onClick={() => handlePulseComment(c.id)}
                      >
                      ...
                      <button
                        className="flex items-center gap-1 group"
                        onClick={() => setCommentInput(`@${c.handle} `)}
                      >
            ...
            <div className="p-4 border-t border-white/10 bg-[#141414] shrink-0 sticky bottom-0">
              <div className="flex gap-3 items-center">
                <img
                  src={currentUser?.avatar || "https://i.pravatar.cc/150?u=you"}
                  alt="You"
...
export function PulseShareSheet({
  ...
  const [view, setView] = useState<"main" | "connect" | "veil" | "more">(
    "main",
  );
              ...
              <h3 className="text-lg font-bold text-white pl-4 flex items-center gap-2">
                {view !== "main" && (
                  <button
                    ...
                    className="mr-2 p-1 hover:bg-white/10 rounded-full transition-colors text-white"
                  >
                    ←
                  </button>
                )}
            ...
            <div className="p-4 overflow-y-auto max-h-[60vh] no-scrollbar">
              {view === "main" && (
                <div className="flex flex-col gap-2">
                  <button
                    onClick={() =>
                      handleShare("spark", "Added to your Sparks! ✨")
                    }
                  ...
                  >
                    ...
                    <div>
                      <div className="text-white font-semibold">
                        Share as Spark Story
                      </div>
                      <div className="text-xs text-gray-400 mt-0.5">
                        Post card to your Sparks
                      </div>
                    </div>
                  </button>
                ...
                <div className="flex flex-col gap-2">
                  {["priya_vibes", "raju_3idiots", "amit_kumar"].map((u, i) => (
                    ...
                    >
                      ...
                      <div className="ml-auto bg-[#00F0FF]/20 text-[#00F0FF] font-medium text-xs px-3 py-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                        Send
                      </div>
                    </button>
                  ))}
              ...
              {view === "veil" && (
                <div className="flex flex-col gap-2">
                  {["amit_kumar", "secure_node_7"].map((u, i) => (
                    <button
                      key={u}
                      onClick={() => handleShare("veil", "Sent via Veil 🔒")}
                      className="flex items-center gap-4 p-3 rounded-2xl hover:bg-white/10 transition-colors text-left group border border-[#B026FF]/20"
                    >
                      ...
                      <span className="text-white font-semibold">@{u}</span>
                      <div className="ml-auto bg-[#B026FF]/20 text-[#B026FF] font-medium text-xs px-3 py-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                        Encrypt & Send
                      </div>
                    </button>
                  ))}
              ...
              {view === "more" && (
                <div className="grid grid-cols-4 gap-4 pt-2">
                  <button
                    onClick={() => handleShare("arattai", "Shared to Arattai!")}
                  ...
                  <button
                    onClick={() => {
                      ...
                      handleShare("copy", "Link copied to clipboard! 📋");
                    }}
                    className="flex flex-col items-center gap-2 group"
                  >
