...
interface Props {
  ...
  onSeenByClick?: (msg: Message) => void;
        ...
        <AnimatePresence initial={false}>
              ...
              <React.Fragment key={message.id}>
                {(() => {
                  const currentMoodObj =
                    message.sender === "me"
                      ? CHAT_MOODS.find((m) => m.id === message.mood)
                      : null;
                  ...
                  return (
                    <motion.div
                      ...
                      layout="position"
                      className={`relative ${searchMode && activeSearchId === message.id ? "z-50" : ""}`}
                    >
                      ...
                      <MessageBubble
                        ...
                        isSearchFocused={
                          searchMode && activeSearchId === message.id
                        }
                        onLongPress={() => onLongPress?.(message)}
                        onReactionClick={() => onReactionClick?.(message)}
                        onSeenByClick={() => onSeenByClick?.(message)}
          ...
          {isOtherTyping && (
            <motion.div
              ...
              layout="position"
            >
              <div className="px-4 py-3.5 rounded-[18px_18px_18px_4px] bg-white/[0.08] backdrop-blur-[10px] border border-white/[0.06] flex items-center justify-center gap-1 min-w-[60px]">
                <motion.div
