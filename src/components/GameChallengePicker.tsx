...
export function GameChallengePicker({
  onClose,
  onSendChallenge,
  opponentName,
}: Props) {
                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      {game.isMultiplayer ? (
                        ...
                        <div className="flex items-center gap-1 bg-yellow-500/20 text-yellow-400 px-2.5 py-1 rounded-full text-xs font-bold">
                          <Zap size={12} fill="currentColor" /> My:{" "}
                          {game.defaultScore}
                        </div>
                      )}
                    </div>
          ...
          <div className="flex flex-col h-full relative">
            <div className="p-6 pb-4 border-b border-white/5 bg-gradient-to-b from-neon-purple/10 to-transparent">
              <button
                ...
                className="text-white hover:text-white/70 flex items-center gap-2 mb-4"
              >
                <ArrowLeft size={20} />
                <span className="font-bold">
                  {selectedGame.label} Challenge
                </span>
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div>
                ...
                <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar pb-1">
                  {(
                    CHALLENGE_MESSAGES[selectedGame.id] ||
                    CHALLENGE_MESSAGES.snake
                  ).map((msg, i) => (
                    <button
                      key={i}
              ...
              <div className="flex items-center justify-between bg-white/5 px-4 py-3 rounded-2xl">
                <div className="flex items-center gap-2 text-white">
                  <Clock size={18} className="text-white/70" /> Expires in:
                </div>
                <select
                  value={expiryHours}
