...
export function SparkCreator({ isOpen, onClose, onPost }: SparkCreatorProps) {
  ...
  const handlePost = () => {
    ...
    const textMentions =
      (mode === "text" ? text : caption).match(/@[\w_]+/g) || [];
    const mediaMentions =
      mode !== "text" ? imageTaggedUsers.map((u) => u.username) : [];
    ...
    const postData: any = {
      ...
      image:
        mode !== "text" && selectedMedia?.type === "image"
          ? selectedMedia.url
          : undefined,
      video:
        mode !== "text" && selectedMedia?.type === "video"
          ? selectedMedia.url
          : undefined,
      hasAudio:
        mode !== "text" && selectedMedia?.type === "video"
      ...
      duration:
        mode !== "text" && selectedMedia?.type === "video"
          ...
          : undefined,
      textOverlay:
        mode !== "text" && selectedMedia?.type === "image" && overlayText.trim()
          ? {
              text: overlayText,
              position: overlayPos,
              color: overlayColor,
      ...
      taggedUsers:
        mode !== "text" && selectedMedia?.type === "image"
          ? imageTaggedUsers
          : imageTaggedUsers.map((u) => u.username),
      mood: mood,
  ...
  const filteredMentions =
    suggestionType === "mention"
      ? mockUsers
          .filter(
            (u) =>
              u.username?.toLowerCase().includes(suggestionQuery) ||
              (u.displayName || "").toLowerCase().includes(suggestionQuery),
          )
          .slice(0, 5)
      : [];
  ...
  const filteredHashtags =
    suggestionType === "hashtag"
      ? trendingHashtags
          .filter((h) => h?.toLowerCase().includes(suggestionQuery))
          .slice(0, 5)
      : [];
  ...
  const renderTextWithTags = (t: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("@")) {
        return (
          <span key={i} style={{ color: "#B026FF", fontWeight: "bold" }}>
            {part}
          </span>
        );
      }
      if (part.startsWith("#")) {
        return (
          <span key={i} style={{ color: "#3B82F6", fontWeight: "bold" }}>
            {part}
          </span>
        );
      }
      ...
      return (
        <div className="flex-1 flex flex-col bg-black">
          ...
          >
            ...
            {(suggestionType === "mention" || suggestionType === "hashtag") &&
              (filteredMentions.length > 0 || filteredHashtags.length > 0) && (
                <div
          ...
          <div className="bg-black/40 backdrop-blur-xl border-t border-white/10 p-5 sm:p-6 flex flex-col gap-6 rounded-t-[32px] sm:rounded-none z-20 pb-safe-bottom">
            <div className="flex flex-col gap-3">
              <span className="text-[10px] text-white/70 font-bold uppercase tracking-widest pl-1">
                Background Theme
              </span>
            ...
            <div className="flex flex-col gap-3 pb-2 w-full">
              <div className="flex items-center gap-2 w-full">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2.5 rounded-full text-[11px] font-bold transition-all ${isChallenge ? "bg-[#B026FF] text-white shadow-[0_0_15px_rgba(176,38,255,0.4)]" : "bg-black/50 border border-white/10 text-white/70 hover:bg-white/10 hover:text-white"}`}
                >
                  🎯 Challenge
                </button>
                <button
                  onClick={() => setIsCollab(!isCollab)}
                  className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2.5 rounded-full text-[11px] font-bold transition-all ${isCollab ? "bg-[#3B82F6] text-white shadow-[0_0_15px_rgba(59,130,246,0.4)]" : "bg-black/50 border border-white/10 text-white/70 hover:bg-white/10 hover:text-white"}`}
                >
                  👥 Collab
                </button>
                ...
                <button
                  onClick={handlePost}
                  disabled={!text.trim()}
                  className={`font-bold px-4 py-2.5 rounded-full flex-1 max-w-[80px] h-[40px] transition-all shadow-lg flex items-center justify-center text-sm ${text.trim() ? "bg-gradient-to-r from-[#B026FF] to-[#00F0FF] text-white hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(176,38,255,0.4)]" : "bg-white/10 text-white/40 cursor-not-allowed"}`}
                >
                  Post ⚡
                </button>
              </div>
              ...
              {isCollab && (
                <div className="bg-black/40 rounded-xl border border-white/10 p-3 mt-2">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] text-white/70 font-bold uppercase">
                      👥 Collab with:
                    </span>
                    {collabPartner && (
                      <button
                        onClick={() => setCollabPartner(null)}
                        className="text-[#00F0FF] text-xs font-bold px-2 py-1 bg-[#00F0FF]/10 rounded-full"
                      >
                  ...
                  {!collabPartner && (
                    <>
                      ...
                      <div className="flex flex-col gap-1 max-h-[120px] overflow-y-auto no-scrollbar">
                        <span className="text-[10px] text-white/50 font-bold mb-1">
                          Suggestions
                        </span>
                        {mockUsers
                          .filter((u) =>
      ...
      <div className="flex-1 flex flex-col bg-[#0A0A0A] w-full relative h-[100dvh]">
        ...
        <div className="flex justify-between items-center p-4 absolute top-0 w-full z-50 bg-gradient-to-b from-black/80 to-transparent">
          <button
            ...
            className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center text-white border border-white/10 hover:bg-white/10"
          >
            <X className="w-5 h-5" />
          </button>
          <span className="font-bold text-white tracking-widest text-sm shadow-sm drop-shadow-md">
            POST ⚡
          </span>
          <div className="w-10" />
        </div>
        ...
        {/* Media Preview */}
        <div
          className={`w-full relative bg-black flex items-center justify-center overflow-hidden shrink-0 ${selectedMedia?.type === "video" ? "h-[55vh]" : "h-[60vh]"}`}
        >
            ...
            <>
                ...
                >
                  ...
                  <div className="flex gap-2 justify-center mt-2">
                    ...
                    <button
                      ...
                      className="w-4 h-4 rounded-full bg-black border border-white/50"
                    />
                    <button
                      onClick={() => setOverlayColor("purple")}
                      className="w-4 h-4 rounded-full bg-[#B026FF]"
                    />
                  </div>
                </motion.div>
              )}
              {imageTaggedUsers.map((u, i) => (
                ...
                >
                  ...
                  <span
                    className="ml-1 cursor-pointer opacity-50 hover:opacity-100 text-white/50"
                    onClick={() =>
                      setImageTaggedUsers((prev) =>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar pb-safe">
          ...
          <div className="w-full overflow-x-auto no-scrollbar py-3 px-4 flex gap-4 border-b border-white/5">
            ...
            >
              <div
                className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all ${showTextOverlay ? "bg-[#B026FF]/30 border-2 border-[#B026FF] scale-110" : "bg-white/5 border border-white/10 group-hover:bg-white/10"}`}
              >
                <span className="text-xl">✏️</span>
              </div>
              <span
                className={`text-[10px] font-bold ${showTextOverlay ? "text-white" : "text-white/60"}`}
              >
                Text
              </span>
            </button>
            ...
            >
              <div
                className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all ${showUserSearch ? "bg-[#B026FF]/30 border-2 border-[#B026FF] scale-110" : "bg-white/5 border border-white/10 group-hover:bg-white/10"}`}
              >
                <span className="text-xl">🏷️</span>
              </div>
              <span
                className={`text-[10px] font-bold ${showUserSearch ? "text-white" : "text-white/60"}`}
              >
                Tag
              </span>
            </button>
            <button
              onClick={() => {
                setSuggestionType("hashtag");
                setSuggestionQuery("");
                document.getElementById("spark-caption-input")?.focus();
              }}
              className={`flex flex-col items-center gap-1 shrink-0 group`}
            >
              <div
                className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all bg-white/5 border border-white/10 group-hover:bg-white/10`}
              >
            ...
            >
              <div
                className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all bg-white/5 border border-white/10 group-hover:bg-white/10 text-xl`}
              >
                {mood === "🔥 Trending" ? "😊" : mood}
              </div>
            ...
            {selectedMedia?.type === "video" ? (
              <>
                ...
                >
                  <div
                    className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all ${showTrimSlider ? "bg-[#B026FF]/30 border-2 border-[#B026FF] scale-110" : "bg-white/5 border border-white/10 group-hover:bg-white/10"}`}
                  >
                    <span className="text-xl">✂️</span>
                  </div>
                  <span
                    className={`text-[10px] font-bold ${showTrimSlider ? "text-white" : "text-white/60"}`}
                  >
                    Trim
                  </span>
                </button>
                <button
                  onClick={() => {
                    ...
                    if (mediaVideoRef.current)
                      mediaVideoRef.current.muted = !isVideoMuted;
                  }}
                  className={`flex flex-col items-center gap-1 shrink-0 group`}
                >
                  <div
                    className={`w-[50px] h-[50px] rounded-full flex items-center justify-center transition-all ${isVideoMuted ? "bg-red-500/20 border-2 border-red-500 scale-110" : "bg-white/5 border border-white/10 group-hover:bg-white/10"}`}
                  >
                    <span className="text-xl">🔊</span>
                  </div>
                  <span
                    className={`text-[10px] font-bold ${isVideoMuted ? "text-red-400" : "text-white/60"}`}
                  >
                    Audio
                  </span>
                </button>
              ...
              <div className="flex flex-col gap-2 p-4 bg-[#111] border border-white/10 rounded-2xl">
                ...
                <div className="relative mt-2 pb-6">
                  ...
                  <div
                    ...
                    style={{
                      ...
                      right: `${100 - (trimEnd / Math.max(videoDuration, 1)) * 100}%`,
                    }}
                  />
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (val <= trimEnd - 1) {
                        setTrimStart(val);
                        if (mediaVideoRef.current) {
                          mediaVideoRef.current.currentTime = val;
                          mediaVideoRef.current.play().catch(() => {});
                        }
                      }
                    }}
                  ...
                  />
                  <div
                    className="absolute top-1/2 -translate-y-[calc(50%+12px)] w-5 h-5 rounded-full bg-[#B026FF] border-2 border-white shadow-[0_0_10px_rgba(176,38,255,0.8)] z-30 pointer-events-none"
                    style={{
                      left: `calc(${(trimStart / Math.max(videoDuration, 1)) * 100}% - 10px)`,
                    }}
                  />
                  ...
                  <div
                    className="absolute top-1/2 -translate-y-[calc(50%+12px)] w-5 h-5 rounded-full bg-[#B026FF] border-2 border-white shadow-[0_0_10px_rgba(176,38,255,0.8)] z-30 pointer-events-none"
                    style={{
                      left: `calc(${(trimEnd / Math.max(videoDuration, 1)) * 100}% - 10px)`,
                    }}
                  />
                </div>
                ...
                <div className="flex justify-between items-center text-[10px] font-bold font-mono uppercase">
                  <span className="text-white/60">
                    0:{Math.floor(trimStart).toString().padStart(2, "0")}
                  </span>
                  <span
                    className={`${trimEnd - trimStart > 30 ? "text-red-400" : "text-white/40"}`}
                  >
                    Duration: {Math.floor(trimEnd - trimStart)}s / Max 30s
                  </span>
                  <span className="text-white/60">
                    0:{Math.floor(trimEnd).toString().padStart(2, "0")}
                  </span>
                </div>
              </div>
            )}
                ...
                <div className="absolute bottom-full left-0 right-0 mb-2 bg-[#1A1A1A] border border-[#333] rounded-2xl shadow-2xl overflow-hidden z-50">
                  {suggestionType === "mention" &&
                    filteredMentions.length > 0 && (
                      <div className="flex flex-col">
                        <span className="text-[10px] text-[#B026FF] font-bold px-4 py-2 uppercase bg-black/40 border-b border-[#333] tracking-wider">
                          Mentions
                        </span>
                        {filteredMentions.map((user) => (
                          <button
                          ...
                          >
                            <img
                              src={user.avatar}
                              className="w-8 h-8 rounded-full border border-[#B026FF]/30 object-cover"
                            />
                  ...
                  {suggestionType === "hashtag" &&
                    filteredHashtags.length > 0 && (
                      <div className="flex flex-col">
                        <span className="text-[10px] text-[#3B82F6] font-bold px-4 py-2 uppercase bg-black/40 border-b border-[#333] tracking-wider">
                          Trending
                        </span>
                        {filteredHashtags.map((tag) => (
                          <button
              ...
              <div className="bg-[#111] rounded-2xl border border-[#222] p-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {collabPartner && (
                    <button
                      onClick={() => setCollabPartner(null)}
                      className="text-[#00F0FF] text-xs font-bold px-2 py-1 bg-[#00F0FF]/10 rounded-full flex items-center gap-1"
                    >
                      {collabPartner.username} ✕
                    </button>
                  )}
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <img
                                src={u.avatar}
                                alt="avatar"
                                className="w-10 h-10 rounded-full object-cover shadow-sm bg-black border border-white/10"
                              />
                              <span className="text-sm font-bold text-white">
                                {u.username}
                              </span>
          ...
          <div className="absolute inset-0 bg-[#0A0A0A]/95 backdrop-blur-xl z-[100] flex flex-col p-5 pb-safe-bottom">
            <div className="flex justify-between items-center mb-6 pt-10 sm:pt-4 border-b border-[#333] pb-4">
              ...
              <button
                ...
                className="p-2 bg-[#222] hover:bg-[#333] rounded-full transition-colors border border-[#333]"
              >
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-[#111] border border-[#333] rounded-2xl px-4 py-4 text-white mb-6 focus:outline-none focus:border-[#B026FF] shadow-inner"
            />
              ...
              {mockUsers
                ...
                .map((u) => (
                  <button
                    ...
                    onClick={() => {
                      if (
                        !imageTaggedUsers.find((x) => x.username === u.username)
                      ) {
                        setImageTaggedUsers([
                          ...imageTaggedUsers,
                          {
                  ...
                  >
                    <img
                      src={u.avatar}
                      className="w-12 h-12 rounded-full border border-white/10 shadow-md bg-black object-cover"
                    />
      ...
      <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/95 backdrop-blur-md sm:p-4 perspective-[1000px]">
        ...
        >
          <div className="flex items-center justify-between p-4 bg-black/40 backdrop-blur-xl absolute top-0 left-0 right-0 z-50 border-b border-white/5">
            <button
              onClick={() =>
                mode === "select" ? onClose() : setMode("select")
              }
              className="p-2 hover:bg-white/10 transition-colors rounded-full text-white backdrop-blur-md cursor-pointer"
            >
              <X className="w-6 h-6" />
            </button>
