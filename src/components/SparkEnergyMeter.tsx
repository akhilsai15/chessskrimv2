...
const getEnergyColor = (level: number) => {
  if (level >= 100)
    return "linear-gradient(to top, #FF2D87, #FF6B00, #FFD700, #00FF64, #4488FF)"; // Rainbow
...
export function SparkEnergyMeter({
  ...
  const isOwnSpark =
    currentUser?.id === spark.userId ||
    spark.name === currentUser?.displayName ||
    spark.user === currentUser?.displayName;
  ...
  useEffect(() => {
    try {
      ...
      if (storedMapStr) {
        try {
          storedMap = JSON.parse(storedMapStr);
        } catch (e) {}
      }
      const now = Date.now();
      ...
      if (!currentSparkEnergy) {
        // Initial Calculation
        const reactionsTotal = Object.values(spark.reactions || {}).reduce(
        ...
        if (intervals > 0) {
          currentSparkEnergy.level = Math.max(
            5,
            currentSparkEnergy.level - intervals * 2,
          );
      ...
      const myBoosts = currentSparkEnergy.userBoosts[userId] || {
        todayCount: 0,
  ...
  const saveEnergy = (
    ...
    try {
      ...
      const myBoosts = currentSparkEnergy.userBoosts[uId] || {
        todayCount: 0,
        lastReset: getStartOfDay(),
      };
  ...
  const handlePointerDown = (e: any) => {
    ...
    holdIntervalRef.current = setInterval(() => {
      if (
        sessionBoostRef.current >= 20 ||
        newLevel + sessionBoostRef.current >= 100
      ) {
        endHold(true);
        return;
      }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Meter Bar Container */}
        <div
          className="relative w-[6px] h-[200px] bg-white/10 rounded-[4px] overflow-visible"
        ...
        >
          ...
          <motion.div
            ...
            style={{
              ...
              boxShadow:
                level >= 100 ? `0 0 12px #fff` : `0 0 8px ${currentColor}`,
            }}
            initial={{ height: 0 }}
            animate={{
              height: `${level}%`,
          ...
          <AnimatePresence>
            {particles.map((p) => (
              <motion.div
                ...
                style={{
                  color: level < 100 ? currentColor : "#fff",
        ...
        <div
          ...
          style={{ color: level < 100 ? currentColor : "#fff" }}
        >
          {Math.round(level)}%
        </div>
      </motion.div>
