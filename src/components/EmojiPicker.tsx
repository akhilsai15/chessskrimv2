...
export function EmojiPicker({
  ...
  const saveRecentSticker = (sticker: any) => {
    const recent = JSON.parse(
      localStorage.getItem("skrimchat_recent_stickers") || "[]",
    );
  ...
  const getFilteredGifs = () => {
    if (!gifSearch.trim())
      return (
        MOCK_GIFS[gifCategory as keyof typeof MOCK_GIFS] || MOCK_GIFS.trending
      );
  ...
  return (
    <div className="absolute left-0 right-0 bottom-[85px] z-50 bg-[#1A1A24] h-[360px] rounded-t-3xl flex flex-col border-t border-white/10 shadow-2xl">
      <div className="flex px-4 py-2 border-b border-white/5">
        <button
          className={`flex-1 py-1 text-sm font-semibold text-center ${tab === "emoji" ? "text-white border-b-2 border-white" : "text-white/50"}`}
          onClick={() => setTab("emoji")}
        >
          😊 Emoji
        </button>
        <button
          className={`flex-1 py-1 text-sm font-semibold text-center ${tab === "gif" ? "text-white border-b-2 border-white" : "text-white/50"}`}
          onClick={() => setTab("gif")}
        >
          GIF
        </button>
        <button
          className={`flex-1 py-1 text-sm font-semibold text-center ${tab === "stickers" ? "text-white border-b-2 border-white" : "text-white/50"}`}
      ...
      {tab === "emoji" && (
        <div className="flex-1 bg-[#1A1A24] overflow-y-auto p-4 flex flex-wrap content-start gap-1 justify-between max-w-md mx-auto">
          ...
          {emojisArray.map((e, i) => (
            <button
              key={i}
              className="text-3xl w-[46px] h-[46px] hover:bg-white/10 rounded-xl flex items-center justify-center transition-colors hover:scale-110 active:scale-95"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {currentGifs.map((gif) => (
                  <button
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${gif.color}40, ${gif.color})`,
                    }}
                  >
                    <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors" />
                    <div className="flex flex-col items-center z-10">
                      <span
                        className="text-4xl filter drop-shadow-md transform transition-transform group-hover:scale-125 mb-1"
                        style={{
                          animation: `gif-${gif.animation} 2s infinite`,
                        }}
                      >
                        {gif.emoji}
                      </span>
        ...
        <div className="flex-1 bg-[#1A1A24] flex flex-col pt-2">
          ...
          <div className="flex gap-1 overflow-x-auto no-scrollbar px-3 pb-2 border-b border-white/5">
            ...
            >
              ...
              {stickerPack === "recent" && (
                <div className="absolute -bottom-1 w-4 h-1 bg-neon-purple rounded-full" />
              )}
            </button>
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-4">
            {stickerPack === "recent" && recentStickers.length === 0 ? (
              ...
              <div className="grid grid-cols-4 gap-3 md:gap-4 lg:grid-cols-5">
                {(stickerPack === "recent"
                  ? recentStickers
                  : currentStickerPack.stickers
                ).map((sticker: any, i: number) => (
                  <motion.button
                    ...
                    style={{
                      background: `linear-gradient(to bottom right, ${sticker.bg}80, ${sticker.bg}40)`,
                    }}
                  >
                    <span className="text-4xl filter drop-shadow-md mb-1">
                      {sticker.emoji}
                    </span>
