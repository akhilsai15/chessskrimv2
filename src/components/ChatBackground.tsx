...
export function ChatBackground({
  ...
  // Decide what type of wallpaper it is
  const isSolidOrGradient =
    wallpaper.startsWith("#") ||
    wallpaper.startsWith("linear-gradient") ||
    wallpaper.startsWith("hsl");
  const isImage = wallpaper.startsWith("http") || wallpaper.startsWith("data:");
  const themeObj =
    CHAT_THEMES.find((t) => t.id === wallpaper) || CHAT_THEMES[0];
  const isTheme = !isSolidOrGradient && !isImage;
  const level = getEnergyLevel(energyLevel);

  // Decide number of orbs based on energy level
  const numOrbs = !energyOn
    ? 0
    : level === 0
      ? 2
      : level === 1
  ...
  const [particles, setParticles] = useState<
    { id: number; left: string; delay: number; dur: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (themeObj.particles === "stars") {
      setParticles(
        Array.from({ length: 30 + level * 20 }).map((_, i) => ({
          id: i,
      ...
      themeObj.particles === "diyas" ||
      themeObj.particles === "raindrops"
    ) {
      const count =
        themeObj.particles === "raindrops" ? 20 + level * 30 : 10 + level * 10;
      setParticles(
        Array.from({ length: count }).map((_, i) => ({
          ...
          dur:
            themeObj.particles === "raindrops"
              ? 2 + Math.random() * 2 - level * 0.2
              : 5 + Math.random() * 10 - level,
        })),
      );
    } else {
      setParticles([]);
    }
  }, [themeObj?.particles, level, energyOn, isTheme]);

  return (
    <div
      className="absolute inset-0 overflow-hidden transition-colors duration-1000 z-0 bg-[#0A0A0F]"
      style={
        isSolidOrGradient
          ...
          : isImage
            ...
            : {
                backgroundColor: themeObj.preview,
                color: themeObj.textColor || "white",
              }
      }
    >
        ...
        particles.map((p) => (
          <motion.div
            ...
            style={{
              left: p.left,
              top: (p as any).top,
              width: Math.random() > 0.9 ? 3 : 1.5,
              height: Math.random() > 0.9 ? 3 : 1.5,
            }}
            animate={{ opacity: [0.2, 1, 0.2] }}
            transition={{ duration: p.dur, repeat: Infinity, delay: p.delay }}
          />
        ))}
      {isTheme && themeObj.particles === "stars" && energyOn && level >= 3 && (
        <motion.div
      ...
      {isTheme && themeObj.id === "holi" && energyOn && level >= 3 && (
        <motion.div
          className="absolute rounded-full mix-blend-screen opacity-20 filter blur-3xl pointer-events-none"
          style={{
            background:
              themeObj.orbs[Math.floor(Math.random() * themeObj.orbs.length)],
            left: "50%",
            top: "50%",
            width: 300,
            height: 300,
