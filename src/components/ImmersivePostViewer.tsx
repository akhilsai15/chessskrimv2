...
export function ImmersivePostViewer({
  ...
  const [lastReaction, setLastReaction] = useState<
    (typeof SKRIM_REACTIONS)[0] | null
  >(null);
  const [showShareMenu, setShowShareMenu] = useState(false);
  const [showCommentsSheet, setShowCommentsSheet] = useState(false);
  const [showReactionPicker, setShowReactionPicker] = useState(false);
  const [pulseRipples, setPulseRipples] = useState<
  ...
  const spawnEmojis = (
    reaction: (typeof SKRIM_REACTIONS)[0],
    count: number = 3,
    startX?: number,
    startY?: number,
  ) => {
    ...
    setTimeout(() => {
      setFloatingEmojis((prev) =>
        prev.filter((e) => !newEmojis.find((n) => n.id === e.id)),
      );
    }, 2000);
  };

  const handleTriggerReaction = (
    r: (typeof SKRIM_REACTIONS)[0],
    e?: React.MouseEvent,
  ) => {
    setLastReaction(r);
    if (e) addRipple(e);
  ...
  const handleDragEnd = (_e: any, { offset, velocity }: any) => {
    ...
    if (type === "vibe") {
      if (swipeY < -50 && currentIndex < urls.length - 1) {
        ...
        swipeY > 100 ||
        (swipeY > 50 && velocity.y > 500 && currentIndex === 0)
      ) {
        onClose();
      }
    } else {
      ...
      case "pulse":
        return {
          initial: {
            ...
            x: "-50%",
            y: "-50%",
            filter: "drop-shadow(0px 0px 5px #B026FF)",
          },
          animate: {
            top: [
              `${emoji.y}%`,
      ...
      case "blaze":
        return {
          initial: {
            ...
            x: "-50%",
            y: "-50%",
            filter: "drop-shadow(0px 0px 5px #FF6B00)",
          },
          animate: {
            top: "-10%",
            scale: [0.5, 1.2, 0.8],
      ...
      case "vibe":
        return {
          initial: {
            ...
            x: "-50%",
            y: "-50%",
            filter: "drop-shadow(0px 0px 5px #CC44FF)",
          },
          animate: {
      ...
      case "nova":
        return {
          initial: {
            ...
            x: "-50%",
            y: "-50%",
            filter: "drop-shadow(0px 0px 10px white) brightness(1.5)",
          },
      ...
      case "slay":
        ...
        return {
          initial: {
            ...
            x: "-50%",
            y: "-50%",
            filter: "drop-shadow(0px 0px 5px #FF2D87)",
          },
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-black overflow-hidden flex flex-col justify-center items-center">
      ...
      <div className="absolute top-0 w-full p-4 flex justify-between items-center z-30">
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-2">
              <p className="text-sm font-bold text-white drop-shadow-md group-hover:underline">
                {author?.username || "user"}
              </p>
              <BadgeRow
                stats={generateMockStatsForBadge(author?.username || "")}
                isSmall={true}
              />
            </div>
        ...
        <div
          className="px-3 py-1.5 rounded-full bg-black/40 backdrop-blur-md border flex items-center gap-1.5 shadow-lg"
          style={{
            borderColor:
              type === "post" || type === "image"
                ? "#B026FF50"
                : type === "reel" || type === "video" || type === "vibe"
                  ? "#00F0FF50"
                  : type === "story"
                    ? "#FF2D8750"
                    : type === "saved"
        ...
        >
          {(!type || type === "post" || type === "image") &&
            !currentUrl.includes("400/700") && (
              <>
                <span className="text-sm drop-shadow-md">⚡</span>{" "}
                <span className="text-xs font-bold text-[#B026FF]">
                  Pulse Post
                </span>
              </>
            )}
          {(type === "reel" ||
            ...
            <>
              <span className="text-sm drop-shadow-md">🎬</span>{" "}
              <span className="text-xs font-bold text-[#00F0FF]">Vibe</span>
            </>
          )}
          {type === "story" && (
            <>
              <span className="text-sm drop-shadow-md">✨</span>{" "}
              <span className="text-xs font-bold text-[#FF2D87]">Spark</span>
            </>
          )}
          {type === "saved" && !currentUrl.includes("400/700") && (
            <>
              <span className="text-sm drop-shadow-md grayscale opacity-80">
                🔖
              </span>{" "}
              <span className="text-xs font-bold text-gray-400">Saved</span>
            </>
          )}
          {type === "repost" && (
            <>
              <span className="text-sm drop-shadow-md">🔄</span>{" "}
              <span className="text-xs font-bold text-green-400">Repost</span>
            </>
          )}
          {type === "tagged" && (
            <>
              <span className="text-sm drop-shadow-md">👤</span>{" "}
              <span className="text-xs font-bold text-white">Tagged</span>
            </>
          )}
        </div>
      ...
      <div className="relative z-10 w-full h-[65dvh] max-h-[800px] flex items-center justify-center">
            ...
            <AnimatePresence>
              {activeEffect === "blaze" && (
                <motion.div
                  ...
                  animate={{ opacity: 0.3 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 bg-[#FF6B00] pointer-events-none z-[5]"
                />
              )}
              {activeEffect === "vibe" && (
                <motion.div
                  ...
                  animate={{ opacity: 0.2 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 bg-[#CC44FF] pointer-events-none z-[5]"
                />
              )}
              {activeEffect === "wave" && (
                <motion.div
                  ...
                  animate={{ opacity: 0.2 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 bg-[#00F0FF] pointer-events-none z-[5]"
                />
              )}
              {activeEffect === "pulse" && (
                <motion.div
                  ...
                  className="absolute inset-0 border-[6px] border-[#B026FF] rounded-[24px] pointer-events-none z-[100]"
                />
              )}
            </AnimatePresence>
        ...
        <div className="absolute right-4 top-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-3">
          <div
            className="text-[10px] font-bold text-white/50 tracking-widest uppercase rotate-180"
        ...
        {showReactionPicker && (
          ...
          >
            {SKRIM_REACTIONS.map((r, idx) => (
              ...
              >
                <div
                  className="absolute inset-0 rounded-full blur-xl transition-opacity opacity-0 group-hover:opacity-40"
                  style={{ backgroundColor: r.color }}
                />
                <span className="text-3xl relative z-10 drop-shadow-md">
                  {r.emoji}
                </span>
                <span className="text-[10px] font-bold text-gray-400 transition-colors relative z-10 group-hover:text-white uppercase mt-1">
                  {r.name}
                </span>
              </motion.div>
            ))}
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          <div>
            <p className="text-white text-base mb-1 font-medium leading-tight">
              Lost in the sauce. What a weekend! ✨🚀
            </p>
            <p className="text-[#00F0FF] text-sm font-semibold">
              #weekendvibes #skrimchat #pulse
            </p>
          </div>

          <div className="flex items-center gap-2 mb-4 cursor-pointer">
            <Music className="w-4 h-4 text-white/70" />
            <span className="text-xs text-white/70 font-semibold truncate hover:underline">
              Trending Audio — Mumbai After Hours
            </span>
          </div>

          <div className="mb-6">
            <ReactionRow
              initialReactions={reactions as Record<string, number>}
              onReact={(rId, r) => {
                if (rId && r) {
          ...
          <div className="flex items-center gap-6 mb-6">
            ...
            >
              <Zap
                className={`w-8 h-8 transition-all drop-shadow-md duration-300 lg:group-hover:scale-110 ${pulsed ? "text-[#B026FF] fill-[#B026FF]" : "text-white group-hover:text-[#B026FF]"}`}
                style={
                  pulsed ? { filter: "drop-shadow(0 0 12px #B026FF)" } : {}
                }
              />
              <span className="text-xs font-bold text-white">
                {pulsesCount >= 1000
                  ? parseFloat((pulsesCount / 1000).toFixed(3)) + "K"
                  : pulsesCount}
              </span>
            </button>
            <button
            ...
            >
              <Bookmark
                className={`w-5 h-5 transition-all duration-300 lg:group-hover:scale-110 ${isSaved ? "text-[#B026FF] fill-[#B026FF]" : "text-white group-hover:text-[#B026FF]"}`}
              />
              <span className="text-xs font-bold text-white">
                {isSaved ? "Saved" : "Save"}
              </span>
              ...
              <div className="flex-1 border-t border-white/10 pt-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    ...
                    className="text-xs font-semibold text-[#00F0FF] mt-1 text-left w-full hover:underline"
                  >
                    View all {482 + comments.length - 3} comments →
                  </button>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full flex items-center gap-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl p-4 transition group"
                >
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 flex flex-col items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl p-4 transition"
                  >
                  ...
                  <div key={comment.id} className="flex gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-white/50 mb-1 flex items-center gap-2">
                        ...
                        <BadgeRow
                          stats={generateMockStatsForBadge(comment.username)}
                          isSmall={true}
                        />
                        <span className="font-normal text-white/30 ml-1 flex-1 text-right">
                          2h
                        </span>
                      </p>
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-xs text-white/40 font-medium cursor-pointer hover:text-white transition">
                          Reply
                        </span>
