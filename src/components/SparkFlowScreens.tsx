...
import { getTierInfo } from "../lib/sparkFlowEngine";
...
export function SparkFlowMilestoneModal({
  milestone,
  contactName,
  onDismiss,
}: Props) {
  ...
  if (milestone >= 7) {
    title = `ONE WEEK FLOW! 🎉`;
    subtitle = `You and ${contactName} are on fire! 7 days of daily connection`;
  }
  if (milestone >= 14) {
    title = `TWO WEEKS BLAZING! 🔥`;
    subtitle = `Nothing can stop you now!`;
  }
  if (milestone >= 30) {
    title = `30 DAY INFERNO! 🏆`;
    subtitle = `LEGENDARY BLAZERS! You and ${contactName} are unstoppable! 💎`;
  }
  if (milestone >= 100) {
    title = `100 DAYS! LEGENDARY 💎`;
    subtitle = `Over 3 months of daily connection! History made.`;
  }
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/90 backdrop-blur-lg">
      ...
      >
        ...
        <p className="text-white/90 text-lg mb-8 font-medium">{subtitle}</p>
...
export function BrokenSparkFlowScreen({
  contactName,
  bestScore,
  onDismiss,
}: {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/95">
      ...
      >
        ...
        <button
          onClick={onDismiss}
          className="w-full py-4 text-white font-bold bg-[#1A1A24] border border-white/10 rounded-xl mb-3 hover:bg-white/5 transition-colors"
        >
          Maybe Later
        </button>
        <button
          onClick={onDismiss}
          className="w-full py-4 bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold rounded-xl shadow-[0_0_15px_rgba(239,68,68,0.4)]"
        >
          🔥 Start New Flow
        </button>
      </motion.div>
