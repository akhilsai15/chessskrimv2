...
export function ReactionRow({
  ...
  const handleTap = (rId: string) => {
    ...
    if (activeReactionId === rId) {
      ...
      if (activeReactionId) {
        newCounts[activeReactionId] = Math.max(
          0,
          (newCounts[activeReactionId] || 0) - 1,
        );
      }
      // Add new reaction
      ...
      if (typeof window !== "undefined") {
        ...
        if (r) {
          window.dispatchEvent(
            new CustomEvent("skrim_fly_reaction", { detail: { reaction: r } }),
          );
        }
      }
    }
    ...
    if (onReact) {
      onReact(
        newActive,
        newActive ? SKRIM_REACTIONS.find((x) => x.id === newActive) : undefined,
      );
    }
  };
                ...
                return (
                  <>
                    <div className="p-4 border-b border-white/10 flex items-center justify-between">
                      ...
                      <button
                        onClick={() => setShowWhoReacted(null)}
                        className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/10 text-white/50 hover:text-white transition cursor-pointer"
                      >
                        ✕
                      </button>
                    </div>
