...
export function ChatInput({
  ...
  const [showHoldTooltip, setShowHoldTooltip] = useState(() => {
    return localStorage.getItem("skrim_smart_reply_tooltip_shown") !== "true";
  });
  ...
  const startRecording = () => {
    ...
    recordInterval.current = setInterval(() => {
      setRecordingTime(
        Math.floor((Date.now() - recordStartTime.current) / 1000),
      );
    }, 1000);
  ...
  const inputBorderGlow =
    text.trim() && currentMoodObj
      ? `0 0 10px ${currentMoodObj.glow}, inset 0 0 5px ${currentMoodObj.glow}`
      : "";
  const inputBorderColor =
    text.trim() && currentMoodObj ? currentMoodObj.bubbleGradient[0] : "";

  const sendBtnBg =
    currentMoodObj && text.trim()
      ? `linear-gradient(to bottom right, ${currentMoodObj.bubbleGradient[0]}, ${currentMoodObj.bubbleGradient[1]})`
      : "";
  const sendBtnGlow =
    currentMoodObj && text.trim() ? `0 0 15px ${currentMoodObj.glow}` : "";
      ...
      <AnimatePresence>
        {showAttachMenu && (
          <AttachmentPicker
            ...
            onSendPhoto={(data) => {
              onSendAttachment?.("photo", data);
              setShowAttachMenu(false);
            }}
            onSendVideo={(data) => {
              onSendAttachment?.("video", data);
              setShowAttachMenu(false);
            }}
            onSendFile={(data) => {
              onSendAttachment?.("file", data);
              setShowAttachMenu(false);
            }}
            onSendSong={(data) => {
              onSendAttachment?.("song", data);
              setShowAttachMenu(false);
            }}
            onSendLocation={(data) => {
              onSendAttachment?.("location", data);
              setShowAttachMenu(false);
            }}
            onSendPoll={(data) => {
              onSendPoll?.(data);
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          boxShadow: isActive
                            ? `0 0 15px ${mood.glow}, inset 0 0 0 2px white`
                            : "none",
                          transform: isActive ? "scale(1.05)" : "scale(1)",
                          opacity: isActive ? 1 : 0.8,
                        }}
                      >
                        <span className="text-[32px] mb-2 leading-none drop-shadow-md">
                          {mood.emoji}
                        </span>
                        <span className="text-[11px] font-bold text-white drop-shadow-md uppercase tracking-wider">
                          {mood.label}
                        </span>
                      </button>
                    );
        ...
        <AnimatePresence>
          {smartReplies && smartReplies.length > 0 && (
            <motion.div
              ...
              exit={{ y: 20, opacity: 0 }}
              className="px-2 flex flex-col items-start pointer-events-auto w-full mb-1"
            >
              <div className="flex items-center justify-between w-full px-2 mb-1.5">
                <span className="text-[10px] font-bold text-[#B026FF] flex items-center gap-1 drop-shadow-md">
                  ⚡ SkrimAI
                  {showHoldTooltip && (
                    <span className="text-white/50 ml-1 font-normal animate-pulse">
                      (Hold to edit before sending)
                    </span>
                  )}
                </span>
                <button
                  onClick={onDismissSmartReplies}
                  className="text-white/40 hover:text-white text-[10px] uppercase font-bold px-2 py-0.5 bg-white/5 rounded-full"
                >
                  ✕ Dismiss
                </button>
              </div>
                    ...
                    <motion.button
                        ...
                        holdTimerRef.current = setTimeout(() => {
                          ...
                          if (showHoldTooltip) {
                            ...
                            localStorage.setItem(
                              "skrim_smart_reply_tooltip_shown",
                              "true",
                            );
                          }
                      ...
                      onPointerUp={() => {
                        if (holdTimerRef.current) {
                          // It was a short tap, cancel timer and send
                      ...
                      className="whitespace-nowrap px-4 py-2 rounded-full bg-[rgba(20,20,30,0.85)] backdrop-blur-md border border-[#B026FF]/30 text-white text-[13px] font-medium shadow-[0_4px_12px_rgba(176,38,255,0.15)] hover:bg-[#B026FF]/20 transition-colors flex items-center relative"
                    >
                      {reply}
                    </motion.button>
                  ))}
          ...
          {replyingTo && (
            ...
            >
              ...
              <span className="text-neon-purple font-semibold text-[13px] flex items-center gap-1.5 mb-1">
                <ArrowUp size={12} className="rotate-[-45deg]" /> Replying to{" "}
                {replyingTo.senderName}
              </span>
              <span className="text-white/70 text-[13px] truncate pr-6">
                {replyingTo.text || "Voice/Media message"}
              </span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {!isRecording && (
          <div className="flex">
            <button
              ...
              style={{
                borderLeftColor: currentMoodObj
                  ? currentMoodObj.bubbleGradient[0]
                  : undefined,
            ...
            >
              {currentMoodObj ? (
                ...
                <>
                  <span className="text-[16px] leading-none opacity-50">
                    😶
                  </span>
              ...
              {!isRecordingLocked ? (
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="w-7 h-7 bg-[#B026FF] rounded-full text-white shadow-[0_0_10px_rgba(176,38,255,0.6)] flex items-center justify-center"
                  >
            ...
            <button
              ...
              onPointerMove={(e) => {
                if (isRecording && !isRecordingLocked) {
                  if (
                    ...
                    e.clientX <
                      e.currentTarget.getBoundingClientRect().left - 100
                  ) {
                    stopRecording(true);
                  } else if (
                    e.clientY < window.innerHeight - 150 ||
                    e.clientY < e.currentTarget.getBoundingClientRect().top - 80
                  ) {
                    setIsRecordingLocked(true);
                  }
                }
              ...
              style={{
                background: text.trim()
                  ? sendBtnBg ||
                    "linear-gradient(to bottom right, #7B2FF7, #B026FF)"
                  : "",
                boxShadow: text.trim()
                  ? sendBtnGlow || "0 0 15px rgba(176,38,255,0.4)"
                  : "",
              }}
            >
