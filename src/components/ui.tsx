...
export function NeonButton({
  ...
  const base =
    "w-full py-3 px-6 rounded-full font-semibold transition-all duration-300 flex items-center justify-center gap-2";
  ...
  const variants = {
    ...
    outline:
      "border border-[rgba(255,255,255,0.2)] text-white hover:bg-[rgba(255,255,255,0.05)] active:scale-95",
  };
...
export function AvatarWithRing({
  ...
  let frameClasses = isStory
    ? "p-[2px] bg-gradient-to-tr from-[#B026FF] to-[#00F0FF]"
    : "";
  let frameStyle = {};

  if (ach.avatarFrame === "flame_frame") {
    frameClasses =
      "p-[3px] bg-gradient-to-tr from-orange-500 to-red-500 shadow-[0_0_10px_rgba(249,115,22,0.5)]";
  } else if (ach.avatarFrame === "blaze_frame") {
    frameClasses =
      "p-[3px] bg-gradient-to-tr from-[#B026FF] to-[#00F0FF] shadow-[0_0_12px_#B026FF]";
  } else if (ach.avatarFrame === "nova_frame") {
    frameClasses =
      "p-[3px] bg-gradient-to-tr from-[#FF2D87] to-white shadow-[0_0_15px_#FF2D87]";
  } else if (ach.avatarFrame === "legend_frame") {
    frameClasses =
      "p-[3px] bg-gradient-to-tr from-yellow-300 to-yellow-600 shadow-[0_0_20px_#FFD700] animate-[pulse_2s_infinite]";
  }

  return (
    <div
      className={cn(
        "relative flex items-center justify-center rounded-full pointer-events-none shrink-0",
        sizes[size],
        className,
        frameClasses,
      )}
    ...
    >
      <img
        src={src}
        alt="avatar"
        className="w-full h-full object-cover rounded-full border-2 border-[#0A0A0A] pointer-events-auto"
      />
...
export function FollowButton({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(
        "skrimchat_social_graph_updated",
        handleUpdate,
      );
  }, [username]);
  ...
  return (
    <button
      onClick={handleToggle}
      className={cn(
        `px-3 py-1.5 text-xs font-semibold rounded-full transition-colors active:scale-95 ${
          following
