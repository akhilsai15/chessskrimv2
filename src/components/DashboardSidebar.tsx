...
function SidebarNavItem({
    ...
    return (
      ...
      >
        <Link
          ...
          className={`flex items-center justify-between px-3 h-[44px] rounded-xl transition-all duration-200 cursor-pointer group ${isActive ? "bg-[#7B2FF7]/10 border-l-[3px] border-[#7B2FF7]" : "bg-[rgba(123,47,247,0.03)] hover:bg-[#7B2FF7]/10 hover:border-l-[2px] hover:border-[#7B2FF7]/50 border-l-[0px] border-transparent"}`}
        >
          <div className="flex items-center gap-3">
            <Lock
              className={`w-5 h-5 transition-all duration-500 group-hover:rotate-12 ${isActive ? "text-[#7B2FF7] drop-shadow-[0_0_6px_#7B2FF7]" : "text-gray-400 group-hover:text-[#b382fc]"}`}
            />
            <span
              className={`text-[14px] ${isActive ? "text-[#b382fc] font-bold" : "text-gray-400 group-hover:text-white"}`}
            >
              {label}
            </span>
          </div>
          {chip && (
            <div className="bg-[#7B2FF7] px-1.5 py-0.5 rounded text-[9px] font-black text-white">
              {chip}
            </div>
          )}
        </Link>
  ...
  return (
    ...
    >
      <Link
        to={path}
        className={`flex items-center justify-between px-3 h-[44px] rounded-xl transition-all duration-200 cursor-pointer group ${isActive ? "bg-[#B026FF]/15 border-l-[3px] border-[#B026FF]" : "hover:bg-[#B026FF]/[0.08] hover:border-l-[2px] hover:border-[#B026FF]/50 border-l-[0px] border-transparent"}`}
      >
        <div className="flex items-center gap-3">
          <Icon
            className={`w-5 h-5 transition-colors ${isActive ? "text-[#B026FF] drop-shadow-[0_0_6px_#B026FF]" : "text-[#666] group-hover:text-white"}`}
          />
          <span
            className={`text-[14px] ${isActive ? "text-white font-bold" : "text-[#888] group-hover:text-white"}`}
          >
            {label}
          </span>
        </div>
...
export function DashboardSidebar() {
  ...
  if (completedMissions === totalMissions)
    missionText = "MISSIONS COMPLETE! 🎉 +100⚡";
  else if (completedMissions === totalMissions - 1)
    missionText = "SO CLOSE! 1 mission left! 💪";
  ...
  const activeChallenge =
    achievements.activeChallenges.length > 0
      ? achievements.activeChallenges[0]
      : null;
  let challengeDay = 1;
  let challengeText = "";
  ...
  const badgeDef =
    currentUser && achievements.badges.length > 0
      ? achievements.badges[achievements.badges.length - 1]
      : null;
  const badgeName =
    badgeDef === "flame_creator"
      ? "FLAME CREATOR"
      : badgeDef === "blaze_creator"
        ? "BLAZE CREATOR"
        : badgeDef === "nova_creator"
          ? "NOVA CREATOR"
          : badgeDef === "legend"
            ? "LEGEND"
            : "CREATOR";
  ...
  // Avatar frame
  let frameClasses = "";
  if (achievements.avatarFrame === "flame_frame")
    frameClasses =
      "p-[2px] bg-gradient-to-tr from-orange-500 to-red-500 shadow-[0_0_8px_rgba(249,115,22,0.5)]";
  else if (achievements.avatarFrame === "blaze_frame")
    frameClasses =
      "p-[2px] bg-gradient-to-tr from-[#B026FF] to-[#00F0FF] shadow-[0_0_10px_#B026FF]";
  else if (achievements.avatarFrame === "nova_frame")
    frameClasses =
      "p-[2px] bg-gradient-to-tr from-[#FF2D87] to-white shadow-[0_0_12px_#FF2D87]";
  else if (achievements.avatarFrame === "legend_frame")
    frameClasses =
      "p-[2px] bg-gradient-to-tr from-yellow-300 to-yellow-600 shadow-[0_0_15px_#FFD700] animate-[pulse_2s_infinite]";
    ...
    <div className="flex flex-col flex-1 h-full px-4 py-4 relative z-50 overflow-hidden">
      ...
      <div className="flex flex-col items-start pb-4 hover:bg-white/5 transition rounded-xl p-2 cursor-pointer active:scale-95">
        <div className="flex items-center gap-2">
          <Zap
            className="w-6 h-6 text-[#B026FF] animate-pulse"
            style={{ filter: "drop-shadow(0 0 6px #B026FF)" }}
          />
          <h1 className="text-xl font-black text-white tracking-tight drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">
            SkrimChat
          </h1>
        </div>
      ...
      >
        <div className="relative">
          <div className={`w-9 h-9 rounded-full ${frameClasses}`}>
            <StealthAvatar className="w-full h-full object-cover rounded-full border border-[#141414]" />
          </div>
          {isCurrentUserOnline && (
            <div className="absolute bottom-0 right-0 w-[10px] h-[10px] bg-[#00FF64] rounded-full border-[2px] border-[#0A0A0A] z-10" />
          )}
        </div>
        <div 
        ...
        >
          <p className="text-[13px] font-bold text-white truncate group-hover:text-[#B026FF] transition-colors">
            {currentUser?.fullName ||
              currentUser?.displayName ||
              currentUser?.username?.replace("@", "") ||
              "No Name"}
          </p>
          <p className="text-[9px] font-black tracking-wider text-orange-400 truncate flex items-center gap-1">
            🔥 {badgeName}
          </p>
      ...
      <div className="flex gap-2 mb-1 justify-between">
        {/* Pulse */}
        <div
          className="flex-1 bg-white/5 backdrop-blur-md rounded-xl p-2 flex flex-col items-center justify-center border border-white/5 hover:bg-white/10 cursor-pointer active:scale-95 transition group"
          onClick={() =>
            window.dispatchEvent(
              new CustomEvent("skrimchat_show_stat", { detail: "pulse" }),
            )
          }
        >
        ...
        <div
          ...
          onClick={() =>
            window.dispatchEvent(
              new CustomEvent("skrimchat_show_stat", { detail: "rank" }),
            )
          }
        >
          <div className="flex items-center gap-1 mb-0.5">
            <Crown
              className={`w-3.5 h-3.5 group-hover:scale-110 transition-transform ${parseInt(rank) <= 10 ? "text-[#FFD700]" : parseInt(rank) <= 100 ? "text-[#00F0FF]" : "text-gray-400"}`}
            />
        ...
        {/* Blaze Run */}
        <div
          className={`flex-1 backdrop-blur-md rounded-xl p-2 flex flex-col items-center justify-center border hover:bg-white/10 cursor-pointer active:scale-95 transition group ${blazeWarning ? "bg-red-500/10 border-red-500/30" : "bg-white/5 border-white/5"}`}
          onClick={() =>
            window.dispatchEvent(
              new CustomEvent("skrimchat_show_stat", { detail: "blaze" }),
            )
          }
        >
          <div className="flex items-center gap-1 mb-0.5 relative">
            <Flame
              className={`w-3.5 h-3.5 group-hover:scale-110 transition-transform ${blazeWarning ? "text-red-500 animate-pulse" : "text-[#FF6B00]"}`}
            />
            <span
              className={`text-[13px] font-black text-white ${blazeWarning ? "text-red-400 animate-pulse" : ""}`}
            >
              {blazeRun}d
            </span>
          </div>
      ...
      <div
        className="bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl p-3 cursor-pointer transition active:scale-95 mb-1"
        onClick={(e) => {
      ...
      >
        ...
        <p className="text-[9px] text-gray-400 font-bold mt-1.5 truncate">
          {missionText}
        </p>
      </div>
      ...
      {activeChallenge && (
        <>
          <div className="h-[1px] w-full bg-white/5 mt-2 mb-3" />
          <div
            className="bg-gradient-to-br from-[#1A0B2E] to-[#140024] hover:brightness-110 border border-[#B026FF]/30 rounded-xl p-3 cursor-pointer transition active:scale-95 shadow-[0_0_15px_rgba(176,38,255,0.1)] mb-1"
            onClick={() =>
              window.dispatchEvent(new CustomEvent("skrimchat_show_challenge"))
            }
          >
            <div className="flex justify-between items-end mb-1">
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] font-black uppercase text-white tracking-widest">
                  {activeChallenge.badgeId.split("_")[0]} WEEK
                </span>
              </div>
            ...
            <p className="text-[9px] text-[#00F0FF]/80 font-bold mt-1.5 truncate">
              {challengeText}
            </p>
          </div>
        </>
      ...
      <div className="mt-3 overflow-y-auto no-scrollbar pb-6 flex-1 flex flex-col">
        <div className="flex flex-col gap-[4px]">
          ...
          <SidebarNavItem
            ...
            label="Worlds"
            path="/worlds"
            badge={!hasUnseen || unreadWorldNotifs === 0 ? undefined : unreadWorldNotifs}
            delay={0.3}
          />

          <div className="pt-3 pb-1 px-3">
            <span className="text-[10px] uppercase text-[#444] tracking-[1.5px]">
              PRIVATE
            </span>
          </div>
...
export function MobileStatsDashboard() {
  ...
  return (
    <div className="w-full bg-[#0D0D0D] border-b border-[#B026FF]/20 px-3 py-2 flex items-center gap-2 overflow-x-auto no-scrollbar z-50 shrink-0">
      {/* Pulse */}
      <div
        className="bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 flex items-center gap-1.5 whitespace-nowrap active:scale-95 transition shrink-0"
        onClick={() =>
          window.dispatchEvent(
            new CustomEvent("skrimchat_show_stat", { detail: "pulse" }),
          )
      ...
      >
        <Crown
          className={`w-3 h-3 ${parseInt(rank) <= 10 ? "text-[#FFD700]" : "text-[#00F0FF]"}`}
        />
      ...
      {/* Missions */}
      <div
        className="bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 flex items-center gap-2 whitespace-nowrap active:scale-95 transition shrink-0"
        onClick={() =>
          window.dispatchEvent(new CustomEvent("skrimchat_show_missions"))
        }
      >
      ...
      {activeSheet && (
        <div className="fixed inset-0 z-[9999] flex flex-col justify-end pointer-events-auto">
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-md mx-auto bg-[#141414] border border-white/10 rounded-t-3xl pt-2 pb-8 px-6 max-h-[85vh] overflow-y-auto hidden-scrollbar shadow-[0_-10px_40px_rgba(0,0,0,0.5)]"
          >
            ...
            {activeSheet === "pulse" && (
              <StatDetailSheet
                ...
                color="#B026FF"
                desc="Your overall prestige within the app based on engagement."
              />
            )}
            {activeSheet === "rank" && (
              <StatDetailSheet
                title="Weekly Rank"
                icon={<Crown className="w-6 h-6 text-[#00F0FF]" />}
                color="#00F0FF"
                desc="Your placement among top creators this week."
              />
            )}
            {activeSheet === "blaze" && (
              <StatDetailSheet
                ...
                color="#FF6B00"
                desc="Consecutive days engaging with the community."
              />
            )}
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <h2 className="text-2xl font-black text-white" style={{ color }}>
          {title}
        </h2>
      </div>
      <p className="text-sm text-gray-400 mb-6">{desc}</p>
      <div className="bg-white/5 border border-white/10 rounded-xl p-4">
        <p className="text-xs text-gray-500 font-bold mb-3 uppercase tracking-widest">
          History
        </p>
    ...
    <div>
      ...
      <div className="space-y-3 mb-6">
        {missions.missions.map((m) => (
          <div
            key={m.id}
            className={`flex items-center justify-between p-3 rounded-xl border ${m.done ? "bg-white/5 border-green-500/30" : "bg-black/40 border-white/5"}`}
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-5 h-5 rounded-full flex items-center justify-center border-2 ${m.done ? "border-green-500 bg-green-500/20" : "border-gray-600"}`}
              >
                {m.done && <span className="text-green-500 text-xs">✓</span>}
              </div>
              <span
                className={`text-sm font-semibold ${m.done ? "text-gray-400 line-through" : "text-white"}`}
              >
                {m.desc}
              </span>
            </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-black text-white uppercase">
            {badgeDef?.name || "Challenge"} WEEK
          </h2>
          <p className="text-xs text-[#00F0FF] font-bold tracking-widest">
            Day {challengeDay} of 7 • {remainingDays} days remaining
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3 mb-6 max-h-[40vh] overflow-y-auto no-scrollbar pr-2">
        {ch.tasks.map((t, i) => {
          ...
          return (
            <div
              key={t.id}
              className={`flex items-center justify-between p-3 rounded-xl border ${status === "done" ? "bg-white/5 border-green-500/30" : status === "active" ? "bg-[#00F0FF]/10 border-[#00F0FF]/30" : "bg-black/40 border-white/5 opacity-50"}`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-5 h-5 rounded-full flex items-center justify-center border-2 ${status === "done" ? "border-green-500 bg-green-500/20" : status === "active" ? "border-[#00F0FF]" : "border-gray-600"}`}
                >
                  {status === "done" && (
                ...
                <div>
                  ...
                  <span
                    className={`text-sm font-semibold ${status === "done" ? "text-gray-400 line-through" : "text-white"}`}
                  >
                    {t.desc}
                  </span>
                </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-4">
        ...
        <div className="flex items-center gap-3">
          {badgeDef?.id === "flame_creator" && (
            <div className="w-10 h-10 rounded-full border-2 border-orange-500 p-0.5 bg-gradient-to-tr from-orange-500 to-red-500" />
          )}
          {badgeDef?.id !== "flame_creator" && (
