...
export function SparkViewer({
  ...
  useEffect(() => {
    if (isPaused || showInsights || activeSheet || radialMenuOpen || !spark)
      return;
  ...
  const handleReaction = (emoji: string) => {
    ...
    if (spark.isCollab && spark.status === "accepted") {
      const theirName =
        (spark.creator?.username === currentUser?.username
          ? spark.collabPartner?.username
          : spark.creator?.username
        )?.replace("@", "") || "partner";
  ...
  const handleAddToHighlight = (hlId: string) => {
    ...
    if (hlIndex >= 0) {
      ...
      const alreadyExists = updatedHl.sparks.some(
        (s: any) => s.originalSparkId === spark.id || s === spark.id,
      );
      if (!alreadyExists) {
        const highlightCopy = {
          ...spark,
              ...
              <div className="flex-1 flex flex-col pt-safe-top pb-safe-bottom relative bg-[#121212] items-center justify-center p-8 text-center h-full">
                ...
                <div className="flex flex-col items-center justify-center">
                  <HighlightAvatar
                    emoji={group.emoji || "✨"}
                    theme={
                      group.user?.avatar?.includes("gradient") ||
                      group.user?.avatar?.startsWith("#")
                        ? group.user.avatar
                      ...
                      {spark.type === "text" ? (
                        ...
                        <>
                          <video
                            ...
                            playsInline
                            onError={() =>
                              console.log("Spark video play error")
                            }
                          />
                          <button
                            onClick={(e) => {
                        ...
                        <div className="relative w-full h-full">
                          ...
                          {spark.taggedUsersPositions?.map(
                            (u: any, i: number) => (
                              <div
                                key={i}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {isHighlightMode ? (
                        <HighlightAvatar
                          ...
                          emoji={group.emoji || "✨"}
                          theme={
                            group.sparks?.[0]?.backgroundTheme ||
                            group.sparks?.[0]?.background
                          }
                        />
                      ) : spark.isCollab ? (
                        <div className="relative w-[52px] h-[36px] flex items-center shrink-0">
                          <img
                            src={spark.creator?.avatar || group.user?.avatar}
                            alt="Creator"
                            className="absolute left-0 w-[36px] h-[36px] rounded-full object-cover border-2 border-[#121212] z-10"
                          />
                          <img
                            src={spark.collabPartner?.avatar}
                            alt="Partner"
                            className="absolute left-[16px] w-[36px] h-[36px] rounded-full object-cover border-2 border-[#121212] z-20"
                          />
                        </div>
                      ) : (
                          ...
                          <>
                            <div className="flex items-center gap-1.5 mb-0.5">
                              ...
                              >
                                ...
                                {
                                  (
                                    spark.collabPartner?.displayName ||
                                    spark.collabPartner?.username
                                  )?.split(" ")[0]
                                }
                              </motion.span>
                            ...
                            <span className="text-[12px] font-medium text-gray-300 leading-tight">
                              ...
                              &amp; @
                              {(spark.collabPartner?.username || "").replace(
                                "@",
                                "",
                              )}
                            </span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() =>
                          setActiveSheet(
                            isHighlightMode ? "highlight-options" : "options",
                          )
                        }
                                ...
                                try {
                                  const invites = JSON.parse(
                                    localStorage.getItem(
                                      "skrimchat_collab_invites",
                                    ) || "[]",
                                  );
                                  const inviteIdx = invites.findIndex(
                                    (i: any) => i.spark.id === spark.id,
                                  );
                                  if (inviteIdx >= 0) {
                                    invites[inviteIdx].status = "accepted";
                                    invites[inviteIdx].spark.status =
                                      "accepted";
                                  ...
                                  const sparks = JSON.parse(
                                    localStorage.getItem("skrimchat_sparks") ||
                                      "[]",
                                  );
                                  const sidx = sparks.findIndex(
                                    (s: any) => s.id === spark.id,
                                  );
                                  if (sidx >= 0) {
                                  ...
                                  localStorage.setItem(
                                    "skrimchat_sparks",
                                    JSON.stringify(sparks),
                                  );
                                } catch (e) {}

                                showToast(
                                  "✅ Collab accepted! Now live on both profiles.",
                                );
                              }}
                    ...
                    <div className="flex gap-3 mb-6 p-3 bg-white/5 rounded-xl border border-white/5">
                      <div className="w-12 h-16 rounded bg-black/50 overflow-hidden shrink-0">
                        {spark.type === "video" ? (
                          <video
                            ...
                            title="preview"
                            onError={() =>
                              console.log("Spark video preview error")
                            }
                          />
                        ) : spark.type === "image" ? (
                          <img
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm truncate font-medium">
                          {renderTextWithTags(spark.caption || spark.text) ||
                            "Spark"}
                        </p>
                  ...
                  <div className="px-5 pb-6">
                    ...
                    <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 items-start px-2">
                        ...
                        return (
                          ...
                          >
                            <div className="w-16 h-16 rounded-full p-[2px] bg-gradient-to-tr from-[#B026FF] to-[#00F0FF] shadow-[0_0_8px_rgba(176,38,255,0.3)] transition-transform group-active:scale-95">
                              <div
                                className="w-full h-full rounded-full overflow-hidden border-2 border-[#121212]"
                                style={bgStyle}
                              >
                                {isImage && (
                                  <img
                            ...
                            <span className="text-xs font-semibold text-gray-300 w-16 truncate text-center group-active:text-white transition-colors">
                              {hl.title}
                            </span>
                          </button>
                        );
                      ...
                      >
                        ...
                        <span className="text-xs font-semibold text-gray-300 group-active:text-white transition-colors">
                          New
                        </span>
                      </button>
                    </div>
                        ...
                        {[
                          ...
                          <button
                            ...
                            className={`h-12 w-12 flex items-center justify-center text-2xl rounded-full transition-all ${
                              newHighlightEmoji === emoji
                                ? "border-2 border-[#B026FF] bg-[#B026FF]/20"
                      ...
                      <div className="mt-8 pt-4 border-t border-white/10 flex justify-between items-center px-2">
                        ...
                        {(() => {
                          const hoursLeft = Math.floor(
                            Math.max(0, timeRemaining) / (1000 * 60 * 60),
                          );
                          ...
                          return (
                            <span
                              className={`font-bold text-sm ${colorClass} ${animationClass}`.trim()}
                            >
                ...
                {activeSheet === "highlight-options" && (
                  <div className="px-0 pb-0">
                    <div className="flex flex-col">
                      ...
                      <button
                        onClick={() =>
                          setActiveSheet("remove-highlight-confirm")
                        }
                ...
                {activeSheet === "remove-highlight-confirm" && (
                  <div className="px-5 pb-6 text-center">
                    ...
                    <div className="flex flex-col gap-3">
                      <button
                        onClick={() => {
                          let hlList = JSON.parse(
                            localStorage.getItem("skrimchat_highlights") ||
                              "[]",
                          );
                          ...
                          const finalHighlights = updatedHighlights.filter(
                            (h: any) => h.sparks && h.sparks.length > 0,
                          );
                          localStorage.setItem(
