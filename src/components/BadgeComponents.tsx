  ...
  const handleStart = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("skrimchat_toast", {
        detail: "7-Day Challenge Started! Good luck! 🚀",
      }),
    );
  };
    ...
    <div className="fixed inset-0 z-[250] flex flex-col justify-end p-0 pointer-events-auto">
      ...
      >
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            ...
            <p className="text-sm text-gray-400">
              Unlock exclusive perks for {badgeDef.name}
            </p>
          </div>
        ...
        <div className="flex flex-col gap-3 py-4 overflow-y-auto no-scrollbar flex-1 mb-4">
          <p className="text-xs text-blue-400 font-bold mb-1 uppercase tracking-widest">
            Missions:
          </p>
          {ch.tasks.map((t, i) => (
            <div
              key={i}
              className="flex gap-3 items-center bg-white/5 border border-white/5 p-3 rounded-xl"
            >
          ...
          <p className="text-xs text-[#FFD700] font-bold mt-4 mb-1 uppercase tracking-widest">
            Rewards:
          </p>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white/5 border border-white/5 p-3 rounded-xl flex items-center gap-2">
              <div
                className="w-8 h-8 rounded-full border-2 p-[2px] bg-gradient-to-tr"
            ...
            <div className="bg-white/5 border border-white/5 p-3 rounded-xl flex items-center gap-2 col-span-2">
              ...
              <span className="flex-1 text-sm text-yellow-400 font-bold">
                Bonus Pulse Points
              </span>
            </div>
          </div>
    ...
    <AnimatePresence>
      ...
      {challengeBadge && (
        <div key="challengeSheet" className="contents">
          <ChallengeSheet
            badgeId={challengeBadge}
            onClose={() => setChallengeBadge(null)}
          />
        </div>
      )}
    </AnimatePresence>
