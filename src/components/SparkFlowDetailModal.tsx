...
interface Props {
  ...
  flow: SparkFlowData;
  onClose: () => void;
  onUpdate: (f: SparkFlowData) => void;
}

export function SparkFlowDetailModal({
  chatId,
  contactName,
  flow,
  onClose,
  ...
  const handleFreeze = () => {
    if (flow.freezesLeft > 0 && !flow.isFrozen) {
      const newFlow = {
        ...flow,
        isFrozen: true,
        freezesLeft: flow.freezesLeft - 1,
      };
      saveSparkFlow(chatId, newFlow);
      onUpdate(newFlow);
    }
  };
      ...
      >
        ...
        >
          ...
          <div className="p-8 relative z-10 text-center flex flex-col items-center">
            <div className="text-neon-purple font-bold tracking-widest text-xs mb-6 uppercase flex items-center gap-1">
              SkrimChat <Zap size={12} fill="currentColor" />
            </div>
            <div className="text-5xl mb-4 drop-shadow-[0_0_15px_rgba(255,100,50,0.8)]">
              {tier.emojis}
            </div>
            ...
            <p className="text-white/70 italic text-sm">
              "Blazing together since
              <br />
              {flow.startDate}! 🔥"
            </p>
          </div>
          <div className="bg-white/10 backdrop-blur-md p-4 relative z-10 flex gap-2">
            <button className="flex-1 bg-[#25D366] text-white py-2.5 rounded-xl font-bold flex items-center justify-center gap-2">
              WhatsApp
            </button>
            <button className="flex-1 bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#F56040] text-white py-2.5 rounded-xl font-bold flex items-center justify-center gap-2">
              Insta
            </button>
          </div>
        </motion.div>
    ...
    >
      ...
      >
        <div className="p-6 flex flex-col items-center relative text-center">
          ...
          <div className="text-6xl mb-3 drop-shadow-2xl mt-4">
            {tier.emojis}
          </div>
          ...
          <div className="w-full mb-6">
            ...
            <p className="text-white/60 text-xs">
              Send today to keep the flow alive! 🔥
            </p>
          </div>
          ...
          {flow.freezesLeft > 0 && !flow.isFrozen && (
            <button
              ...
              className="w-full p-3 rounded-xl bg-blue-500/10 border border-blue-500/30 flex items-center gap-3 mb-4 hover:bg-blue-500/20 transition-colors"
            >
              ...
              <div className="flex-1 text-left">
                ...
                <div className="text-blue-300/60 text-xs">
                  Protects your flow for 1 day · {flow.freezesLeft} left
                </div>
              </div>
            </button>
          )}
