  ...
  return (
    <div className="flex gap-4 overflow-x-auto no-scrollbar py-4 px-4 items-start">
      ...
      <motion.div
        ...
        onClick={() =>
          firstOwnSpark ? onSparkClick(firstOwnSpark) : onAddSpark()
        }
      >
        <div className="relative w-[68px] h-[68px]">
          {!firstOwnSpark ? (
            ...
            <div
              className="absolute inset-0 rounded-full p-[3px] bg-gradient-to-tr from-[#B026FF] to-[#00F0FF]"
              style={{ animation: "spin 3s linear infinite" }}
            >
              <div
                className="w-full h-full rounded-full bg-[#121212] overflow-hidden border-2 border-[#121212]"
        ...
        <span className="text-[11px] text-[#888888] font-medium mt-1 truncate w-16 text-center">
          Your Spark
        </span>
      </motion.div>
      ...
      {otherSparks.map((spark, index) => {
        ...
        let ringClass = hasViewed
          ? "bg-white/20"
          : "bg-gradient-to-tr " + getEnergyColor(spark.energy);
        let ringStyle: any = {
          animation: hasViewed
            ? "none"
            : `spin ${getEnergyAnimationDuration(spark.energy)} linear infinite`,
          boxShadow:
            isNova && !hasViewed ? "0 0 15px rgba(255, 107, 0, 0.6)" : "none",
        };
        ...
        return (
          ...
          >
            <div className="relative">
              {spark.isCollab ? (
                <div
                  className={`relative w-[60px] h-[36px] flex items-center justify-start mt-2 mb-2 ${spark.status === "pending" ? "opacity-60" : ""}`}
                >
                  <div
                    className={`w-[36px] h-[36px] rounded-full p-[2px] ${spark.status === "pending" ? "border-2 border-dashed border-white/40 bg-transparent" : ringClass} absolute left-0 z-10`}
                    style={spark.status === "pending" ? {} : ringStyle}
                  >
                    <div
                      className="w-full h-full rounded-full bg-[#121212] overflow-hidden border border-[#121212]"
                      style={{
                        animation: hasViewed
                          ? "none"
                          : `spin ${getEnergyAnimationDuration(spark.energy)} linear infinite reverse`,
                      }}
                    >
                      <img
                        src={spark.creator?.avatar || spark.user?.avatar}
                        alt="Creator"
                        className="w-full h-full object-cover"
                      />
                    </div>
                  ...
                  <div
                    className={`w-[36px] h-[36px] rounded-full p-[2px] ${spark.status === "pending" ? "border-2 border-dashed border-white/40 bg-transparent" : ringClass} absolute left-[24px] z-20`}
                    style={spark.status === "pending" ? {} : ringStyle}
                  >
                    <div
                      className="w-full h-full rounded-full bg-[#121212] overflow-hidden border border-[#121212]"
                  ...
                  <div
                    className={`absolute -bottom-2 -right-1 bg-[#121212] rounded-full px-1.5 py-0.5 border border-white/20 z-30 ${spark.status === "pending" ? "text-[10px] text-white/70" : "text-[10px]"}`}
                  >
                  ...
                  <div
                    ...
                    style={{
                      animation: hasViewed
                        ? "none"
                        : `spin ${getEnergyAnimationDuration(spark.energy)} linear infinite reverse`,
                    }} // Reverse spin to keep image upright
                  >
                    <img
                      src={spark.user?.avatar}
                      alt={spark.user?.displayName}
                      className="w-full h-full object-cover"
                    />
                  </div>
                </div>
                ...
                <div className="absolute -bottom-1 -right-1 bg-white/10 backdrop-blur-md rounded-full px-1.5 py-0.5 border border-white/20 z-10 shadow-sm flex items-center gap-1">
                  ...
                  {spark.duration && (
                    <span className="text-[9px] font-bold text-white">
                      0:{spark.duration.toString().padStart(2, "0")}
                    </span>
                  )}
                </div>
              )}
            ...
            <div className="flex flex-col items-center mt-1">
              <span
                className={`text-[10px] font-medium truncate w-[68px] text-center ${hasViewed ? "text-gray-500" : "text-white"}`}
              >
                {spark.isCollab
                  ? `${(spark.creator?.username || spark.user?.username)?.replace("@", "")} + ${spark.collabPartner?.username?.replace("@", "")}`
                  : spark.user?.username}
              </span>
              {countdownText && !hasViewed && (
                <span
                  className={`text-[10px] font-bold ${isExpiringVerySoon ? (isExpiringImminent ? "text-red-500 " : "text-red-400") : "text-orange-400"}`}
                  style={
                    isExpiringVerySoon
                      ? {
                          animation:
