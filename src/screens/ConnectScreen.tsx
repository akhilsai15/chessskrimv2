...
function SwipeableChatRow({
  ...
  const getReadReceiptIcon = (lastMessage: string, unread: number) => {
    ...
    if (lastMessage.startsWith("You:")) {
      return (
        <span className="text-neon-blue text-[10px] ml-1 opacity-80 shrink-0 select-none tracking-tight">
          ✓✓
        </span>
      );
    }
  ...
  return (
    <div className="group relative overflow-hidden bg-[#0A0A12]">
      ...
      <div className="absolute inset-0 flex justify-between items-center px-4">
        ...
        <div
          className={`flex items-center text-blue-400 transition-opacity ${dragX > 20 ? "opacity-100" : "opacity-0"}`}
        >
        ...
        <div
          className={`flex items-center gap-4 text-gray-300 transition-opacity ${dragX < -20 ? "opacity-100" : "opacity-0"}`}
        >
      ...
      >
        ...
        <div className="relative shrink-0 w-12 h-12">
          {chat.isGroup ? (
            <div className="relative w-full h-full">
              <img
                src={chat.avatar2}
                className="w-8 h-8 rounded-full absolute top-0 right-0 border-2 border-[#0A0A12] bg-zinc-800"
              />
              <img
                src={chat.avatar}
                className="w-8 h-8 rounded-full absolute bottom-0 left-0 border-2 border-[#0A0A12] bg-zinc-700"
              />
            </div>
          ) : (
            <>
            ...
            <div className="ml-2 mt-[1px]">
              {(() => {
                ...
                if (flow && flow.count > 0)
                  return <SparkFlowIcon flow={flow} />;
                return null;
              })()}
            </div>
            {chat.pinned && (
              <Pin className="w-3 h-3 text-[10px] fill-gray-500 text-gray-500 ml-1.5 shrink-0" />
            )}
          </div>
          <div className="flex items-center">
            <p
              className={`text-[13px] truncate leading-tight mt-1 ${chat.unread > 0 ? "text-white font-medium" : "text-gray-400"}`}
            >
              {chat.lastMessage.includes("You:") ? (
                <span className="text-gray-500 mr-0.5">You:</span>
              ) : (
                ""
              )}
              {detectLanguage(chat.lastMessage) !== "English" ? "🌍 " : ""}
        ...
        <div className="flex flex-col items-end justify-between h-[44px] pb-0.5 pt-0.5 pointer-events-none">
          <span
            className={`text-[11px] ${chat.unread > 0 ? "text-[#B026FF] font-bold" : "text-gray-500"} leading-none`}
          >
            {chat.time}
          </span>
...
export default function ConnectScreen() {
  ...
  useEffect(() => {
    const fetchChats = async () => {
      ...
      const finalChats = [
        ...
        ...fetchedChats.filter(
          (fc) =>
            !customChatEntries.find(
              (cc) => cc.name.replace("@", "") === fc.name.replace("@", ""),
            ),
        ),
      ];
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("skrimchat_requests_updated", loadRequests);
  }, []);
  ...
  if (targetUserParam) {
    const targetUserData = mockUsers.find(
      (u) =>
        u.username === `@${targetUserParam}` || u.username === targetUserParam,
    ) || {
      ...
      <div className="w-full h-full flex flex-col bg-[#0A0A12] relative overflow-hidden z-50">
        <header className="flex items-center justify-between px-4 py-4 border-b border-white/5 bg-[#0A0A12]/90 backdrop-blur-md sticky top-0 z-40">
          <div className="flex items-center gap-3">
            <button
              ...
              className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-white/10 transition active:scale-95"
            >
              <ArrowLeft className="w-5 h-5 text-white" />
            </button>
            <div
              className="flex items-center gap-2 cursor-pointer"
              onClick={() => navigate(`/profile/${targetUserParam}`)}
            >
              <AvatarWithRing
        ...
        <div className="flex-1 overflow-y-auto w-full flex flex-col p-4 no-scrollbar gap-4 justify-end">
          <div className="flex flex-col items-center justify-center py-10 opacity-50">
            ...
            <span className="text-xs text-gray-500 mt-4 bg-white/5 px-4 py-1 rounded-full">
              New Chat Created
            </span>
          </div>
          ...
          {messages.length > 0 ? (
            messages.map((msg: any) => {
              const isMine =
                msg.sender !== targetUserData.username &&
                msg.sender !== `@${targetUserParam}`;
              ...
              return (
                <div
                  key={msg.id}
                  className={`self-${isMine ? "end" : "start"} bg-${isMine ? "[#B026FF]/20" : "white/10"} border border-${isMine ? "[#B026FF]/50" : "white/10"} rounded-2xl rounded-t${isMine ? "r" : "l"}-sm px-4 py-2 text-sm text-white max-w-[80%] ${isMine ? "shadow-[0_0_10px_rgba(176,38,255,0.2)]" : ""}`}
                >
                        ...
                        if (part.match(urlRegex)) {
                          return (
                            <span
                              key={i}
                              onClick={() => {
                                if (part.includes("skrim.chat/vibe/"))
                                  return navigate("/vibes");
                                if (part.includes("skrim.chat/spark/"))
                                  return navigate("/");
  ...
  const MOCK_CHATS = [
    {
      ...
      lastMessage: "Did you watch that vibe I sent? 🔥",
      time: "2:34 PM",
      unread: 3,
      online: true,
      blazeStreak: 7,
    ...
    },
    {
      id: "3",
      name: "Telugu Squad 🔥",
      avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=telugu1",
      avatar2: "https://api.dicebear.com/7.x/avataaars/svg?seed=telugu2",
      lastMessage: "Arjun: Oka vibe chudandi guys!",
      time: "12:03 PM",
      unread: 12,
      online: false,
      isGroup: true,
    ...
    },
    {
      id: "7",
      name: "Vibes Gang 🌟",
      avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=vibes1",
      avatar2: "https://api.dicebear.com/7.x/avataaars/svg?seed=vibes2",
      lastMessage: "Priya: sending good vibes only✨",
      time: "Sun",
      unread: 5,
      online: false,
      isGroup: true,
    ...
    <div className="w-full h-full flex flex-col pt-4 pb-24 relative overflow-hidden bg-[#0A0A12] text-white">
      ...
      <header className="flex justify-between items-center px-4 mb-4 z-10 relative mt-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors mr-2">
      ...
      <div className="px-4 mb-5 z-10 relative">
        <div
          className={`flex items-center bg-white/5 rounded-3xl px-4 py-2.5 border transition-all duration-300 ${isSearchFocused ? "border-[#B026FF] shadow-[0_0_15px_rgba(176,38,255,0.2)]" : "border-transparent"}`}
        >
          <Search
            className={`w-4 h-4 ${isSearchFocused ? "text-[#B026FF]" : "text-gray-400"} mr-2`}
          />
          <input
            type="text"
          ...
          <div className="mb-2">
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar px-4 pb-2 pt-1 w-full">
              ...
              <div className="flex flex-col items-center gap-2 shrink-0 cursor-pointer w-14">
                <div className="relative w-[52px] h-[52px]">
                  <img
                    ...
                    className="w-full h-full rounded-full bg-white/10 border-2 border-transparent"
                  />
              ...
              {ACTIVE_MOCKS.map((contact) => (
                <div
                  key={contact.id}
                  className="flex flex-col items-center gap-2 shrink-0 cursor-pointer w-14"
                >
                  <div className="relative w-[52px] h-[52px]">
                    {/* Neon Ring */}
                    <div
                      className="absolute inset-[-2px] rounded-full border-[2px] border-emerald-400/80 shadow-[0_0_8px_rgba(52,211,153,0.5)] animate-pulse"
                      style={{ animationDuration: "3s" }}
                    />
                    <img
                      src={contact.avatar}
                      className="w-full h-full rounded-full bg-white/10 relative z-10"
                    />
                    <div className="absolute right-0 bottom-0 w-3.5 h-3.5 bg-emerald-500 rounded-full border-[2.5px] border-[#0A0A12] z-20" />
                  </div>
                  <span className="text-[10px] text-white font-medium truncate w-14 text-center block">
                    {contact.name}
                  </span>
                </div>
              ))}
        ...
        {filteredChats.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-8 opacity-60">
            ...
            <p className="text-sm text-gray-500">
              Start connecting with people you follow!
            </p>
          </div>
        ) : (
          <div className="flex flex-col">
            {atRiskChat && (
              <div className="px-4 mb-2">
                <div
                  onClick={() => navigate(`/chat/${atRiskChat.id}`)}
                  className="bg-gradient-to-r from-orange-500/20 to-red-500/10 border border-orange-500/30 rounded-2xl p-3 flex items-center justify-between cursor-pointer shadow-[0_0_15px_rgba(255,165,0,0.1)]"
                >
                  <div>
                    ...
                    <p className="text-orange-200/80 text-xs">
                      "Message now or lose your flow! ⏰"
                    </p>
                  </div>
                  <div className="px-3 py-1.5 bg-orange-500 text-white font-bold text-xs rounded-full">
                    Message
                  </div>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-[9999] flex flex-col justify-end">
            ...
            >
              ...
              <div className="space-y-3">
                <button
                  onClick={() => {
                    setShowCreateMenu(false); /* normal new chat logic */
                  }}
