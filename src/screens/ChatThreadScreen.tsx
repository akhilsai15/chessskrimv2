...
} from "../components/SparkFlowScreens";
import {
  getSparkFlow,
  updateSparkFlow,
  SparkFlowData,
} from "../lib/sparkFlowEngine";
...
export default function ChatThreadScreen() {
  ...
  const [cryptKey, setCryptKey] = useState<CryptoKey | null>(null);
  ...
  const [showRawStorageViewer, setShowRawStorageViewer] = useState(false);
  const [rawStorageData, setRawStorageData] = useState<{
    iv: number[];
    data: number[];
  } | null>(null);
  const [bannerTapCount, setBannerTapCount] = useState(0);
  ...
  useEffect(() => {
    let timeout: any;
    if (bannerTapCount > 0) {
      timeout = setTimeout(() => setBannerTapCount(0), 1000);
      if (bannerTapCount >= 3) {
        setShowRawStorageViewer(true);
        setBannerTapCount(0);
      }
    }
  ...
  const [customization, setCustomization] = useState(() => {
    ...
    const storedTheme = localStorage.getItem(
      `chat_theme_${chatId || "default"}`,
    );
    let defaultWallpaper = "dark_space";
    if (storedTheme) {
      try {
        defaultWallpaper = JSON.parse(storedTheme).themeId || "dark_space";
      } catch (e) {}
    }
  ...
  const [energyOn, setEnergyOn] = useState<boolean>(() => {
    ...
    if (stored) {
      try {
        return JSON.parse(stored).energyOn;
      } catch (e) {}
    }
    return true;
  });
    ...
    if (moodStored) {
      try {
        ...
        if (
          ...
          new Date(parsed.timestamp).toDateString()
        ) {
          return parsed.mood;
        }
      } catch (e) {}
  ...
  const [moodNotification, setMoodNotification] = useState<{
    name: string;
    mood: string;
  } | null>(null);
  ...
  useEffect(() => {
    if (mood) {
      localStorage.setItem(
        "chat_mood",
        JSON.stringify({ mood, timestamp: Date.now() }),
      );
    } else {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "hide-translation",
      handleHideTranslation as EventListener,
    );
    return () =>
  ...
  const [searchHistory, setSearchHistory] = useState<string[]>(() => {
    const stored = localStorage.getItem(
      `search_history_${chatId || "default"}`,
    );
    return stored ? JSON.parse(stored) : [];
  });
  const searchInputRef = useRef<HTMLInputElement>(null);
  const [reactionSheetMessage, setReactionSheetMessage] =
  ...
  const [activeChallengeMessage, setActiveChallengeMessage] =
    useState<Message | null>(null);

  const [sendRipple, setSendRipple] = useState<{
    id: string;
    color: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "open-media-viewer",
      handleOpenMediaViewer as EventListener,
    );
    return () =>
      window.removeEventListener(
        "open-media-viewer",
    ...
    if (newMood) {
      setTimeout(() => {
        ...
        if (nextMoodObj) {
          ...
          setMoodNotification({
            name: "Priya",
            mood: `${nextMoodObj.emoji} ${nextMoodObj.label}`,
          });
    ...
    const results = messages
      .filter((m) => {
        ...
        if (
          ...
          !(
            m.type === "text" &&
            (m.text?.includes("http") || m.text?.includes("link"))
          )
        )
          return false;
        ...
        const translationMatch =
          (m.showTranslation && m.translation?.toLowerCase().includes(query)) ||
          false;
        const captionMatch =
          (m.type === "image" || m.type === "video") &&
          (m as any).caption?.toLowerCase().includes(query);
        const fileMatch =
          m.type === "file" &&
          (m as any).file?.name?.toLowerCase().includes(query);
        const songMatch =
          m.type === "song" &&
          ((m as any).song?.title?.toLowerCase().includes(query) ||
            (m as any).song?.artist?.toLowerCase().includes(query));
  ...
  useEffect(() => {
    const initEncryption = async () => {
      ...
      if (!key) {
        ...
        setTimeout(() => setIsGeneratingKey(false), 500);
      }
      setCryptKey(key);
      const code = await generateSecurityCode(key);
      setSecurityCode(code);
    };
    initEncryption();
  }, []);

  const handleSendMessage = async (text: string, isPulsed: boolean = false) => {
    ...
    if (cryptKey) {
      encryptMessage(text, cryptKey)
        .then((enc) => {
          setMessages((prev) =>
            prev.map((m) =>
              m.id === newMessage.id ? { ...m, encrypted: enc } : m,
            ),
          );
        })
        .catch(console.error);
    ...
    if (messages.length % 2 === 0) {
      setTimeout(() => {
        ...
        setTimeout(async () => {
          ...
          const replyText = "I know right?! 🤯 So crazy";
          const reply: Message = {
            id: Date.now().toString(),
            sender: "them",
            type: "text",
          ...
          if (cryptKey) {
            encryptMessage(replyText, cryptKey)
              .then((enc) => {
                setMessages((prev) =>
                  prev.map((m) =>
                    m.id === reply.id ? { ...m, encrypted: enc } : m,
                  ),
                );
              })
              .catch(console.error);
  ...
  const handleAcceptChallenge = (message: Message) => {
    ...
    setMessages((prev) =>
      prev.map((m) =>
        m.id === message.id ? { ...m, challengeStatus: "accepted" } : m,
      ),
    );
  };
  ...
  const handleDeclineChallenge = (message: Message) => {
    setMessages((prev) =>
      prev.map((m) =>
        m.id === message.id ? { ...m, challengeStatus: "declined" } : m,
      ),
    );
  };
  ...
  const handleFinishGame = (myScore: number, opponentScore: number) => {
    ...
    setMessages((prev) =>
      prev.map((m) =>
        m.id === activeChallengeMessage.id
          ? { ...m, challengeStatus: "completed" }
          : m,
      ),
    );
    ...
    setTimeout(() => {
      setMessages((prev) =>
        prev.map((m) =>
          m.id === resultMessageId ? { ...m, status: "sent" } : m,
        ),
      );
    }, 500);
  };
  ...
  const handleSendAttachment = (type: string, data: any) => {
    ...
    if (type === "photo")
      appendedMessage = { ...baseMessage, type: "photo", photo: data } as any;
    else if (type === "video")
      appendedMessage = { ...baseMessage, type: "video", video: data } as any;
    else if (type === "file")
      appendedMessage = { ...baseMessage, type: "file", file: data } as any;
    else if (type === "song") {
      appendedMessage = { ...baseMessage, type: "song", song: data } as any;
      delete appendedMessage?.uploadProgress;
        ...
        const interval = setInterval(() => {
          ...
          if (progress >= 100) {
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === newMessageId
                  ? { ...m, uploadProgress: undefined, status: "sent" }
                  : m,
              ),
            );
          } else {
            setMessages((prev) =>
              prev.map((m) =>
                m.id === newMessageId
                  ? { ...m, uploadProgress: Math.min(progress, 99) }
                  : m,
              ),
            );
          }
  ...
  const handleForwardMessage = (msg: Message) => {
    ...
    setTimeout(
      () =>
        setMessages((prev) =>
          prev.map((m) =>
            m.id === forwardedMsg.id ? { ...m, status: "sent" } : m,
          ),
        ),
      500,
    );
  ...
  return (
    <div className="w-full h-full flex flex-col relative z-[999] bg-black overflow-hidden">
                  ...
                  ? {
                      backgroundColor:
                        energyLevel >= 80
                          ...
                          : "#FFF",
                      scale: energyLevel >= 80 ? [1, 1.2, 1] : 1,
                    }
      ...
      {searchMode ? (
        <div className="flex-none p-4 pb-2 z-50 flex flex-col pt-safe bg-[#0A0A0F]/95 backdrop-blur-xl border-b border-white/5">
          ...
          <div className="flex items-center gap-2 mt-3 overflow-x-auto hide-scrollbar">
            {["All", "📷 Media", "📁 Files", "🔗 Links", "🎵 Songs"].map(
              (filter) => {
                const isSelected =
                  searchFilter === filter ||
                  (filter !== "All" && searchFilter === filter.split(" ")[1]);
                const filterName = filter.includes(" ")
                  ? filter.split(" ")[1]
                  : filter;
                return (
                  <button
          ...
          {!searchQuery && searchMode && searchHistory.length > 0 && (
            <div className="mt-4 px-2">
              <div className="text-[10px] text-white/40 font-bold mb-2">
                RECENT SEARCHES
              </div>
        ...
        <ChatHeader
          name={
            customization.nickname
              ? `${customization.nickname} 🎨`
              : recipientUser?.displayName || "Priya Sharma"
          }
          avatar={
          ...
          onSecurityCodeClick={() => setShowSecurityCodeModal(true)}
        />
      )}

      {showEncryptionBanner && (
        <div
          className="mx-auto mt-4 px-4 relative z-[45] mb-2"
          onClick={() => setBannerTapCount((c) => c + 1)}
        >
          <div
            className="bg-[#1A1A24]/80 backdrop-blur-md border border-white/10 rounded-full px-4 py-2 flex items-center justify-between shadow-lg cursor-pointer hover:bg-white/10 transition-colors"
            onClick={() => setShowEncryptionDetails(true)}
          >
            <div className="flex items-center gap-2">
              <span className="text-white/80">🔒</span>
              <p className="text-white/80 text-xs font-medium">
                Messages are end-to-end encrypted · Tap to learn
              </p>
            </div>
            <button
              onClick={(e) => {
        ...
        <AnimatePresence>
          ...
          >
            <span className="text-white/90 text-sm font-medium">
              {moodNotification.name} is feeling {moodNotification.mood}!
            </span>
          </motion.div>
        </AnimatePresence>
      )}
          ...
          >
            ...
            <div>
              ...
              <div className="text-white/50 text-xs">
                to your {reactionToast.moodEmoji} {reactionToast.moodLabel}{" "}
                mood!
              </div>
            </div>
          </motion.div>
        ...
        <div className="absolute top-[60px] inset-x-0 z-40 px-4 pt-2">
          <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-xl p-3 flex justify-between items-center shadow-lg">
            <span className="text-white font-medium text-sm">
              ⚠️ Your {sparkFlow.count}-day flow ends at midnight! Message now
              🔥
            </span>
      ...
      {pinnedMessages.length > 0 && (
        <div
          className="bg-[#1A1A2A] border-l-4 border-[#B026FF] px-4 py-2.5 flex items-center gap-3 relative z-[45] shadow-md mx-2 mt-2 rounded-r-xl"
          onClick={() => setShowPinnedMessagesPage(true)}
        >
          <div className="flex-1 overflow-hidden">
            <p className="text-white text-sm font-medium truncate flex gap-2">
              <span>📌</span>{" "}
              {pinnedMessages[pinnedMessages.length - 1].type === "announcement"
                ? (pinnedMessages[pinnedMessages.length - 1] as any).text
        ...
        {sendRipple && (
          <motion.div
            ...
            initial={{
              ...
              y: "50%",
              opacity: 0.5,
              border: `2px solid ${sendRipple.color}`,
            }}
            animate={{
              width: 600,
              height: 600,
              opacity: 0,
              border: `10px solid ${sendRipple.color}`,
            }}
        ...
        <ChatInput
          ...
          replyingTo={
            replyingTo
              ? {
                  senderName:
                    replyingTo.sender === "me" ? "You" : "Priya Sharma",
                  text:
                    replyingTo.type === "text"
                      ? replyingTo.text || ""
                      : "Attachment",
          ...
          <GameModal
            game={
              AVAILABLE_GAMES.find(
                (g) => g.id === activeChallengeMessage.game,
              ) || AVAILABLE_GAMES[0]
            }
            ...
            <div className="flex justify-between items-center p-4 bg-gradient-to-b from-black/50 to-transparent z-10 absolute top-0 inset-x-0">
              <div className="flex items-center gap-4">
                ...
                <span className="text-white font-medium">
                  {mediaViewer.sender === "me" ? "You" : "Priya Sharma"}
                </span>
              </div>
            ...
            <div className="flex-1 overflow-hidden relative flex items-center justify-center">
              {mediaViewer.type === "photo" && (
                <div
                  ...
                  style={{
                    backgroundColor: mediaViewer.photo.color,
                    filter:
                      mediaViewer.photo.filter === "Vivid"
                        ? "saturate(200%)"
                        : mediaViewer.photo.filter === "Cool"
                          ? "hue-rotate(90deg)"
                          : mediaViewer.photo.filter === "Warm"
                            ? "sepia(50%)"
                            : "none",
                  }}
                >
      ...
      {actionMessage && (
        <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center p-4">
          ...
          <div className="relative top-4 w-64 bg-[#1A1A24]/90 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            <button
              className="w-full flex items-center gap-3 px-4 py-3 text-white/90 hover:bg-white/10 transition-colors border-b border-white/5"
              onClick={() => {
            ...
            >
              <span>📋</span>{" "}
              {actionMessage.type === "voice" ? "Copy transcript" : "Copy"}
            </button>
            <button
              className="w-full flex items-center gap-3 px-4 py-3 text-white/90 hover:bg-white/10 transition-colors border-b border-white/5"
              onClick={() => {
                handleForwardMessage(actionMessage);
                setActionMessage(null);
              }}
            >
              <span>↪️</span> Forward
            ...
            {actionMessage.sender === "me" ? (
              <button
                className="w-full flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-white/10 transition-colors"
                onClick={() => {
                  setMessages((prev) =>
                    prev.filter((m) => m.id !== actionMessage.id),
                  );
                  setActionMessage(null);
                }}
              >
              ...
              <button
                className="w-full flex items-center gap-3 px-4 py-3 text-orange-500 hover:bg-white/10 transition-colors"
                onClick={() => setActionMessage(null)}
              >
                <span>⚠️</span> Report
              </button>
      ...
      {reactionSheetMessage && (
        <div className="fixed inset-0 z-[9999] flex flex-col justify-end">
          <div
            ...
            onClick={() => setReactionSheetMessage(null)}
          />
          <div className="relative bg-[#1A1A24] border-t border-white/10 rounded-t-3xl p-4 min-h-[50vh] max-h-[80vh] flex flex-col">
            ...
            <div className="flex gap-2 mb-6 overflow-x-auto no-scrollbar">
              <button className="px-4 py-1.5 rounded-full bg-white/10 text-white text-sm font-medium whitespace-nowrap border border-white/5">
                All{" "}
                {Object.values(reactionSheetMessage.reactions || {}).reduce(
                  (acc: number, arr) => acc + (arr as string[]).length,
                  0,
                )}
              </button>
              {Object.entries(reactionSheetMessage.reactions || {}).map(
                ([emoji, users]) => (
                  <button
                    key={emoji}
                    className={`px-4 py-1.5 rounded-full bg-white/5 hover:bg-white/10 text-white text-sm font-medium whitespace-nowrap border border-white/5 flex items-center gap-1.5 ${emoji === "⚡" ? "text-yellow-400" : ""}`}
                  >
            ...
            <div className="flex flex-col gap-4 overflow-y-auto">
              {Object.entries(reactionSheetMessage.reactions || {}).flatMap(
                ([emoji, users]) =>
          ...
          >
            ...
            <div className="px-6 pb-4">
              ...
              <p className="text-white/50 text-sm mb-4">
                {(seenByMessage as any).seenBy || 4} members have seen your
                announcement
              </p>
        ...
        <AnimatePresence>
          ...
          >
            <div className="flex items-center px-4 py-4 border-b border-white/5 bg-[#0A0A0A]">
              <button
                onClick={() => setShowPinnedMessagesPage(false)}
              ...
              {pinnedMessages.map((pm, i) => (
                <div key={i} className="flex flex-col gap-2">
                  <div className="text-xs text-white/50 font-medium">
                    📌 Pinned by {pm.sender === "me" ? "rajani" : "Admin"} ·{" "}
                    {pm.time}
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                    ...
                    <div className="text-white text-sm line-clamp-3 mb-3">
                      {pm.type === "text" || pm.type === "announcement"
                        ? (pm as any).text
                        : "Media Message"}
                    </div>
                    <div className="flex justify-end">
                      <button
                        onClick={() => setShowPinnedMessagesPage(false)}
                        className="text-[#B026FF] text-xs font-bold hover:underline"
                      >
                        Go to Message →
                      </button>
                    </div>
        ...
        <AnimatePresence>
          ...
          >
            <div className="flex justify-between items-center px-4 py-4 border-b border-white/5 bg-[#0A0A0A]">
              <button
                onClick={() => setShowAnnounceComposer(false)}
                className="text-white flex items-center gap-2"
              >
                <ArrowLeft size={20} /> New Announcement
              </button>
              <button
                onClick={() => {
            ...
            <div className="p-4 bg-gradient-to-r from-[#B026FF]/20 to-transparent border-b border-[#B026FF]/30">
              ...
              <p className="text-white/80 text-sm">
                This will be sent to all members and pinned.
              </p>
            </div>
            <div className="p-4 flex-1 flex flex-col">
              ...
              <div className="text-right text-white/40 text-xs mt-2">
                {announceText.length}/500 characters
              </div>
            </div>
            ...
            <div className="p-4 bg-black/40 border-t border-white/5 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2 items-center text-white">
                  <span className="text-xl leading-none mr-1">📌</span> Pin this
                  announcement
                </div>
                <button
                  onClick={() => setAnnouncePin(!announcePin)}
                  className={`w-12 h-6 rounded-full relative transition-colors ${announcePin ? "bg-neon-purple" : "bg-white/20"}`}
                >
                  <motion.div
                    className="w-5 h-5 bg-white rounded-full absolute top-0.5 shadow-md"
                    animate={{ left: announcePin ? "26px" : "2px" }}
                  />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2 items-center text-white">
                  <span className="text-xl leading-none mr-1">🔔</span> Notify
                  all members
                </div>
                <button
                  onClick={() => setAnnounceNotify(!announceNotify)}
                  className={`w-12 h-6 rounded-full relative transition-colors ${announceNotify ? "bg-neon-purple" : "bg-white/20"}`}
                >
                  <motion.div
                    className="w-5 h-5 bg-white rounded-full absolute top-0.5 shadow-md"
                    animate={{ left: announceNotify ? "26px" : "2px" }}
                  />
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2 items-center text-white/60">
                  <span className="text-xl leading-none mr-1">🗓️</span> Schedule
                  for later
                </div>
                <button
                  onClick={() => setAnnounceSchedule(!announceSchedule)}
                  className={`w-12 h-6 rounded-full relative transition-colors ${announceSchedule ? "bg-neon-purple" : "bg-white/20"}`}
                >
                  <motion.div
                    className="w-5 h-5 bg-white rounded-full absolute top-0.5 shadow-md"
                    animate={{ left: announceSchedule ? "26px" : "2px" }}
                  />
                </button>
              </div>
            </div>
      ...
      <AnimatePresence>
        {isGeneratingKey && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-gradient-to-br from-[#0A0A12] to-[#1A1A24]">
            <motion.div
            ...
            >
              <div className="text-6xl mb-6 shadow-[#B026FF]/50 drop-shadow-2xl">
                🔐
              </div>
              <h2 className="text-white text-xl font-bold mb-8">
                Securing your chats...
              </h2>
              <div className="w-64 h-2 bg-white/10 rounded-full mb-8 overflow-hidden relative">
                <motion.div
                  className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-[#B026FF] to-[#00F0FF] rounded-full"
                  style={{ width: `${keyProgress}%` }}
                  animate={{ width: `${keyProgress}%` }}
                  transition={{ ease: "linear" }}
                />
              </div>
              <div className="space-y-2 text-sm text-white/60 font-mono text-left w-64">
                <p>
                  {keyProgress >= 30
                    ? "AES-256 keys generated ✓"
                    : "Generating key..."}
                </p>
                <p>{keyProgress >= 60 ? "Zero-knowledge session ✓" : "..."}</p>
                <p>{keyProgress >= 90 ? "No server storage ✓" : "..."}</p>
              </div>
            </motion.div>
          </div>
        )}
        ...
        {showEncryptionDetails && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-[#1A1A24] border border-[#B026FF]/30 p-6 rounded-2xl w-full max-w-sm relative z-10 shadow-2xl"
            >
              <button
                onClick={() => setShowEncryptionDetails(false)}
              ...
              <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                🔐 SkrimChat Encryption
              </h2>
              <div className="space-y-3 text-sm text-white/80">
                <p>
                  Your messages are secured with{" "}
                  <strong>AES-256-GCM encryption</strong> — the same standard
                ...
                <ul className="space-y-2 mt-4 ml-2">
                  ...
                  <li className="flex items-center gap-2">
                    ✅ <span>Never stored as plain text</span>
                  </li>
                  <li className="flex items-center gap-2">
                    ✅ <span>Zero-knowledge session</span>
                  </li>
                  <li className="flex items-center gap-2">
                    ✅ <span>Keys never leave device</span>
                  </li>
                </ul>
                <div className="mt-6 text-center text-xs text-[#B026FF] font-bold">
        ...
        {showRawStorageViewer && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-black/60 backdrop-blur-sm"
              onClick={() => setShowRawStorageViewer(false)}
            />
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="bg-[#1A1A24] border border-[#B026FF]/50 p-6 rounded-2xl w-full max-w-sm relative z-10 shadow-2xl"
            >
              <button
                onClick={() => setShowRawStorageViewer(false)}
                className="absolute top-4 right-4 text-white/50 hover:text-white"
              ...
              <div className="space-y-4 text-sm">
                <div>
                  <p className="text-white/60 mb-1">What Skrim stores:</p>
                  <div className="bg-[#0A0A12] p-3 rounded-lg border border-white/5 font-mono text-xs text-green-400 break-all max-h-32 overflow-y-auto w-full">
                    {messages[messages.length - 1]?.encrypted
                      ? JSON.stringify(messages[messages.length - 1]?.encrypted)
                      : '{"iv":[23,187,44,11],"data":[198,44,201,33,18]}'}
                  </div>
                </div>
                <div>
                  <p className="text-white/60 mb-1">What you see:</p>
                  <div className="bg-[#0A0A12] p-3 rounded-lg border border-white/5 text-white">
                    {messages[messages.length - 1]?.type === "text"
                      ? (messages[messages.length - 1] as any).text ||
                        "Encrypted Message"
                      : "Media/Other Message"}
                  </div>
                </div>
                <div className="mt-4 text-center text-[#B026FF] font-bold">
                  🔐 That's real encryption.
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-4 text-center">
                <div className="grid grid-cols-6 gap-2 text-2xl mb-6">
                  {securityCode.map((emoji, i) => (
                    <div
                      key={i}
                      className="bg-white/5 rounded-lg p-2 flex items-center justify-center"
                    >
                      {emoji}
                    </div>
                  ))}
                ...
                <p className="text-white/80 text-sm">
                  "Compare this code with Priya to verify your chat is secure."
                </p>
                <p className="text-white/40 text-xs">Changes every session.</p>
              </div>
            </motion.div>
          </div>
        )}
