...
export default function GamesLeaderboardScreen() {
  ...
  const filteredScores = useMemo(() => {
    ...
    for (const [gameId, gameScores] of Object.entries(scores) as [
      string,
      GameScore[],
    ][]) {
    ...
    if (activeTab !== "overall") {
      return (filteredByTime[activeTab] || []).sort(
        (a, b) => b.score - a.score,
      );
    } else {
  ...
  const currentUserRank = useMemo(() => {
    ...
    const index = filteredScores.findIndex(
      (s) => s.playerName === name || s.playerName === "You",
    );
    if (index === -1) {
    ...
    const ptsAway =
      index > 0
        ? filteredScores[index - 1].score - filteredScores[index].score
        : 0;
    return {
      rank: index + 1,
      score: filteredScores[index].score,
      ptsAway,
      avatar: filteredScores[index].avatar,
    };
      ...
      <div className="flex-1 overflow-y-auto pb-24 p-4 no-scrollbar">
          ...
          <div className="flex items-end justify-center gap-3 md:gap-6 mb-12 mt-8">
            ...
            <div className="flex flex-col items-center">
              <div className="relative mb-2">
                <span className="absolute -top-3 -left-3 text-2xl z-10">
                  🥈
                </span>
                <img
                  src={topThree[1].avatar}
                  alt=""
                  className="w-16 h-16 rounded-full border-4 border-slate-300 shadow-[0_0_20px_rgba(203,213,225,0.4)]"
                />
              </div>
              <span className="font-bold text-sm text-white/90 truncate w-20 text-center">
            ...
            <div className="flex flex-col items-center -mt-8">
              <div className="relative mb-2">
                <span className="absolute -top-6 -left-4 text-4xl z-10 drop-shadow-[0_0_15px_rgba(251,191,36,0.8)] filter">
                  👑
                </span>
                <img
                  src={topThree[0].avatar}
                  alt=""
                  className="w-20 h-20 rounded-full border-4 border-amber-400 shadow-[0_0_30px_rgba(251,191,36,0.6)]"
                />
              </div>
              <span className="font-black text-amber-400 text-lg truncate w-24 text-center">
            ...
            <div className="flex flex-col items-center">
              <div className="relative mb-2">
                <span className="absolute -top-3 -left-1 text-2xl z-10">
                  🥉
                </span>
                <img
                  src={topThree[2].avatar}
                  alt=""
                  className="w-14 h-14 rounded-full border-4 border-orange-700 shadow-[0_0_20px_rgba(194,65,12,0.4)]"
                />
              </div>
              <span className="font-bold text-xs text-white/80 truncate w-16 text-center">
        ...
        <div className="space-y-2">
          {restOfList.map((score, idx) => (
            <div
              key={idx}
              className="flex items-center gap-4 bg-white/5 hover:bg-white/10 p-3 rounded-2xl border border-white/5 transition-colors"
            >
              <div className="w-8 text-center text-white/40 font-black text-lg">
                #{idx + 4}
              </div>
              <img
                src={score.avatar}
              ...
              <div className="text-right">
                <span className="block font-mono font-black text-[#00F0FF]">
          ...
          <div className="flex items-start justify-between">
            ...
            {currentUserRank.rank > 1 && (
              <div className="bg-emerald-500/20 text-emerald-400 px-3 py-1.5 rounded-lg text-xs font-bold border border-emerald-500/30 flex items-center gap-1">
                {currentUserRank.ptsAway.toLocaleString()} pts to #
                {currentUserRank.rank - 1} <ArrowUp className="w-3 h-3" />
              </div>
            )}
            {currentUserRank.rank === 1 && (
