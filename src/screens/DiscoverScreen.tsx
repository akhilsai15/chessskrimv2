        ...
        {loading ? (
          ...
          <>
            {[1, 2, 3, 4].map((i) => (
              ...
              >
                <img
                  src={`https://picsum.photos/seed/${title.replace(/\s+/g, "")}${i}/140/200`}
                  alt="Discover"
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <p className="text-white text-xs font-bold truncate leading-tight drop-shadow-md">
                    #{title.split(" ")[0]}_{i}
                  </p>
                  <p className="text-white/70 text-[10px] mt-0.5 drop-shadow-md">
                    {(10 * i + Math.random() * 10).toFixed(1)}k views
                  </p>
                </div>
              </div>
            ))}
...
const getTrendingScore = (vibe: any) => {
  const ageHours = Math.max(
    0,
    (Date.now() - (vibe.createdAt || Date.now() - 3600000)) / (1000 * 60 * 60),
  );
    ...
    <>
      {parts.map((p, i) =>
        p.toLowerCase() === highlight.toLowerCase() ? (
          <strong key={i} className="font-extrabold text-white text-[105%]">
            {p}
          </strong>
        ) : (
          <span key={i}>{p}</span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <img
          src={creator.avatar}
          className="w-10 h-10 rounded-full border border-white/10 bg-[#151520] object-cover"
        />
        <div className="flex flex-col">
          <div className="flex items-center gap-1">
            <span className="text-white font-bold text-sm tracking-tight leading-none group-hover:text-[#00F0FF] transition-colors">
              {creator.name}
            </span>
            {creator.isVerified && (
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-white/40 text-[10px] font-medium font-mono">
              {(creator.followers / 1000).toFixed(1)}K
            </span>
            <span className="text-white/40 text-[10px] font-medium text-ellipsis overflow-hidden whitespace-nowrap max-w-[120px]">
              🌍 {creator.creatorLanguage} · {creator.creatorRegion}
            </span>
          </div>
        </div>
      </div>
...
function SearchResultsSection({
  ...
  const [activeTab, setActiveTab] = useState<"all" | "creators" | "vibes">(
    "all",
  );
  ...
  const autocompleteSuggestions = React.useMemo(() => {
    ...
    // Add words
    const matchedWords = Array.from(allWords)
      .filter((w) => w.startsWith(q) && w !== q)
      .slice(0, 3);
    matchedWords.forEach((w) => suggestions.push({ type: "text", label: w }));

    // Add hashtags
    const matchedHashtags = Array.from(allHashtags)
      .filter((h) => h.toLowerCase().includes(q))
      .slice(0, 2);
    matchedHashtags.forEach((h) =>
      suggestions.push({ type: "hashtag", label: h }),
    );
  ...
  const hasResults = creatorResults.length > 0 || vibeResults.length > 0;

  return (
    <div className="flex flex-col w-full animate-[scaleIn_0.2s_ease-out]">
      {/* Tabs */}
      <div className="flex items-center px-4 mb-4 gap-2 border-b border-white/5 pb-2">
        {(["all", "creators", "vibes"] as const).map((tab) => (
          <button
            key={tab}
              ...
              {autocompleteSuggestions.map((s, i) => (
                <div
                  ...
                  onClick={() =>
                    navigate(
                      s.type === "hashtag"
                        ? `/hashtag/${encodeURIComponent(s.label)}`
                        : s.type === "creator"
                          ? `/profile/${s.label.replace("@", "")}`
                          : `/search?q=${s.label}`,
                    )
                  }
                >
                  ...
                  {s.type === "hashtag" && (
                    <span className="text-white/40 text-[10px] ml-auto uppercase tracking-wider font-bold">
                      Hashtag
                    </span>
                  )}
                </div>
        ...
        {!query.trim() ? (
          ...
          <div className="flex flex-col gap-6">
            {recentSearches.length > 0 && (
              <div>
                <h3 className="text-white/60 text-xs font-bold uppercase tracking-wider mb-3 px-1">
                  Recent Searches
                </h3>
                <div className="flex flex-col gap-1">
                  {recentSearches.map((term) => (
                    <div
                      key={term}
                      onClick={() =>
                        navigate(`/hashtag/${encodeURIComponent(term)}`)
                      }
                    ...
                    >
                      ...
                      <button
                        ...
                        className="p-1 rounded-full hover:bg-white/10 opacity-50 group-hover:opacity-100 transition-opacity"
                      >
          ...
          <div className="flex flex-col gap-6 pb-6">
            ...
            {(activeTab === "all" || activeTab === "vibes") &&
              vibeResults.length > 0 && (
                <div>
                  {activeTab === "all" && (
                    <h3 className="text-white/60 text-xs font-bold uppercase tracking-wider mb-3 px-1 mt-2">
                      Vibes
                    </h3>
                  )}
                  <div className="flex flex-col gap-3">
            ...
            {activeTab === "vibes" && vibeResults.length === 0 && (
              <div className="py-20 flex flex-col items-center justify-center text-center px-4">
                ...
                <button
                  ...
                  className="bg-white/10 hover:bg-white/20 text-white font-bold px-6 py-3 rounded-full border border-white/20 transition-all text-sm flex items-center gap-2"
                >
                  See Trending{" "}
  ...
  return (
    <section className="mb-8">
      <div className="flex items-center justify-between mb-4 px-4">
        ...
        <button
          onClick={onSeeAll}
          className="text-white/60 text-sm font-semibold hover:text-white transition-colors flex items-center"
        >
          See All <ChevronRight className="w-4 h-4 ml-0.5" />
        </button>
      </div>
          ...
          trendingVibes.map((vibe, idx) => (
            ...
            >
              ...
              <div className="absolute top-2 left-2 z-10">
                {idx === 0 ? (
                  <div className="bg-yellow-500 text-black text-[10px] font-black px-2 py-0.5 rounded-full shadow-[0_0_10px_orange]">
                    #1 👑
                  </div>
                ) : idx === 1 ? (
                  <div className="bg-gray-300 text-black text-[10px] font-black px-2 py-0.5 rounded-full opacity-90 shadow-md">
                    #2
                  </div>
                ) : idx === 2 ? (
                  <div className="bg-amber-700 text-white text-[10px] font-black px-2 py-0.5 rounded-full opacity-90 shadow-md">
                    #3
                  </div>
                ) : (
                  <div className="bg-black/50 backdrop-blur-md text-white text-[10px] font-bold px-2 py-0.5 rounded-full border border-white/20">
                    #{idx + 1}
                  </div>
                )}
              </div>
              ...
              <div className="absolute bottom-3 left-3 right-3 flex flex-col gap-1">
                <CaptionWithHashtags
                  ...
                  className="text-white text-[11px] font-medium leading-snug line-clamp-2 drop-shadow-md"
                />
                <div className="flex items-center gap-1.5 mt-0.5">
                  <div className="flex items-center gap-0.5">
                    ...
                    <span className="text-white font-bold text-[10px] drop-shadow-md">
                      {formatCount(
                        vibe.pulseCount || vibe.reactions?.pulse || 0,
                      )}
                    </span>
                  </div>
                </div>
    ...
    <section className="mb-8">
      ...
      <div className="flex flex-col gap-2 px-4 pb-2">
        {trendingHashtags.map((ht, idx) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="text-white/40 font-bold text-sm w-4">
                {idx + 1}
              </span>
              <div className="flex flex-col">
...
const getRisingScore = (vibe: any) => {
  ...
  const recentPulses = (vibe.pulseHistory || []).filter(
    (p: any) => p.timestamp > oneHourAgo,
  ).length;
  const recentComments = (vibe.commentHistory || []).filter(
    (c: any) => c.timestamp > oneHourAgo,
  ).length;
  const recentShares = (vibe.shareHistory || []).filter(
    (s: any) => s.timestamp > oneHourAgo,
  ).length;
...
function RisingCard({ vibe }: { vibe: any; key?: any }) {
  ...
  const velocity = Math.max(
    50,
    Math.floor(
      (vibe.pulseHistory?.filter((p: any) => p.timestamp > Date.now() - 3600000)
        .length || 0) * 60,
    ),
  );
    ...
    >
      ...
      <div className="absolute top-2 left-2 z-10 flex items-center justify-center pointer-events-none">
        <div className="bg-[#FF6B35]/20 backdrop-blur-md rounded-full px-2 py-0.5 border border-[#FF6B35]/50 flex items-center gap-1 shadow-[0_0_10px_rgba(255,107,53,0.5)]">
          ...
          <span className="text-[#FF6B35] text-[9px] font-black tracking-wider uppercase">
            Rising
          </span>
        </div>
      </div>
      ...
      <div className="absolute bottom-2 left-2 right-2 flex flex-col gap-1 z-10 pointer-events-none">
        <CaptionWithHashtags
          caption={vibe.caption}
          className="text-white text-[10px] whitespace-nowrap overflow-hidden text-ellipsis drop-shadow-md"
        />
        <p className="text-[#00F0FF] text-[9px] font-bold">
          Rising for {risingMins} mins 🚀
        </p>
        <div className="flex items-center justify-between mt-0.5">
          <div className="flex items-center gap-0.5">
            <span className="text-[10px] animate-pulse">⚡</span>
            <span className="text-white font-bold text-[10px] drop-shadow-md">
              {formatCount(livePulse)}
            </span>
          </div>
          <span className="text-[#FF6B35] text-[9px] font-bold bg-black/40 px-1 py-0.5 rounded shadow">
            +{formatCount(velocity)}/hr
          </span>
        </div>
      </div>
    </div>
  ...
  useEffect(() => {
    const fetchRising = async () => {
      try {
        const reels = await getReels();
        const processed = reels.map((r) => {
          const pulseHistory = Array.from({
            length: Math.floor(Math.random() * 100),
          }).map(() => ({
            timestamp: Date.now() - Math.random() * 2 * 60 * 60 * 1000,
          }));
          const commentHistory = Array.from({
            length: Math.floor(Math.random() * 40),
          }).map(() => ({
            timestamp: Date.now() - Math.random() * 2 * 60 * 60 * 1000,
          }));
          const shareHistory = Array.from({
        ...
        const withScore = processed.map((v) => ({
          ...v,
          risingScore: getRisingScore(v),
        }));
        ...
        {loading ? (
          ...
          <div className="w-full flex flex-col items-center justify-center py-8 bg-white/5 rounded-2xl mx-4 border border-white/10 shrink-0">
            ...
            <p className="text-white/60 text-sm font-medium text-center">
              Nothing exploding yet... <br />
              Check back soon!
            </p>
          </div>
        ) : (
          risingVibes.map((vibe) => <RisingCard key={vibe.id} vibe={vibe} />)
        )}
      </div>
      <div className="h-px bg-white/5 mt-6 mx-4" />
    </section>
    ...
    const fetchAll = async () => {
      try {
        ...
        for (let i = 0; i < 7; i++) {
          moreReels = [
            ...moreReels,
            ...reels.map((r) => ({ ...r, id: `v_see_${i}_${r.id}` })),
          ];
        }
    ...
    <div className="fixed inset-0 z-[100] bg-[#0A0A12] overflow-y-auto pb-24">
      <div className="sticky top-0 z-50 bg-[#0A0A12]/80 backdrop-blur-xl border-b border-white/5 p-4 flex items-center justify-between">
        ...
        <h1 className="text-lg font-bold text-white flex items-center gap-2">
          <Flame className="w-5 h-5 text-orange-500" /> Top 100 Trending
        </h1>
      ...
      <div className="p-4 grid grid-cols-2 gap-3">
        {loading
          ? Array.from({ length: 10 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[3/4] w-full bg-white/5 rounded-2xl animate-pulse"
              />
            ))
          : trendingVibes.map((vibe, idx) => (
              <div
              ...
              >
                ...
                <div className="absolute top-2 left-2 z-10">
                  {idx === 0 ? (
                    ...
                    <div className="bg-gray-300 text-black text-[10px] font-black px-2 py-0.5 rounded-full shadow-md">
                      #2
                    </div>
                  ) : idx === 2 ? (
                    <div className="bg-amber-700 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-md">
                      #3
                    </div>
                  ) : (
                    <div className="bg-black/50 backdrop-blur-md text-white text-[10px] font-bold px-2 py-0.5 rounded-full border border-white/20">
...
const getRoomOrder = (userLanguages: string[], allRooms: any[]) => {
  ...
  const otherRooms = allRooms
    .filter((r) => !userLanguages.includes(r.langCode))
    .sort((a, b) => b.vibeCount - a.vibeCount);
  return [...myRooms, ...otherRooms];
};

function LanguageRoomSection({
  loading,
  onOpenRoom,
}: {
  loading: boolean;
  ...
  const orderedRooms = getRoomOrder(userLanguages, MOCK_ROOMS).filter(
    (r) => r.vibeCount >= 10,
  );
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-white/60 text-xs mb-3">
                    {formatCount(room.vibeCount)} vibes today
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between mt-auto">
                  <div className="flex -space-x-2">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="w-8 h-8 rounded-full border-2 border-[#151520] overflow-hidden bg-white/10"
                      >
                        <img
                          src={`https://picsum.photos/seed/${room.langCode}${i}/100/100`}
                          alt="preview"
                          className="w-full h-full object-cover"
                        />
                      </div>
    ...
    const fetchData = async () => {
      try {
        ...
        const withRising = localizedReels.map((r) => ({
          ...r,
          pulseHistory: Array.from({
            length: Math.floor(Math.random() * 50),
          }).map(() => ({
            timestamp: Date.now() - Math.random() * 2 * 60 * 60 * 1000,
          })),
          commentHistory: Array.from({
            length: Math.floor(Math.random() * 20),
          }).map(() => ({
            timestamp: Date.now() - Math.random() * 2 * 60 * 60 * 1000,
          })),
          shareHistory: Array.from({
        ...
        setRecentVibes(
          localizedReels.sort((a, b) => b.createdAt - a.createdAt).slice(0, 6),
        );
              ...
              {topVibes.map((vibe, idx) => (
                ...
                >
                  ...
                  <div className="absolute bottom-3 left-3 right-3 flex flex-col gap-1">
                    <CaptionWithHashtags
                      caption={vibe.caption}
                      className="text-white text-[11px] font-medium leading-snug line-clamp-1 drop-shadow-md"
                    />
                    <div className="flex items-center gap-0.5">
                      <span className="text-[10px]">⚡</span>
                      <span className="text-white font-bold text-[10px] drop-shadow-md">
                        {formatCount(vibe.pulseCount)}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar px-4 pb-2 snap-x">
              {recentVibes.map((vibe) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent pointer-events-none" />
                  <div className="absolute top-2 left-2 z-10">
                    <div className="bg-purple-500/80 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-md">
                      NEW
                    </div>
                  </div>
                  <div className="absolute bottom-3 left-3 right-3 flex flex-col gap-1">
                    <CaptionWithHashtags
                      caption={vibe.caption}
                      className="text-white text-[10px] font-medium leading-snug line-clamp-2 drop-shadow-md"
                    />
                  </div>
                </div>
              ))}
...
function LeaderboardSection({ loading }: { loading: boolean; key?: any }) {
  ...
  useEffect(() => {
    ...
    const fetchTopCreators = async () => {
      const mockCreators = Array.from({ length: 15 })
        .map((_, i) => {
          ...
          return {
            ...
            avatar: isUser
              ? localStorage.getItem("skrimchat_avatar") ||
                `https://i.pravatar.cc/150?img=${Math.floor(Math.random() * 50) + 10}`
      ...
      {loading || creators.length < 3 ? (
        ...
        <div className="px-4 flex flex-col gap-5">
            ...
            <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/30 rounded-2xl p-4 flex items-center justify-between shadow-[0_0_15px_rgba(234,179,8,0.15)] relative overflow-hidden">
              ...
              <div className="relative z-10 hidden sm:block">
                <h3 className="text-white font-bold text-sm tracking-tight drop-shadow-md">
                  🏆 You're #{userRankData.rank} in <br />
                  Andhra Pradesh!
                </h3>
              </div>
              <div className="relative z-10 sm:hidden">
                <h3 className="text-white font-bold text-[13px] tracking-tight drop-shadow-md">
                  🏆 #{userRankData.rank} in AP!
                </h3>
              </div>
              <button
                onClick={() =>
                  generateShareCard(userRankData.rank, "Andhra Pradesh")
                }
                className="relative z-10 bg-yellow-500 hover:bg-yellow-400 text-black text-xs font-bold px-3 py-1.5 rounded-full shadow-lg transition-colors active:scale-95 flex items-center gap-1.5"
              >
          ...
          <div className="flex items-end justify-center gap-3 pt-6 pb-2">
            ...
            {top3[1] && (
              <div
                ...
                className="flex flex-col items-center gap-2 cursor-pointer pb-2 group w-[30%]"
              >
                <div className="relative">
                  <img
                    src={top3[1].avatar}
                    className="w-14 h-14 rounded-full border-2 border-gray-300 object-cover group-hover:scale-105 transition-transform shadow-[0_0_15px_rgba(209,213,219,0.3)]"
                  />
                  <div className="absolute -bottom-2 -right-2 text-xl drop-shadow-md">
                    🥈
                  </div>
                </div>
                <div className="flex flex-col items-center text-center w-full">
                  <span
                    className={`text-[11px] font-bold truncate w-full ${top3[1].isUser ? "text-[#00F0FF]" : "text-white"}`}
                  >
                    {top3[1].displayName}
                  </span>
            ...
            {top3[0] && (
              <div
                ...
                className="flex flex-col items-center gap-2 cursor-pointer pb-8 group w-[40%]"
              >
                <div className="relative">
                  <img
                    src={top3[0].avatar}
                    className="w-20 h-20 rounded-full border-2 border-yellow-400 object-cover group-hover:scale-105 transition-transform shadow-[0_0_20px_rgba(250,204,21,0.4)]"
                  />
                  <div className="absolute -bottom-3 -right-2 text-3xl drop-shadow-lg">
                    🥇
                  </div>
                </div>
                <div className="flex flex-col items-center text-center w-full">
                  <span
                    className={`text-sm font-black truncate w-full ${top3[0].isUser ? "text-[#00F0FF]" : "text-white"}`}
                  >
                    {top3[0].displayName}
                  </span>
                  <span className="text-yellow-400 text-xs font-black drop-shadow-sm">
                    {formatCount(top3[0].score)} pts
                  </span>
                </div>
              </div>
            )}
            ...
            {top3[2] && (
              <div
                ...
                className="flex flex-col items-center gap-2 cursor-pointer group w-[30%]"
              >
                <div className="relative pb-0 sm:pb-2">
                  <img
                    src={top3[2].avatar}
                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-amber-700 object-cover group-hover:scale-105 transition-transform shadow-[0_0_15px_rgba(180,83,9,0.3)]"
                  />
                  <div className="absolute -bottom-2 -right-2 text-xl drop-shadow-md">
                    🥉
                  </div>
                ...
                <div className="flex flex-col items-center text-center w-full">
                  <span
                    className={`text-[11px] font-bold truncate w-full ${top3[2].isUser ? "text-[#00F0FF]" : "text-white"}`}
                  >
                    {top3[2].displayName}
                  </span>
            ...
            {restOut.map((c, i) => (
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <span className="text-white/40 text-sm font-bold w-5 text-center shrink-0">
                    #{i + 4}
                  </span>
                  <img
                    src={c.avatar}
                  ...
                  />
                  <span
                    className={`text-sm font-bold truncate ${c.isUser ? "text-[#00F0FF]" : "text-white"}`}
                  >
                    {c.displayName}
                  </span>
                </div>
          ...
          <div className="flex justify-center mt-2 mb-4">
            <span className="text-white/40 text-[10px] font-bold uppercase tracking-wider bg-white/5 px-3 py-1 rounded-full border border-white/5">
              {timeLeft}
            </span>
          </div>
        </div>
    ...
    <section className="mb-8 px-4">
      ...
      >
        ...
        <div className="bg-[#0A0A12]/80 backdrop-blur-md rounded-[30px] p-6 flex flex-col items-center justify-center relative z-10 text-center gap-3 w-full">
          <div
            className={`text-5xl transition-transform duration-500 ${isSpinning ? "rotate-[360deg] scale-110" : ""}`}
          >
            🎲
          </div>
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-white/70 text-sm font-medium">
              "Throw me into something random ⚡"
            </p>
          </div>
...
function SurprisePlayerOverlay({
  vibe,
  onClose,
}: {
  vibe: any;
        ...
        <div className="absolute top-20 left-1/2 -translate-x-1/2 w-[90%] max-w-[300px] z-20 animate-[slideDown_0.3s_ease-out]">
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-3 flex items-center gap-3 shadow-2xl">
            ...
            <div>
              ...
              <p className="text-[#00F0FF] text-[11px] font-black drop-shadow-md">
                {vibe.user} in {vibe.creatorCity}
              </p>
            </div>
          </div>
        </div>
...
function AnotherSurprisePrompt({
  onHitMeAgain,
  onBackToDiscover,
}: {
  onHitMeAgain: () => void;
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#151520] border border-white/10 rounded-3xl p-6 w-full max-w-[320px] flex flex-col items-center text-center animate-[scaleIn_0.2s_ease-out]">
        ...
        <h3 className="text-white font-bold text-xl tracking-wide mb-1">
          Another surprise? ⚡
        </h3>
        <p className="text-white/50 text-sm mb-6">
          The next one might blow your mind.
        </p>

        <div className="flex flex-col gap-3 w-full">
          <button
            onClick={onHitMeAgain}
            className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-400 hover:to-pink-400 text-white font-bold py-3.5 rounded-2xl transition-all shadow-[0_0_20px_rgba(219,39,119,0.3)] hover:shadow-[0_0_30px_rgba(219,39,119,0.5)] active:scale-95"
          >
            Yes! Hit me again
          </button>
          <button
            onClick={onBackToDiscover}
            className="w-full bg-white/5 hover:bg-white/10 text-white font-bold py-3.5 rounded-2xl transition-all border border-white/5 active:scale-95"
          >
            Back to Discover
          </button>
        </div>
...
function YourPeopleSection({
  ...
  useEffect(() => {
    ...
    const processedVibes = allVibes.map((vibe) => {
      ...
      const engagedFollowers = [...mockFollowing]
    ...
    if (followers.length === 1)
      return (
        <span className="text-[10px] text-white/70">
          <b>{followers[0].username}</b> watched this ⚡
        </span>
      );
    if (followers.length === 2)
      return (
        <span className="text-[10px] text-white/70">
          <b>{followers[0].username}</b> and <b>{followers[1].username}</b>{" "}
          watched this
        </span>
      );
    return (
      <span className="text-[10px] text-white/70">
        <b>{followers[0].username}</b>, <b>{followers[1].username}</b> and{" "}
        {followers.length - 2} others watched this 🔥
      </span>
    );
  };
        ...
        {loading ? (
          ...
          <div className="w-full flex flex-col items-center justify-center py-8 bg-white/5 rounded-2xl mx-4 border border-white/10 shrink-0">
            ...
            <p className="text-white/80 text-sm font-medium text-center mb-4 leading-tight">
              Follow more creators <br />
              to see what your people are watching!
            </p>
          ...
          peopleWatchingVibes.map((vibe) => (
            ...
            >
              ...
              <div className="absolute top-2 right-2 flex -space-x-2 z-10 flex-row-reverse space-x-reverse">
                {vibe._engagedFollowers.slice(0, 3).map((f: any, i: number) => (
                  <img
                    key={f.id}
                    src={f.avatar}
                    className="w-6 h-6 rounded-full border border-white/20 object-cover shadow-sm bg-[#151520]"
                    style={{ zIndex: 10 - i }}
                  />
                ))}
                {vibe._engagedFollowers.length > 3 && (
                  <div
                    className="w-6 h-6 rounded-full border border-white/20 bg-black/60 backdrop-blur-sm text-white text-[9px] font-bold flex items-center justify-center relative shadow-sm"
                    style={{ zIndex: 1 }}
                  >
                    +{vibe._engagedFollowers.length - 3}
                  </div>
              ...
              <div className="absolute bottom-0 w-full p-3 bg-[#151520] flex flex-col h-[35%] justify-between border-t border-white/5">
                <CaptionWithHashtags
                  caption={vibe.caption}
                  className="text-white text-[11px] font-medium leading-snug line-clamp-2"
                />
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          transform: `translateY(${pullY / 2}px)`,
      ...
      <div className="sticky top-0 z-50 bg-[#0A0A12]/80 backdrop-blur-xl border-b border-white/5 pb-3">
        <div className="flex items-center justify-between px-4 pt-2 mb-3">
          <h1 className="text-xl font-black text-white px-1 tracking-tight">Skrim</h1>
          <StealthAvatar />
        </div>
        <div className="px-4 mb-4">
          <div
            className={`relative flex items-center transition-all ${searchFocused ? "ring-2 ring-[#B026FF]" : ""} rounded-2xl overflow-hidden`}
          >
      ...
      {searchFocused ? (
        ...
        <div className="pt-6">
          {(selectedFilter === "all"
            ? DISCOVER_SECTIONS
            : DISCOVER_SECTIONS.filter((s) => s.id === selectedFilter)
          ).map((section) =>
            section.id === "games" ? (
              ...
              <TrendingSection
                key={section.id}
                loading={loading}
                onSeeAll={() => setViewTrendingAll(true)}
              />
            ) : section.id === "hashtags" ? (
      ...
      {viewTrendingAll && (
        <TrendingAllScreen onClose={() => setViewTrendingAll(false)} />
      )}
      {selectedRoom && (
        <RoomScreen room={selectedRoom} onClose={() => setSelectedRoom(null)} />
      )}

      {/* Surprise Player & Popups */}
      {activeSurpriseVibe && (
        <SurprisePlayerOverlay
          vibe={activeSurpriseVibe}
