...
export default function LagoriGameScreen() {
  ...
  const loadLevel = useCallback(
    (lvl: number) => {
      ...
      if (lvl === 2) {
        ...
        stackX = 300;
        numStones = Math.min(10, 7 + lvl - 4);
        moving = true;
        timeLimit = 15 + (lvl - 4) * 2;
      }
    ...
    if (s.playPhase === "AIMING") {
      ...
      if (clickedStones.length > 0 || (closestStone && minDist < 80)) {
        ...
        if (
          !expected &&
          closestStone &&
          closestStone.id === s.nextExpectedId &&
          minDist < 80
        ) {
      ...
      if (s.playPhase === "AIMING") {
        ...
        if (s.ball.active) {
          ...
          if (
            !s.stackBroken &&
            (s.ball.x > CANVAS_W ||
              (s.ball.y >= GROUND_Y - s.ball.r && Math.abs(s.ball.vx) < 5))
          ) {
            if (s.shotsLeft > 0) {
              s.playPhase = "AIMING";
              s.ball.x = s.ball.startX;
        ...
        for (let st of s.stones) {
          ...
          if (st.x + st.w / 2 > CANVAS_W) {
            st.x = CANVAS_W - st.w / 2;
            st.vx *= -BOUNCE;
          }
  ...
  const renderCanvas = useCallback(() => {
    ...
    if (s.ball.active) {
      ...
      ctx.beginPath();
      ctx.arc(s.ball.x - 4, s.ball.y - 4, s.ball.r / 3, 0, Math.PI * 2);
      ctx.fill();
    }
  ...
  return (
    <div className="flex flex-col h-full bg-[#111827] text-white overflow-hidden select-none relative">
      <div className="absolute top-4 left-4 z-50">
        <Link
          to="/games"
          className="w-10 h-10 bg-black/40 rounded-full flex items-center justify-center hover:bg-black/60 backdrop-blur-md"
        >
          <X className="w-6 h-6" />
        </Link>
      </div>
      ...
      <div
        className="flex-1 w-full relative flex items-center justify-center min-h-[600px] overflow-hidden"
            ...
            <div className="absolute top-4 w-full max-w-sm px-4 flex justify-between items-start pointer-events-none z-10">
              ...
              {uiState.playPhase === "RESTACKING" && (
                <div className="bg-black/80 text-white px-6 py-3 rounded-2xl border border-red-500/50 flex flex-col items-center animate-pulse backdrop-blur-sm">
                  <span className="text-xs uppercase font-bold text-red-300 tracking-widest">
                    Time
                  </span>
                  <span className="text-3xl font-black text-red-500">
                    {uiState.timeRemaining.toFixed(1)}
                  </span>
                </div>
              )}
            ...
            {uiState.playPhase === "RESTACKING" && (
              ...
              >
                ...
                <span className="bg-black/80 px-6 py-3 rounded-full text-white text-lg font-bold backdrop-blur-md border-[3px] border-amber-400/50 shadow-[0_0_30px_rgba(251,191,36,0.3)] text-center">
                  Tap the{" "}
                  <span className="text-amber-400 uppercase tracking-widest text-xl">
                    LARGEST
                  </span>{" "}
                  highlighted stone!
                </span>
          ...
          {appPhase === "MENU" && (
            ...
            >
              <div className="max-w-sm w-full text-center">
                ...
                <p className="text-amber-100/70 font-bold tracking-widest text-xs uppercase mb-10">
                  Seven Stones
                </p>
                ...
                <div className="bg-white/5 rounded-3xl p-6 border border-white/10 mb-8 backdrop-blur-xl text-left space-y-4">
                  <p className="flex items-start gap-3 text-sm text-white/80 font-medium">
                    <span className="bg-orange-500/20 text-orange-400 w-6 h-6 rounded-full flex items-center justify-center shrink-0 font-bold">
                      1
                    </span>
                    <span>
                      Drag & release to throw the ball. You have 3 balls to
                  ...
                  <p className="flex items-start gap-3 text-sm text-white/80 font-medium">
                    ...
                    <span>
                      Once broken, tap the scattered stones to rebuild the
                      stack.
                    </span>
                  </p>
                  <p className="flex items-start gap-3 text-sm text-white/80 font-medium">
                    ...
                    <span>
                      Always tap the{" "}
                      <strong className="text-amber-400">largest</strong>{" "}
                      remaining stone! Be fast before time runs out.
                    </span>
                  </p>
                </div>
            ...
            >
              <div className="bg-gray-900 border border-white/10 p-8 rounded-3xl w-full max-w-sm text-center shadow-2xl">
                {uiState.playPhase === "RESTACKING" ? (
                  <>
                    ...
                    <p className="text-white/60 mb-6">
                      You didn't rebuild the stack in time.
                    </p>
                  </>
                ) : (
                  <>
                ...
                <div className="bg-black/40 rounded-xl py-4 mb-6 border border-white/5">
                  ...
                  <span className="block text-[10px] uppercase font-bold text-white/50 mb-1 tracking-wider">
                    Final Score
                  </span>
          ...
          {appPhase === "LEVEL_CLEAR" && (
            ...
            >
              <div className="bg-gradient-to-b from-orange-900 to-amber-900 border border-orange-500/50 p-8 rounded-3xl w-full max-w-sm text-center shadow-2xl">
                ...
                <p className="text-orange-200 mb-6 font-medium">
                  Great stack rebuilding.
                </p>

                <div className="bg-black/30 rounded-xl py-4 mb-8">
                  <span className="block text-[10px] uppercase font-bold text-amber-200/50 mb-1 tracking-wider">
                    Total Score
                  </span>
