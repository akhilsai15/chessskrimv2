        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar">
          {["Overview", "Audience", "Monetization"].map((tab) => (
            <button
              ...
              className={`text-sm font-medium pb-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === tab ? "border-neon-blue text-white" : "border-transparent text-gray-500"}`}
            >
              {tab}
            </button>
          ))}
        ...
        {loading || !stats ? (
          ...
          <>
            ...
            <div className="bg-skrim-surface p-4 rounded-2xl border border-white/5 h-48 flex flex-col justify-end relative overflow-hidden">
              <h3 className="absolute top-4 left-4 text-xs font-bold uppercase text-gray-400">
                Activity Chart
              </h3>
              <div className="flex items-end gap-2 w-full h-24 mt-8 px-2 justify-between">
                {stats.chartData.map((val: number, i: number) => (
                  <div
                    key={i}
                    className="w-8 bg-neon-blue rounded-t-sm opacity-80"
                    style={{ height: `${val}%` }}
                  />
                ))}
              </div>
            ...
            <div className="bg-gradient-to-tr from-neon-purple/20 to-transparent p-5 rounded-2xl border border-neon-purple/30">
              ...
              <p className="text-xs text-gray-300 mb-4">
                You are eligible for the Creator Fund and ad revenue sharing.
              </p>
              <button className="bg-neon-purple text-white font-bold text-xs px-4 py-2 rounded-xl shadow-neon-purple">
                Manage Payouts
              </button>
            </div>
          </>
