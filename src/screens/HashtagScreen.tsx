...
export default function HashtagScreen() {
    ...
    const fetchHashtagData = async () => {
      try {
        ...
        let hashtagReels = allReels.filter((r) =>
          extractHashtags(r.caption || "").some(
            (h) => h.toLowerCase() === hashtag.toLowerCase(),
          ),
        );
  ...
  const topVibes = [...vibes].sort(
    (a, b) => (b.pulseCount || 0) - (a.pulseCount || 0),
  );
  const recentVibes = [...vibes].sort(
    (a, b) => (b.createdAt || 0) - (a.createdAt || 0),
  );
  const displayVibes = activeTab === "top" ? topVibes : recentVibes;

  const topVibe = topVibes[0];
  const totalViews = vibes.reduce(
    (sum, v) => sum + (v.pulseCount || 100) * 3,
    0,
  ); // rough estimate

  const isChallenge =
    hashtag.toLowerCase().startsWith("#skrim") ||
    hashtag.toLowerCase().includes("challenge");
        ...
        <div className="flex flex-col">
            ...
            <div className="mx-4 mt-4 bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl p-4 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-white/80 text-sm font-medium mb-4">
                  Join {formatCount(vibes.length * 15)} creators!
                </p>
          ...
          <div className="px-4 py-6 text-center flex flex-col items-center">
            ...
            <p className="text-white/60 text-sm font-medium">
              <span className="text-white">{formatCount(totalViews)}</span>{" "}
              total views •{" "}
              <span className="text-white">{formatCount(vibes.length)}</span>{" "}
              vibes
            </p>
          </div>
            ...
            <div className="px-4 mb-6">
              ...
              >
                {topVibe.videoImageHover ? (
                  <video
                    src={topVibe.videoImageHover}
                    poster={topVibe.avatar}
                ...
                <div className="absolute bottom-4 left-4 right-4 flex flex-col gap-2">
                  ...
                  <CaptionWithHashtags
                    caption={topVibe.caption}
                    className="text-white text-sm font-medium drop-shadow-md leading-snug line-clamp-2"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="sticky top-[68px] z-40 bg-[#0A0A12]/90 backdrop-blur-xl border-y border-white/5 flex mb-4">
            ...
            >
              🔥 Top
              {activeTab === "top" && (
                <div className="absolute bottom-0 left-0 w-full h-[2px] bg-[#B026FF] shadow-[0_0_10px_rgba(176,38,255,0.5)]" />
              )}
            </button>
            <button
                ...
                <div className="absolute bottom-2 left-2 right-2">
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-white font-bold text-[10px] drop-shadow-md">
                      {formatCount(vibe.pulseCount || 0)}
                    </span>
                  </div>
                  <CaptionWithHashtags
                    caption={vibe.caption}
                    className="text-white/90 text-[10px] font-medium leading-snug line-clamp-1 drop-shadow-md"
                  />
                </div>
              </div>
            ))}
