...
export default function GilliDandaGameScreen() {
  ...
  const [appPhase, setAppPhase] = useState<"MENU" | "PLAYING" | "GAMEOVER">(
    "MENU",
  );
    ...
    setShots((prev) => {
      ...
      if (newShots.length >= 5) {
        ...
        const overallBest = Math.max(
          best,
          parseFloat(localStorage.getItem("gillidanda_best") || "0"),
        );
          ...
          <div className="absolute top-0 w-full max-w-sm h-full pointer-events-none p-4 flex flex-col pt-16">
            ...
            {(uiState.turnState === "FLICKED" ||
              ...
              <div className="w-full mb-8">
                <div className="text-center text-[10px] uppercase font-bold text-white/60 mb-2">
                  Timing ⚡
                </div>
            ...
            {uiState.turnState === "CHARGING" && (
              <div className="w-full">
                <div className="text-center text-[10px] uppercase font-bold text-orange-400 mb-2">
                  Power 🔋 (Release!)
                </div>
                <div className="bg-white/10 h-6 w-full rounded-full relative border border-white/20 overflow-hidden shadow-inner">
                  <div
                    className="absolute top-0 bottom-0 left-0 bg-gradient-to-r from-orange-500 to-red-500"
            ...
            <AnimatePresence>
              {uiState.hitRating && (
                <motion.div
                  ...
                  className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-black text-4xl whitespace-nowrap drop-shadow-[0_4px_10px_rgba(0,0,0,0.8)] ${
                    uiState.hitRating === "PERFECT"
                      ? "text-yellow-400"
                      : uiState.hitRating === "GOOD"
                        ? "text-blue-400"
                        : "text-red-500"
                  }`}
                >
              ...
              {uiState.turnState === "LANDED" &&
                uiState.hitRating !== "MISS" && (
                  <motion.div
                    key="distance-rating"
            ...
            >
              <div className="max-w-sm w-full">
                <div className="text-center mb-10">
                  ...
                  <p className="text-amber-100/70 font-bold tracking-widest text-[10px] uppercase">
                    Power Timing Challenge
                  </p>
                </div>
                ...
                <div className="bg-white/5 rounded-3xl p-5 border border-white/10 mb-8 w-full backdrop-blur-xl">
                  ...
                  <div className="text-xs text-white/70 font-bold space-y-4 pt-4 border-t border-white/10">
                    <p className="flex items-start gap-3">
                      <span className="bg-white/10 w-5 h-5 rounded-full flex items-center justify-center shrink-0">
                        1
                      </span>{" "}
                      <span>
                        <span className="text-white">Tap</span> to softly flick
                        the gilli up into the air.
                      </span>
                    </p>
                    <p className="flex items-start gap-3">
                      <span className="bg-white/10 w-5 h-5 rounded-full flex items-center justify-center shrink-0">
                      ...
                      <span>
                        <span className="text-white">Hold Tap</span> when the
                        timing bar is inside the 🔥 center zone.
                      </span>
                    </p>
                    <p className="flex items-start gap-3">
                      <span className="bg-white/10 w-5 h-5 rounded-full flex items-center justify-center shrink-0">
                      ...
                      <span>
                        Let power fill up, then{" "}
                        <span className="text-white">Release</span> to hit for
                        maximum distance!
                      </span>
                    </p>
                  </div>
            ...
            >
              ...
              <div className="space-y-2 mb-8 max-w-sm mx-auto">
                {shots.map((dist, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center bg-black/40 rounded-xl p-3 border border-white/5"
                  >
              ...
              <div className="bg-amber-500/10 rounded-3xl p-5 border border-amber-500/20 text-center mb-8 max-w-sm mx-auto">
                <span className="block text-amber-200/50 text-[10px] font-bold uppercase tracking-widest mb-1">
                  Round Best
                </span>
                <span className="text-amber-400 font-black text-5xl">
                  {Math.max(...shots)}m
                </span>
              </div>
