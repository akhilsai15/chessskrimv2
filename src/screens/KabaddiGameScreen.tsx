...
export default function KabaddiGameScreen() {
  ...
  const initTurn = useCallback(
    (turn: TurnPhase) => {
      ...
      s.raider = {
        ...
        vx: 0,
        vy: 0,
        dir: isRaid ? -Math.PI / 2 : Math.PI / 2,
        animPhase: 0,
        tags: 0,
        tackleProgress: 0,
        team: isRaid ? 1 : 2,
  ...
  const advanceTurn = useCallback(() => {
    ...
    if (
      s.mode === "PVP" &&
      prev &&
      prev.substring(0, 2) !== next.substring(0, 2)
    ) {
    ...
    (reason: "ESCAPE" | "CAUGHT" | "TIMEUP") => {
      ...
      if (playerRaiding) {
        if (reason === "ESCAPE") {
          ...
          if (isP1) s.scores.p1 += pts;
          else s.scores.p2 += pts;
  ...
  const updateEngine = useCallback(
    (dt: number) => {
      ...
      const playerRaiding = turn.includes("RAID");
      const inEnemyHalf =
        s.raider.team === 1 ? s.raider.y < MID_LINE : s.raider.y > MID_LINE;
      ...
      if (Math.abs(s.raider.vx) > 0 || Math.abs(s.raider.vy) > 0) {
        ...
        s.raider.animPhase +=
          Math.hypot(s.raider.vx, s.raider.vy) * (dt / 1000) * 0.03;
      } else {
  ...
  const renderCanvas = useCallback(() => {
    ...
    ctx.beginPath();
    ctx.moveTo(10, MID_LINE - 80);
    ctx.lineTo(COURT_W - 10, MID_LINE - 80);
    ctx.moveTo(10, MID_LINE + 80);
    ctx.lineTo(COURT_W - 10, MID_LINE + 80);
    ctx.stroke();
    ctx.globalAlpha = 1.0;
    ...
    if (s.rallyPoint && !playerRaiding) {
      ...
      ctx.stroke();
      ctx.moveTo(s.rallyPoint.x - 5, s.rallyPoint.y);
      ctx.lineTo(s.rallyPoint.x + 5, s.rallyPoint.y);
      ctx.moveTo(s.rallyPoint.x, s.rallyPoint.y - 5);
      ctx.lineTo(s.rallyPoint.x, s.rallyPoint.y + 5);
      ctx.stroke();
    }
    ...
    s.defenders.forEach((def) => {
      ...
      const isTackling =
        !def.isTagged &&
        Math.hypot(s.raider.x - def.x, s.raider.y - def.y) <
          RAIDER_R + DEFENDER_R + 5;
      drawPlayer(
        ctx,
        def.x,
        def.y,
    ...
    if (s.status === "ACTIVE") {
      ...
      ctx.fillStyle =
        s.breath > 40 ? "#4ade80" : s.breath > 20 ? "#facc15" : "#ef4444";
  ...
  const handleChant = (e: React.PointerEvent) => {
    ...
    if (s.status === "ACTIVE" && s.turnCycle[s.turnIndex]?.includes("RAID")) {
      s.lastChantTime = 0.5;
      s.chantEffects.push({
        x: s.raider.x + (Math.random() * 40 - 20),
        y: s.raider.y - 30,
        life: 1,
      });
      ...
      <div className="flex-1 w-full relative z-10 flex flex-col items-center justify-center">
        ...
        {appPhase === "MENU" && (
          <motion.div
            ...
            className="flex flex-col w-full max-w-sm px-6"
          >
            <div className="w-full h-40 bg-green-900 rounded-3xl border-4 border-green-700/50 flex items-center justify-center mb-8 relative overflow-hidden">
              ...
              <div className="font-black text-5xl text-white drop-shadow-[0_4px_10px_rgba(0,0,0,0.5)] z-10">
                KABADDI
              </div>
            </div>
            ...
            <div className="space-y-4 w-full">
              ...
              >
                <div className="flex items-center gap-2">
                  <Users className="w-6 h-6" /> BATTLE MODE
                </div>
            ...
            <div className="mt-8 bg-white/5 p-4 rounded-xl border border-white/10 text-white/70 text-sm">
              ...
              <ul className="list-disc pl-4 space-y-1">
                <li>
                  <strong className="text-green-400">Raider:</strong> Drag to
                  move inside enemy half. Tap{" "}
                  <strong className="text-green-400 border border-green-500/30 px-1 rounded bg-green-500/20">
                    CHANT
                  </strong>{" "}
                  to hold breath. Tag enemies and return alive.
                </li>
                <li>
                  <strong className="text-blue-400">Defender:</strong> Drag/Tap
                  to rally your squad and surround the intruder to catch them!
                </li>
              </ul>
            </div>
          </motion.div>
        ...
        {/* === PLAYING === */}
        <div
          className={`flex flex-col w-full h-full max-w-sm ${appPhase === "PLAYING" ? "opacity-100" : "opacity-0 pointer-events-none absolute"}`}
        >
          {/* Top Scoreboard */}
          ...
          <div className="flex-1 w-full bg-black relative flex items-center justify-center overflow-hidden touch-none p-2">
            ...
            <AnimatePresence>
              {uiState.status === "ACTIVE" &&
                uiState.currentTurn.includes("RAID") && (
                  <motion.button
                    key="chant-btn"
          ...
          {passPvpTurn && appPhase === "PLAYING" && (
            ...
            >
              <motion.div
                initial={{ scale: 0.9 }}
                animate={{ scale: 1 }}
                className="bg-gray-900 border border-white/10 p-8 rounded-3xl max-w-sm w-full"
              >
                ...
                <p className="text-white/60 mb-8 font-bold">
                  ...
                  <span
                    className={
                      passPvpTurn === "P1" ? "text-blue-400" : "text-purple-400"
                    }
                  >
                    Player {passPvpTurn === "P1" ? "1" : "2"}'s
                  </span>{" "}
                  turn.
                </p>
                <button
                  onClick={() => {
                    const next =
                      engineRef.current.turnCycle[engineRef.current.turnIndex];
                    setPassPvpTurn(null);
                    initTurn(next);
                  }}
        ...
        {appPhase === "GAMEOVER" && (
          <motion.div
            ...
            className="flex flex-col w-full max-w-sm px-6 items-center justify-center"
          >
            <div className="text-7xl mb-6 drop-shadow-[0_0_20px_rgba(255,215,0,0.5)]">
              🏆
            </div>
            <h2 className="text-4xl font-black text-white mb-8 text-center text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500">
              {uiState.p1Score > uiState.p2Score
                ? "PLAYER 1 WINS"
                : uiState.p2Score > uiState.p1Score
                  ? mode === "SOLO"
                    ? "AI WINS"
                    : "PLAYER 2 WINS"
                  : "IT'S A TIE"}
            ...
            <div className="w-full bg-white/5 border border-white/10 rounded-3xl p-6 mb-8 backdrop-blur-sm shadow-2xl flex flex-col gap-4">
              ...
              <div className="flex justify-between items-center bg-purple-500/10 p-4 rounded-xl border border-purple-500/20">
                <strong className="text-purple-400 text-xl">
                  {mode === "SOLO" ? "AI" : "Player 2"}
                </strong>
