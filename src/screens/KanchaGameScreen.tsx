...
function getLevelConfig(lvl: number) {
  ...
  const maxShots = Math.max(
    5,
    Math.ceil(numTargets * 0.7) + Math.floor(lvl * 0.2),
  );
...
export default function KanchaGameScreen() {
  ...
  const forceUIRender = useCallback(() => {
    ...
    setUiState({
      ...
      score: s.score,
      levelTimeElapsed:
        Math.floor((performance.now() - s.levelStartTime) / 1000) || 0,
  ...
  const loadLevel = useCallback(
    (lvl: number) => {
      ...
      const { numTargets, maxShots, powerMultiplier, maxPower } =
        getLevelConfig(lvl);
      s.maxShots = maxShots;
      ...
      if (lvl === 1) {
        for (let i = 0; i < numTargets; i++) {
          addMarble(
            COURT_W / 2 + (i - Math.floor(numTargets / 2)) * 30,
            200,
        ...
        for (let i = 0; i < numTargets; i++) {
          ...
          addMarble(
            COURT_W / 2 + Math.cos(angle) * 40,
            200 + Math.sin(angle) * 40,
            colors[i % colors.length],
          );
        }
      } else if (lvl === 3) {
        ...
        for (let i = 0; i < numTargets; i++) {
          addMarble(
            COURT_W / 2 + ((i % 4) - 1.5) * 35,
            150 + Math.floor(i / 4) * 35,
            colors[i % colors.length],
          );
        }
      } else {
  ...
  const spawnCollisionParticles = (
    ...
    s.shockwaves.push({
      ...
      y,
      radius: 0,
      maxRadius: Math.max(20, speed * 0.5),
      life: 0.25,
      maxLife: 0.25,
      color: "#ffffff",
    });
      ...
      for (let i = 0; i < s.marbles.length; i++) {
        ...
        if (Math.abs(m.vx) > 0.5 || Math.abs(m.vy) > 0.5) {
          ...
          if (!m.isPlayer) {
            ...
            if (
              m.x < -m.radius ||
              m.x > COURT_W + m.radius ||
              m.y < -m.radius ||
              m.y > COURT_H + m.radius
            ) {
              m.active = false;
              m.vx = 0;
              m.vy = 0;
      ...
      if (s.turnState === "ROLLING" && !anyMoving) {
        ...
        if (targets === 0) {
          ...
          const timeTaken = Math.max(
            1,
            Math.floor((performance.now() - s.levelStartTime) / 1000),
          );
          ...
          let maxLvl = parseInt(
            localStorage.getItem("kancha_max_level") || "1",
          );
    ...
    if (s.drag.active && s.turnState === "AIMING") {
      ...
      if (pm) {
        ...
        const power = Math.min(
          Math.hypot(dx, dy) * s.powerMultiplier,
          s.maxPower,
        );
  ...
  const gameLoop = useCallback(
    (time: number) => {
      ...
      const currentElapsed = Math.floor(
        (performance.now() - engineRef.current.levelStartTime) / 1000,
      );
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    const s = engineRef.current;
    if (appPhase !== "PLAYING" || s.turnState !== "IDLE" || s.shotsLeft <= 0)
      return;
      ...
      <div className="flex-1 w-full relative z-10 flex flex-col items-center justify-center p-2">
        ...
        {appPhase === "MENU" && (
          <motion.div
            ...
            className="flex flex-col w-full max-w-sm px-6 items-center"
          >
            ...
            <h1 className="text-4xl font-black mb-8 tracking-tight text-white/90">
              Aim & Shoot!
            </h1>
            ...
            <div className="bg-white/5 p-5 rounded-3xl border border-white/10 text-white/70 text-sm w-full">
              ...
              <ul className="list-disc pl-4 space-y-2">
                <li>
                  <strong className="text-orange-300">Drag</strong> your blue
                  marble to aim.
                </li>
                <li>
                  <strong className="text-orange-300">Release</strong> to shoot
                  and hit the other marbles.
                </li>
                <li>
                  Knock target marbles{" "}
                  <strong className="text-orange-300">off the board</strong> to
                  score.
                </li>
                <li>
                  Clear all target marbles before running out of{" "}
                  <strong className="text-blue-300">shots</strong>.
                </li>
              </ul>
            </div>
          </motion.div>
        ...
        >
          <div className="flex justify-between items-center px-4 py-2 bg-black/40 rounded-t-3xl border-t border-x border-white/10 mt-2">
            ...
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-lg font-black ${uiState.shotsLeft <= 1 ? "text-red-400" : "text-blue-400"}`}
              >
          ...
          {appPhase === "LEVEL_CLEAR" && (
            ...
            >
              <div className="bg-gradient-to-b from-orange-900 to-amber-900 border border-orange-500/50 p-8 rounded-3xl max-w-sm w-full text-center shadow-2xl">
                ...
                <p className="text-orange-200 mb-2 font-bold">
                  Awesome aiming.
                </p>
                <div className="bg-black/30 rounded-xl p-4 mb-8">
                  <p className="text-sm text-amber-200/70 font-bold uppercase tracking-wider mb-1">
                    Total Score
                  </p>
            ...
            >
              <div className="bg-gray-900 border border-white/10 p-8 rounded-3xl max-w-sm w-full text-center">
                ...
                <p className="text-white/60 mb-2">
                  You couldn't clear all targets.
                </p>
                <div className="bg-black/50 rounded-xl p-4 mb-8">
                  <p className="text-sm text-white/50 font-bold uppercase tracking-wider mb-1">
                    Final Score
                  </p>
