...
export default function EmojiGuessScreen() {
  ...
  const [animatingWrongChar, setAnimatingWrongChar] = useState<string | null>(
    null,
  );
  const [floatNotice, setFloatNotice] = useState<{
    text: string;
    color: string;
  } | null>(null);
  ...
  const startGameWithSub = (subcategory: string | null) => {
    ...
    if (subcategory) {
      ...
      const subQ = selectedCategory.questions
        .filter((q) => q.subcategory === subcategory)
        .sort(() => Math.random() - 0.5);
      const catOtherQ = selectedCategory.questions
        .filter((q) => q.subcategory !== subcategory)
        .sort(() => Math.random() - 0.5);
      const globalOtherQ = EMOJI_CATEGORIES.filter(
      ...
      const catQ = [...selectedCategory.questions].sort(
        () => Math.random() - 0.5,
      );
      const globalOtherQ = EMOJI_CATEGORIES.filter(
        (c) => c.id !== selectedCategory.id,
  ...
  const initiateGame = (qList: Question[]) => {
    if (qList.length === 0) {
      // fallback
      qList = EMOJI_CATEGORIES.flatMap((c) => c.questions).sort(
        () => Math.random() - 0.5,
      );
    }
    setQuestions(qList);
  ...
  const activeCategory = EMOJI_CATEGORIES.find((c) =>
    c.subcategories.includes(currentQ?.subcategory || ""),
  );
  ...
  const handleGuess = (letter: string) => {
    ...
    if (!currentQ.answer.includes(letter)) {
      ...
      const wrongCount = Array.from(newGuessed).filter(
        (c) => !currentQ.answer.includes(c),
      ).length;
      if (wrongCount >= 5) {
  ...
  const checkWin = (currentGuesses: Set<string>) => {
    if (!currentQ) return;
    const isWin = currentQ.answer
      .split("")
      .every((char) => char === " " || currentGuesses.has(char));
    ...
    if (isWin) {
      ...
      if (newStreak >= 5) {
        ...
        multiplier = 1.5;
        showFloat("🔥 STREAK x1.5", "text-amber-400");
      } else {
        ...
        {gameState === "MENU" && (
          ...
          >
            ...
            <div className="mt-4 mb-6 bg-gradient-to-r from-amber-500 to-orange-600 p-4 rounded-2xl shadow-lg border border-amber-400/30 relative overflow-hidden">
              <div className="absolute -right-4 -top-4 text-8xl opacity-10 filter drop-shadow-lg">
                🌟
              </div>
              <div className="relative z-10">
                ...
                <p className="text-amber-100/90 text-sm font-medium mt-1">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 sm:gap-4">
              {EMOJI_CATEGORIES.map((cat) => (
                ...
                >
                  ...
                  <span className="text-4xl filter drop-shadow-md z-10">
                    {cat.emoji}
                  </span>
                  <span className="text-xs font-bold text-center z-10 leading-tight w-full truncate">
                    {cat.label}
                  </span>
                  <span className="text-[9px] font-semibold opacity-60 z-10">
                    {cat.questions.length} puzzles
                  </span>
                </button>
              ))}
            </div>
        ...
        {gameState === "SUBCATEGORIES" && selectedCategory && (
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <div
                className="w-12 h-12 rounded-full flex items-center justify-center text-3xl shadow-lg border border-white/10"
              ...
              <div>
                ...
                <p className="text-sm opacity-60">
                  {selectedCategory.questions.length} total puzzles
                </p>
              </div>
            </div>
            ...
            >
              <span className="flex items-center gap-2 text-lg">
                <Shuffle className="w-5 h-5" /> Random Mix!
              </span>
            ...
            <div className="space-y-4">
              <h3 className="font-bold text-white/50 text-sm uppercase tracking-wider pl-1">
                Themes
              </h3>
              <div className="bg-white/5 rounded-2xl border border-white/10 overflow-hidden divide-y divide-white/10">
                {selectedCategory.subcategories.map((sub) => {
                  const count = selectedCategory.questions.filter(
                    (q) => q.subcategory === sub,
                  ).length;
                  return (
                    <button
                    ...
                    >
                      ...
                      <span className="text-xs bg-white/10 px-2.5 py-1 rounded-full font-medium text-white/70">
                        {count} items
                      </span>
                    </button>
                  );
          ...
          >
            ...
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-2">
                <span className="text-2xl filter drop-shadow-sm">
                  {isDailyChallenge
                    ? "🌟"
                    : activeCategory?.emoji || selectedCategory?.emoji}
                </span>
                <div>
                  <div className="font-black text-sm opacity-90 truncate max-w-[120px]">
                    {isDailyChallenge ? "Daily Challenge" : activeSubcategory}
                  </div>
              ...
              <div className="flex flex-col items-end">
                ...
                <div
                  className={`font-mono text-sm font-bold flex items-center gap-1 ${timeLeft <= 10 ? "text-red-400 animate-pulse" : "text-white/70"}`}
                >
            ...
            <div className="flex justify-center gap-1.5 mb-6">
              {[...Array(3)].map((_, i) => (
                <Heart
                  key={i}
                  className={`w-5 h-5 transition-all duration-300 ${i < lives ? "fill-red-500 text-red-500 drop-shadow-[0_0_5px_rgba(239,68,68,0.6)]" : "fill-white/10 text-white/10 scale-90"}`}
                />
              ))}
            </div>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`w-7 h-9 sm:w-9 sm:h-11 border-b-4 flex items-center justify-center text-lg sm:text-xl font-black rounded-t-lg transition-colors
                          ${
                            isAnim
                              ? "border-green-400 text-white shadow-[0_0_15px_rgba(74,222,128,0.5)]"
                              : isGuessed
                                ? "border-blue-500 bg-blue-900/40 text-blue-100 shadow-[0_0_10px_rgba(59,130,246,0.3)]"
            ...
            <div className="mt-auto">
              <div className="flex flex-wrap justify-center gap-1.5 mb-6">
                {[...ALPHABET, "."].map((letter) => {
                  ...
                  const isCorrect =
                    isGuessed && currentQ.answer.includes(letter);
                  const isWrong =
                    isGuessed && !currentQ.answer.includes(letter);
                  ...
                  return (
                    <motion.button
                      ...
                      className={`
                        ...
                        ${
                          isCorrect
                            ? "bg-green-500 text-white border-b-2 border-green-700 opacity-100"
                            : isWrong
                              ? "bg-red-500/20 text-red-400 border border-red-500/30 opacity-40"
              ...
              <div className="flex justify-between gap-3">
                ...
                >
                  SKIP <span className="opacity-70 text-xs">-20</span>{" "}
