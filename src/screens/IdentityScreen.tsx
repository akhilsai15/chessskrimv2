          ...
          {dm.missions.map((m) => (
            <div key={m.id} className="relative">
              <div className="flex justify-between items-center mb-1">
                <p
                  className={`text-xs font-bold ${m.done ? "text-gray-400 line-through" : "text-gray-200"}`}
                >
                  {m.desc}
                </p>
                <p
                  className={`text-[10px] font-black ${m.done ? "text-[#00F0FF]" : "text-yellow-400"}`}
                >
                  +{m.points} ⚡
                </p>
              </div>
              <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${Math.min((m.currentCount / m.targetCount) * 100, 100)}%`,
                  }}
...
export default function IdentityScreen() {
  ...
  const { savedPosts, repostedPosts, hydrate } = useSavedStore();
  const [isEditing, setIsEditing] = useState(false);
  const [activeTab, setActiveTab] = useState("posts");
  const [activeSavedTab, setActiveSavedTab] = useState<"bookmarks" | "offline">(
    "bookmarks",
  );
  ...
  const { globalVibeNotificationsEnabled, toggleGlobalVibeNotifications } =
  ...
  const [activeStatType, setActiveStatType] = useState<
    "pulse" | "blaze" | "views" | "vibe" | null
  >(null);
  ...
  const [activeHighlightOptions, setActiveHighlightOptions] = useState<
    any | null
  >(null);
  const [activeHighlightRename, setActiveHighlightRename] = useState<
    any | null
  >(null);
  ...
  useEffect(() => {
    ...
    let pulse = parseInt(
      localStorage.getItem("skrimchat_pulse_score") || "4200",
      10,
    );

    let blaze = parseInt(
      localStorage.getItem("skrimchat_blaze_run") || "12",
      10,
    );
    ...
    let followers = parseInt(
      localStorage.getItem("skrimchat_followers") || "850",
      10,
    );
  ...
  useEffect(() => {
    ...
    setSavedItems(allContent.filter((item) => savedPosts.includes(item.id)));
    setRepostItems(
      allContent.filter((item) => repostedPosts.includes(item.id)),
    );
    ...
    setActiveHighlightGroup([
      {
        ...
        user: {
          ...
          avatar:
            h.cover ||
            h.sparks?.[0]?.image ||
            h.sparks?.[0]?.backgroundTheme ||
            h.sparks?.[0]?.background,
        },
        sparks: h.sparks || [],
        hasViewed: true,
        isOwn: true,
  ...
  return (
    <div className="w-full h-full overflow-y-auto no-scrollbar bg-skrim-bg relative pb-20">
      <input
        ...
        className="hidden"
        accept="image/*"
        onChange={(e) => handleQuickFileChange(e, "avatar")}
      />
      <input
        type="file"
        ref={coverInputRef}
        className="hidden"
        accept="image/*"
        onChange={(e) => handleQuickFileChange(e, "cover")}
      />

      {/* SECTION 1 - Profile Hero */}
      <div
        className="relative w-full h-[160px] md:h-[200px] cursor-pointer group"
      ...
      >
        ...
        {/* Vibe Aura + Avatar */}
        <div
          className="absolute -bottom-[45px] left-6 z-20"
        ...
        >
          <div className="relative">
            ...
            <div className="relative w-[90px] h-[90px] rounded-full p-[3px] bg-gradient-to-br from-[#B026FF] to-[#00F0FF]">
              ...
              <div
                className="absolute bottom-1 right-1 w-6 h-6 bg-[#B026FF] rounded-full border-2 border-skrim-bg flex items-center justify-center cursor-pointer shadow-[0_0_10px_#B026FF]"
                onClick={() => avatarInputRef.current?.click()}
              >
      ...
      <div className="px-6 pt-14 pb-4 relative z-10">
        <div className="flex justify-between items-start">
          <div>
            ...
            {user.bio ? (
              ...
              <p
                className="text-sm text-gray-500 mb-3 cursor-pointer hover:text-gray-400"
        ...
        {/* SECTION 3.25 - Community Memberships */}
        <div className="mb-8 w-[calc(100%+48px)] -ml-6">
          <div className="flex gap-3 overflow-x-auto no-scrollbar px-6">
            <div className="flex-shrink-0 flex items-center gap-2 bg-[#1A1A1A] border border-[#D4AF37]/30 rounded-full px-3 py-1.5 shadow-[0_0_15px_rgba(212,175,55,0.1)]">
               <span className="text-sm">💎</span>
               <span className="text-[11px] font-bold text-[#D4AF37] uppercase tracking-wider">GrindMode</span>
            </div>
            <div className="flex-shrink-0 flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-3 py-1.5">
               <span className="text-sm">🎮</span>
               <span className="text-[11px] font-bold text-white/70 uppercase tracking-wider">Valorant India</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 items-start">
            {highlights.map((h) => {
              ...
              const bgs: Record<string, string> = {
                ...
                dark: "#121212",
                "orange-red":
                  "linear-gradient(to bottom right, #FF8A00, #FF0000)",
                "cyan-blue":
                  "linear-gradient(to bottom right, #00FFFF, #0000FF)",
                "green-teal":
                  "linear-gradient(to bottom right, #00FF00, #008080)",
                "pink-purple":
                  "linear-gradient(to bottom right, #FF00FF, #800080)",
                "gold-orange":
              ...
              const bgStyle = isImage
                ...
                : {
                    background:
                      cover?.includes("gradient") || cover?.startsWith("#")
                        ? cover
                        : bgs[cover] || bgs["purple"],
                  };
              ...
              return (
                ...
                >
                  <HighlightAvatar
                    emoji={h.emoji || "✨"}
                    theme={bgStyle.background as string}
                    size={64}
                  />
                  <span className="text-xs font-semibold text-gray-300 pointer-events-none mt-1">
                    {h.title}
                  </span>
                </motion.div>
              );
        ...
        {[
          {
            ...
            glow: "group-hover:border-yellow-400/50 group-hover:shadow-[0_0_15px_rgba(250,204,21,0.2)]",
            label: "Pulse Score",
            type: "pulse",
            val: statsData.pulse,
            suffix: "",
          ...
          },
          {
            ...
            glow: "group-hover:border-orange-500/50 group-hover:shadow-[0_0_15px_rgba(249,115,22,0.2)]",
            label: "Blaze Run",
            type: "blaze",
            val: statsData.blaze,
            suffix: " days",
          ...
          },
          {
            icon: Eye,
            color: "text-blue-400",
            glow: "group-hover:border-blue-400/50 group-hover:shadow-[0_0_15px_rgba(96,165,250,0.2)]",
            label: "Profile Views",
            type: "views",
            val: statsData.views,
            suffix: "",
          ...
          >
            ...
            <stat.icon
              className={`w-5 h-5 mb-3 ${stat.color} drop-shadow-[0_0_8px_currentColor] relative z-10`}
            />
      ...
      <div className="flex overflow-x-auto no-scrollbar gap-3 px-6 mb-8">
        {[
          ...
          { icon: Users, label: "Worlds", active: false, path: "/worlds" },
          {
            icon: BarChart3,
            label: "Creator",
            active: false,
        ...
        <div
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-full cursor-pointer transition active:scale-95 bg-red-500/10 border border-red-500/30 text-red-500 hover:text-white hover:border-red-500 hover:drop-shadow-[0_0_8px_rgba(239,68,68,0.8)] active:bg-red-500"
        >
      ...
      <div className="mb-8">
        <div className="px-6 flex justify-between items-center mb-4">
          ...
          <span
            ...
            className="text-xs text-[#B026FF] cursor-pointer hover:underline font-semibold"
          >
            See all
          </span>
        </div>
        <div className="flex overflow-x-auto no-scrollbar gap-4 px-6 snap-x">
          {mockUsers.slice(3, 8).map((mu, i) => (
            <motion.div
              ...
              key={mu.id}
              onClick={() =>
                navigate(`/profile/${mu.username.replace("@", "")}`)
              }
            ...
            >
              ...
              <div className="w-16 h-16 rounded-full p-0.5 bg-gradient-to-br from-[#B026FF] to-[#00F0FF] mb-3">
                <img
                  src={mu.avatar}
                  alt={mu.username}
                  className="w-full h-full rounded-full border-2 border-skrim-bg object-cover"
                />
              </div>
              <div className="flex items-center gap-1 w-full justify-center">
                ...
                <BadgeRow
                  stats={generateMockStatsForBadge(mu.username)}
                  isSmall={true}
                />
              </div>
              ...
              <FollowButton
                ...
                initialCount={mu.followers}
                className="w-full justify-center h-8 text-[11px]"
              />
            </motion.div>
          ))}
        </div>
            ...
            >
              ...
              {activeTab === tab.id && (
                <motion.div
                  ...
                  className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-[#B026FF] to-[#00F0FF] shadow-[0_-2px_10px_rgba(176,38,255,0.8)]"
                />
              )}
            </button>
          ))}
                ...
                <motion.div
                  ...
                  onClick={() =>
                    setSelectedMedia({
                      ...
                      urls: posts.map(
                        (_, idx) =>
                          `https://picsum.photos/400/400?random=${idx + 10}`,
                      ),
                    })
                  }
                >
                  ...
                  onClick={() =>
                    setSelectedMedia({
                      ...
                      urls: posts
                        ...
                        .map(
                          (_, idx) =>
                            `https://picsum.photos/400/700?random=${idx + 20}`,
                        ),
                    })
                  }
                >
        ...
        {activeTab === "saved" && (
          <div className="flex flex-col">
            ...
            {activeSavedTab === "bookmarks" && (
              <div className="grid grid-cols-3 gap-0.5 pt-0.5">
                {savedItems.length === 0 ? (
                  <div className="col-span-3 text-center py-20 text-gray-500 text-sm">
                    No saved posts yet.
                  </div>
                ) : (
                  savedItems.map((item, i) => {
                    const url =
                      item.image || item.videoImageHover || item.videoImage;
                      ...
                      <motion.div
                        ...
                        onClick={() =>
                          setSelectedMedia({
                            ...
                            urls: savedItems.map(
                              (it) =>
                                it.image || it.videoImageHover || it.videoImage,
                            ),
                            users: savedItems.map((it) => ({
                              username:
                                it.handle ||
                                it.userName ||
                                it.user?.username ||
                                "@someone",
                              avatar:
                                it.avatar || it.userAvatar || it.user?.avatar,
                            })),
                          })
                        }
                      >
                        ...
                        <div className="absolute top-2 right-2">
                          <Bookmark className="w-4 h-4 fill-[#00F0FF] text-[#00F0FF] drop-shadow-md" />
                        </div>
                      </motion.div>
                    );
                  })
              ...
              <div className="grid grid-cols-3 gap-0.5 pt-0.5">
                {offlineVibes.length === 0 ? (
                  <div className="col-span-3 text-center py-24 text-gray-500 text-sm flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="font-medium text-white/50 text-base flex flex-col">
                      No offline vibes downlaoded.{" "}
                      <span className="text-xs mt-1">
                        Save vibes to watch without internet.
                      </span>
                    </p>
                  </div>
                ) : (
                    ...
                    return (
                      <motion.div
                        ...
                        onContextMenu={(e) => {
                          ...
                          if (
                            window.confirm(
                              `Delete offline vibe: ${item.caption}?`,
                            )
                          ) {
                            deleteVibe(item.id);
                          }
                      ...
                      >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/90 to-transparent">
                          {(item.creator?.username || item.creator?.handle) && (
                            <p className="text-white text-xs font-semibold truncate drop-shadow">
                              {item.creator.username || item.creator.handle}
                            </p>
                          )}
                          {item.caption && (
                            <p className="text-white/80 text-[10px] truncate mt-0.5 drop-shadow">
                              {item.caption}
                            </p>
                          )}
                        </div>
                  ...
                  <motion.div
                    ...
                    onClick={() =>
                      setSelectedMedia({
                        ...
                        urls: repostItems.map(
                          (it) =>
                            it.image || it.videoImageHover || it.videoImage,
                        ),
                        users: repostItems.map((it) => ({
                          username:
                            it.handle ||
                            it.userName ||
                            it.user?.username ||
                            "@someone",
                          avatar: it.avatar || it.userAvatar || it.user?.avatar,
                        })),
                      })
                    }
                  >
                    ...
                    <div className="absolute top-2 right-2">
                      <Repeat className="w-4 h-4 text-green-400 drop-shadow-md" />
                    </div>
                  </motion.div>
                );
              })
                  ...
                  onClick={() =>
                    setSelectedMedia({
                      ...
                      urls: posts
                        ...
                        .map(
                          (_, idx) =>
                            `https://picsum.photos/400/400?random=${idx + 30 + activeTab.charCodeAt(0)}`,
                        ),
                    })
                  }
                >
                  ...
                  <div className="absolute bottom-2 left-2">
                    <UserIcon className="w-4 h-4 fill-white text-white drop-shadow-md" />
                  </div>
                </motion.div>
              );
            })}
        ...
        <div className="fixed inset-0 z-[120] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="text-sm text-gray-400 mb-6 px-2">
              Are you sure you want to log out of SkrimChat?
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowLogoutConfirm(false)}
                className="flex-1 py-2.5 rounded-xl border border-white/10 text-white font-semibold text-sm hover:bg-white/5 transition active:scale-95"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-red-500/20 text-red-500 border border-red-500/50 font-semibold text-sm hover:bg-red-500/30 transition active:scale-95"
              >
                Log Out
              </button>
            </div>
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-white/10 flex justify-between items-center bg-black/40">
              ...
              <button
                ...
                className="text-[#B026FF] hover:text-[#00F0FF] transition font-bold"
              >
                Save
              </button>
            </div>
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  >
                    <img
                      ...
                      alt="Avatar"
                      className="w-24 h-24 rounded-full object-cover border-2 border-white/20 group-hover:opacity-50 transition bg-black"
                    />
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={editUsername}
                      onChange={(e) =>
                        setEditUsername(e.target.value.replace(/@/g, ""))
                      }
                      className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-3 text-white focus:outline-none focus:border-[#B026FF] transition-colors"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#B026FF] transition-colors min-h-[100px] resize-none"
        ...
        <SparkViewer
          ...
          onDelete={(sparkId) => {
            ...
            setActiveHighlightGroup((prev) => {
              ...
              const newSparks = prev[0].sparks.filter(
                (s: any) => s.id !== sparkId,
              );
                    ...
                    setConfirmDialog({
                      ...
                      onConfirm: () => {
                        const updated = highlights.filter(
                          (h) => h.id !== highlightToDelete.id,
                        );
                        localStorage.setItem(
                          "skrimchat_highlights",
          ...
          <>
            ...
            >
              ...
              <div className="p-4 flex flex-col gap-2 overflow-y-auto pb-8">
                <button
                  className="flex items-center gap-4 p-4 hover:bg-white/5 rounded-2xl transition"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-white/50 text-sm">
                      Video Quality, Auto-play limits
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-white/50 text-sm">
                      Regional boost, Content filtering
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    <p className="font-bold text-white text-lg">Log Out</p>
                    <p className="text-white/50 text-sm">
                      Switch accounts securely
                    </p>
                  </div>
                </button>
              </div>
          ...
          <>
            ...
            >
              ...
              <div className="px-6 flex items-center gap-3 pb-4 shrink-0 border-b border-white/5">
                ...
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <Zap className="w-5 h-5 text-[#1DB954]" /> Video Quality
                </h2>
              </div>
              ...
              <div className="p-4 flex flex-col gap-2 overflow-y-auto pb-8">
                {DATA_MODES.map((mode) => (
                  <button
                    ...
                    onClick={() => {
                      setDataMode(mode.id);
                      setToastMessage(`Switched to ${mode.name} mode`);
                      setTimeout(() => setShowDataSettings(false), 300);
                    }}
                  >
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${dataMode === mode.id ? "bg-[#1DB954]/20 text-[#1DB954]" : "bg-white/10 text-white"}`}
                    >
                      <span className="text-xl">{mode.icon}</span>
                    </div>
                    <div className="flex-1">
                      <p
                        className={`font-bold ${dataMode === mode.id ? "text-[#1DB954]" : "text-white"}`}
                      >
                        {mode.name}
                      </p>
                    ...
                    {dataMode === mode.id && (
                      <Check className="w-5 h-5 text-[#1DB954] mt-2 shrink-0" />
                    )}
                  </button>
                ))}
              </div>
          ...
          <>
            ...
            >
              ...
              <div className="px-6 flex items-center gap-3 pb-4 shrink-0 border-b border-white/5">
                ...
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <Repeat className="w-5 h-5 text-[#B026FF]" /> Feed Preferences
                </h2>
              </div>
                ...
                <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
                  <div className="flex-1 pr-4">
                    ...
                    <p className="text-white/60 text-sm mt-1">
                      Boost content from creators in your country and state, and
                      who speak your primary language. Default ON.
                    </p>
                  </div>

                  <button
                    onClick={() => setRegionalBoost(!regionalBoostEnabled)}
                    className={`relative w-14 h-8 rounded-full transition-colors ${regionalBoostEnabled ? "bg-[#B026FF]" : "bg-white/20"}`}
                  >
                    <div
                      className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-all ${regionalBoostEnabled ? "left-7" : "left-1"}`}
                    />
                  </button>
                </div>
              </div>
          ...
          <>
            ...
            >
              ...
              <div className="px-6 flex items-center gap-3 pb-4 shrink-0 border-b border-white/5">
                ...
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <Bell className="w-5 h-5 text-[#00F0FF]" /> Notifications
                </h2>
              </div>
              ...
              <div className="p-4 flex flex-col gap-2 overflow-y-auto pb-8">
                <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
                  <div className="flex-1 pr-4">
                    ...
                    <p className="text-white/60 text-sm mt-1">
                      Receive push notifications and in-app alerts when people
                      you follow post new Vibes.
                    </p>
                  </div>

                  <button
                    onClick={() => toggleGlobalVibeNotifications()}
                    className={`relative w-14 h-8 rounded-full transition-colors ${globalVibeNotificationsEnabled ? "bg-[#00F0FF]" : "bg-white/20"}`}
                  >
                    <div
                      className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-all ${globalVibeNotificationsEnabled ? "left-7" : "left-1"}`}
                    />
                  </button>
                </div>

                <div className="flex flex-col gap-0 bg-white/5 rounded-2xl border border-white/5 mt-2 overflow-hidden">
                  <div className="flex items-center justify-between p-4 border-b border-white/5">
                    ...
                    <button
                      onClick={() =>
                        useNotificationStore
                          .getState()
                          .toggleLikesNotifications(
                            !useNotificationStore.getState()
                              .likesNotificationsEnabled,
                          )
                      }
                      className={`relative w-14 h-8 rounded-full transition-colors ${useNotificationStore.getState().likesNotificationsEnabled ? "bg-[#00F0FF]" : "bg-white/20"}`}
                    >
                      <div
                        className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-all ${useNotificationStore.getState().likesNotificationsEnabled ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                  ...
                  {useNotificationStore.getState()
                    .likesNotificationsEnabled && (
                    <div className="flex items-center justify-between p-4 pl-8 bg-black/20">
                      <div className="flex-1 pr-4">
                        <p className="font-medium text-white text-md flex items-center gap-2">
                          Only milestone likes
                        </p>
                        <p className="text-white/50 text-xs mt-1">
                          Limit notifications to major milestones (10, 100,
                          1K...).
                        </p>
                      </div>
                      ...
                      <button
                        onClick={() =>
                          useNotificationStore
                            .getState()
                            .toggleLikesMilestonesOnly(
                              !useNotificationStore.getState()
                                .likesMilestonesOnly,
                            )
                        }
                        className={`relative w-12 h-6 rounded-full transition-colors ${useNotificationStore.getState().likesMilestonesOnly ? "bg-[#1DB954]" : "bg-white/20"}`}
                      >
                        <div
                          className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-all ${useNotificationStore.getState().likesMilestonesOnly ? "left-7" : "left-1"}`}
                        />
                      </button>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between p-4 border-b border-white/5">
                    ...
                    <button
                      onClick={() =>
                        useNotificationStore
                          .getState()
                          .toggleCommentsNotifications(
                            !useNotificationStore.getState()
                              .commentsNotificationsEnabled,
                          )
                      }
                      className={`relative w-14 h-8 rounded-full transition-colors ${useNotificationStore.getState().commentsNotificationsEnabled ? "bg-[#00F0FF]" : "bg-white/20"}`}
                    >
                      <div
                        className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-all ${useNotificationStore.getState().commentsNotificationsEnabled ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-black/20">
                    ...
                    <button
                      onClick={() =>
                        useNotificationStore
                          .getState()
                          .toggleRepliesNotifications(
                            !useNotificationStore.getState()
                              .repliesNotificationsEnabled,
                          )
                      }
                      className={`relative w-12 h-6 rounded-full transition-colors ${useNotificationStore.getState().repliesNotificationsEnabled ? "bg-[#1DB954]" : "bg-white/20"}`}
                    >
                      <div
                        className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-all ${useNotificationStore.getState().repliesNotificationsEnabled ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                </div>
                  ...
                  <div className="flex items-center justify-between p-4 border-b border-white/5">
                    ...
                    <button
                      onClick={() =>
                        useNotificationStore
                          .getState()
                          .toggleBlazeRunReminders(
                            !useNotificationStore.getState()
                              .blazeRunRemindersEnabled,
                          )
                      }
                      className={`relative w-14 h-8 rounded-full transition-colors ${useNotificationStore.getState().blazeRunRemindersEnabled ? "bg-[#00F0FF]" : "bg-white/20"}`}
                    >
                      <div
                        className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-all ${useNotificationStore.getState().blazeRunRemindersEnabled ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                  ...
                  {useNotificationStore.getState().blazeRunRemindersEnabled && (
                    <div className="flex items-center justify-between p-4 pl-8 bg-black/20">
                      ...
                      <input
                        ...
                        value={
                          useNotificationStore.getState().blazeRunReminderTime
                        }
                        onChange={(e) =>
                          useNotificationStore
                            .getState()
                            .setBlazeRunReminderTime(e.target.value)
                        }
                      />
                    </div>
                  )}
                ...
                <div className="flex flex-col gap-0 bg-white/5 rounded-2xl border border-white/5 mt-2 overflow-hidden">
                  <div className="flex items-center justify-between p-4 border-b border-white/5">
                    ...
                    <button
                      ...
                      className={`relative w-14 h-8 rounded-full transition-colors ${useNotificationStore.getState().pulseRewardsEnabled ? "bg-[#00F0FF]" : "bg-white/20"}`}
                    >
                      <div
                        className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-all ${useNotificationStore.getState().pulseRewardsEnabled ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-0 bg-white/5 rounded-2xl border border-white/5 mt-2 overflow-hidden">
                  <div className="flex items-center justify-between p-4 border-b border-white/5">
                    <div className="flex-1 pr-4">
                      ...
                      <p className="text-white/60 text-sm mt-1">
                        Get notified when fresh vibes drop in your detected
                        language.
                      </p>
                    </div>
                    ...
                    <button
                      onClick={() =>
                        useNotificationStore
                          .getState()
                          .toggleLanguageMatchNotifications(
                            !useNotificationStore.getState()
                              .languageMatchNotificationsEnabled,
                          )
                      }
                      className={`relative w-14 h-8 rounded-full transition-colors ${useNotificationStore.getState().languageMatchNotificationsEnabled ? "bg-[#00F0FF]" : "bg-white/20"}`}
                    >
                      <div
                        className={`w-6 h-6 rounded-full bg-white absolute top-1 transition-all ${useNotificationStore.getState().languageMatchNotificationsEnabled ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                </div>
