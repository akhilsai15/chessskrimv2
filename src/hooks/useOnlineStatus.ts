...
export function initMockUsersOnlineToggle() {
  ...
  setInterval(
    () => {
      const statusesStr = localStorage.getItem(
        "skrimchat_mock_online_statuses",
      );
      if (statusesStr) {
...
export function useIsOnline(username?: string) {
  ...
  useEffect(() => {
    ...
    const isCurrentUser =
      normUser === currentUsername.replace("@", "").toLowerCase();

    if (isCurrentUser) {
      const checkCurrentUser = () => {
        const lastActive = parseInt(
          localStorage.getItem("skrimchat_last_active") || "0",
          10,
        );
        const now = Date.now();
        if (document.hidden) {
